...
let emit_version ppf () =
  fprintf ppf "// Code generated by Stan version 2.18.0"
...
let emit_udf ppf {returntype; name; arguments; body} =
  fprintf ppf "%s"
    (string_of_sexp
       [%sexp ({returntype; name; arguments; body} : Mir.udf_defn)])

let emit_class ppf p = fprintf ppf "XXXclass %s" p.prog_name

let emit_namespaced ppf (p : Mir.prog) =
  fprintf ppf "@[<v>@ %a@ %a@ namespace %s_model_namespace {@ %a @ %a@ }@ @]"
    emit_version () emit_includes () p.prog_name (pp_print_list emit_udf)
    p.functions emit_class p
