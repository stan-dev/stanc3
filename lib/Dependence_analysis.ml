...
let reaching_defn_lookup
    (rds : reaching_defn Set.Poly.t)
    (var : vexpr) : label Set.Poly.t =
  Set.Poly.map
    (Set.Poly.filter
       rds
       ~f:(fun (var', _) -> var' = var))
...
let node_immediate_dependencies
    (statement_map :
       (label,
        (expr_typed_located, label) statement * node_dep_info)
         Map.Poly.t)
    (label : label) : label Set.Poly.t =
  let (stmt, info) = Map.Poly.find_exn statement_map label in
  let rhs_set = stmt_rhs_var_set stmt in
  let rhs_deps = union_map rhs_set ~f:(reaching_defn_lookup info.reaching_defn_entry) in
...
let all_node_dependencies
    ...
    =
    let immediate_map = Map.mapi statement_map ~f:(fun ~key:label ~data:_ ->
        node_immediate_dependencies statement_map label)
    in
    let step_node label m =
      let immediate = Map.find_exn immediate_map label in
      let updated = Set.union (union_map immediate ~f:(fun label -> Map.find_exn m label)) immediate in
...
let mir_reaching_definitions (mir : typed_prog) (stmt : stmt_loc)
  ...
  in
  let to_rd_set set = Set.Poly.map set ~f:(fun (s, label_opt) -> (VVar s, Option.value label_opt ~default:0)) in
  Map.Poly.map rd_map ~f:(fun {entry; exit} -> {entry= to_rd_set entry; exit= to_rd_set exit})

let prog_build_dep_info_map
    (mir : Mir.typed_prog)
    : (label, (expr_typed_located, label) statement * node_dep_info) Map.Poly.t =
...
let prog_dependency_graph
    (mir : Mir.typed_prog)
  : (label, label Set.Poly.t) Map.Poly.t =
  let dep_info_map = prog_build_dep_info_map mir in
  all_node_dependencies dep_info_map
...
let%expect_test "Dependency graph example" =
  ...
  let deps = prog_dependency_graph example1_program in
  print_s
    [%sexp
      (deps : (label, label Set.Poly.t) Map.Poly.t)] ;
  [%expect
    {|
