...
let rec emit_statement ppf s =
  ...
  | Assignment {assignee; indices; op; rhs} ->
      fprintf ppf "%s%a%a%a" assignee
        (pp_print_list ~pp_sep:comma emit_index)
        indices emit_assign_op op emit_expr rhs
  | NRFnApp (fname, args) ->
      fprintf ppf "%s(%a)" fname (pp_print_list ~pp_sep:comma emit_expr) args
...
let rec emit_namespaced cppclass functions usings ppf = function
  | [] ->
      fprintf ppf "%a@ %a@ %a"
        (pp_print_list ~pp_sep:newline emit_using)
        usings
        (pp_print_list ~pp_sep:newline emit_fndef)
        functions emit_class cppclass
  | ns :: tl ->
      fprintf ppf "namespace %s {@ %a@ }" ns
