...
let options =
  Arg.align
    ...
    ; ( "--debug-optimized-mir"
      ...
      , " For debugging purposes: print the MIR after it's been optimized. Only has an \
         effect when optimizations are turned on." )
    ; ( "--debug-optimized-mir-pretty"
      , Arg.Set dump_opt_mir_pretty
      , " For debugging purposes: pretty print the MIR after it's been optimized. Only \
         has an effect when optimizations are turned on." )
    ...
    ; ( "--debug-transformed-mir-pretty"
      ...
      , " For debugging purposes: pretty print the MIR after the backend has transformed \
         it." )
    ...
    ; ( "--name"
      ...
      , " Take a string to set the model name (default = \"$model_filename_model\")" )
    ; ( "--O"
      , Arg.Set optimize
    ...
    ; ( "--include-paths"
      , Arg.String
          (fun str -> Preprocessor.include_paths := String.split_on_chars ~on:[ ',' ] str)
      , " Takes a comma-separated list of directories that may contain a file in an \
...
let print_deprecated_arg_warning =
  ...
  in
  if arg_is_used "--allow_undefined"
  then eprintf "--allow_undefined is deprecated. Please use --allow-undefined.\n";
  if arg_is_used "--include_paths"
  then eprintf "--include_paths is deprecated. Please use --include-paths.\n"
;;
...
let use_file filename =
  ...
  if !pretty_print_program then print_endline (Pretty_printing.pretty_print_program ast);
  ...
  if !generate_data then print_endline (Debug_data_generation.print_data_prog typed_ast);
  Debugging.typed_ast_logger typed_ast;
  if not (!pretty_print_program || !canonicalize_program)
  then (
    let mir = Ast_to_Mir.trans_prog filename typed_ast in
    if !dump_mir
    then Sexp.pp_hum Format.std_formatter [%sexp (mir : Middle.Program.Typed.t)];
    ...
    if !dump_tx_mir
    then Sexp.pp_hum Format.std_formatter [%sexp (tx_mir : Middle.Program.Typed.t)];
    ...
    let opt_mir =
      ...
      then (
        ...
        if !dump_opt_mir
        then Sexp.pp_hum Format.std_formatter [%sexp (opt : Middle.Program.Typed.t)];
...
let main () =
  ...
  if !Semantic_check.model_name = ""
  then
    Semantic_check.model_name
      := remove_dotstan List.(hd_exn (rev (String.split !model_file ~on:'/'))) ^ "_model";
