...
module Make(X: Basic) : S with type t := X.t and module Pattern  := X.Pattern  = struct
open X 
let proj = proj 
...
val proj4 : 'a t ->       'a * ('a * ('a * ('a * 'a t Pattern.t) Pattern.t) Pattern.t) Pattern.t
val proj5 : 'a t -> 'a * ('a * ('a * ('a * ('a * 'a t Pattern.t) Pattern.t) Pattern.t) Pattern.t) Pattern.t
end
...
val meta : ('a,'b) t -> 'b 
val proj_succ : (('a,'b) t -> 'c) -> ('a,'b) t -> 'b * ('a First.t , 'c) Pattern.t
val proj2 : ('a,'b) t ->  'b * ('a First.t , 'b * ('a First.t , ('a,'b) t) Pattern.t) Pattern.t
val proj3 : ('a,'b) t ->  'b * ('a First.t , 'b * ('a First.t , 'b * ('a First.t , ('a,'b) t) Pattern.t) Pattern.t) Pattern.t

val proj4 : ('a,'b) t -> 'b * ('a First.t , 'b * ('a First.t , 'b * ('a First.t , 'b * ('a First.t , ('a,'b) t) Pattern.t) Pattern.t) Pattern.t) Pattern.t

val proj5 : ('a,'b) t -> 'b * ('a First.t , 'b * ('a First.t , 'b * ('a First.t , 'b * ('a First.t , 'b * ('a First.t , ('a,'b) t) Pattern.t) Pattern.t) Pattern.t) Pattern.t) Pattern.t
end 


module Make1(X: Basic1) : S1 with type 'a t := 'a X.t and module Pattern := X.Pattern = struct
open X
let proj = proj
...
module Make2(X: Basic2) : S2 with type ('a,'b) t := ('a,'b) X.t and module First := X.First and module Pattern := X.Pattern  = struct
open X 
let proj = proj 
let pattern x = snd @@ proj x 
let meta x = fst @@ proj x
