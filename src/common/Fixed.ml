...
module Make2 (First : S) (Pattern : Pattern.S2) :
  ...
  let rec fold_pattern ~f ~g { meta; pattern } =
    let pattern' = Pattern.map (First.fold_pattern ~f) (fold_pattern ~f ~g) pattern in
    g (meta, pattern')
  ;;
  ...
  let rec rewrite_bottom_up ~f ~g t =
    g
      { t with
        pattern =
          Pattern.map (First.rewrite_bottom_up ~f) (rewrite_bottom_up ~f ~g) t.pattern
      }
  ;;

  let rec unfold_pattern ~f ~g x =
    let meta, pattern = g x in
    let pattern' = Pattern.map (First.unfold_pattern ~f) (unfold_pattern ~f ~g) pattern in
  ...
  let rec rewrite_top_down ~f ~g x =
    let t = g x in
    { t with
      pattern = Pattern.map (First.rewrite_top_down ~f) (rewrite_top_down ~f ~g) t.pattern
    }
  ;;
end
