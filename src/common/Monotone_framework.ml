...
module Make
  ...
  *)
  let initialize initials extremal_value least_value flow =
    ...
    and f accu label =
      let data =
        if F.Label.Set.mem initials label then extremal_value else least_value
      in
  ...
  *)
  let update_analysis analysis label prop =
    F.Label.Map.update analysis label ~f:(fun prop_opt ->
        L.join (Option.value_exn prop_opt) prop )
  ...
  *)
  let solve {F.flowgraph; initials; associations} x =
    let least_value = L.least_element_of x
    and extremal_value = TF.extremal_value_of x
    ...
    let analysis = iterate all_props flowgraph associations init worklist in
    ( associations
