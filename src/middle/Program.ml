...
let pp_block label pp_elem ppf = function
  | [] -> ()
  | elems ->
    Fmt.pf ppf {|@[<v2>%a {@ %a@]@ }|} pp_keyword label Fmt.(list ~sep:cut pp_elem) elems;
...
module Labelled = struct
  ...
  let rec associate ?init:(assocs = empty) prog =
    ...
    let assoc_input_vars =
      List.fold_left prog.input_vars ~init:assoc_fundef ~f:(fun assocs (_, st) ->
          { assocs with exprs = SizedType.associate ~init:assocs.exprs st })
    in
    let assoc_prepare_data =
      List.fold_left prog.prepare_data ~init:assoc_input_vars ~f:(fun assocs stmt ->
          Stmt.Labelled.associate ~init:assocs stmt)
    in
    let assoc_log_prog =
      List.fold_left prog.log_prob ~init:assoc_prepare_data ~f:(fun assocs stmt ->
          Stmt.Labelled.associate ~init:assocs stmt)
    in
    let assoc_generate_quants =
      List.fold_left prog.generate_quantities ~init:assoc_log_prog ~f:(fun assocs stmt ->
          Stmt.Labelled.associate ~init:assocs stmt)
    in
    let assoc_transform_inits =
      List.fold_left
        prog.transform_inits
        ~init:assoc_generate_quants
  ...
  and associate_fun_def assocs { fdbody; _ } = Stmt.Labelled.associate ~init:assocs fdbody
