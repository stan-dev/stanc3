...
let rec bare_array_type (t, i) =
  match i with
  | 0 -> t
  | j -> UnsizedType.UArray (bare_array_type (t, j - 1))
;;
...
let mk_declarative_sig (fnkinds, name, args) =
  ...
  let create_from_fk_args fk arglists =
    List.concat_map arglists ~f:(fun args ->
        List.map (sfxes fk) ~f:(fun sfx -> name ^ sfx, find_rt UReal args fk, args))
  in
  let add_fnkind = function
    | Rng ->
      ...
      List.map (all_expanded args) ~f:(fun args -> name, find_rt rt args Rng, args)
...
let math_sigs =
  ...
  ; [ UnaryVectorized ], "tanh", [ DDeepVectorized ] (* ; add_nullary ("target") *)
...
let declarative_fnsigs = List.concat_map ~f:mk_declarative_sig all_declarative_sigs
...
let stan_math_returntype name args =
  ...
  | _ ->
    if List.length filteredmatches = 0
    then
      None
      ...
      Some
        (List.hd_exn
           (List.sort
              ~compare:UnsizedType.compare_returntype
...
let pp_math_sigs ppf name = (Fmt.list ~sep:Fmt.cut pp_math_sig) ppf (get_sigs name)
...
let pretty_print_all_math_sigs ppf () =
  ...
  let pp_sigs_for_name ppf name =
    (list ~sep:cut pp_sig) ppf (List.map ~f:(fun t -> name, t) (get_sigs name))
  in
  pf
    ppf
    "@[<v>%a@]"
...
let pretty_print_math_lib_operator_sigs op =
  if op = Operator.IntDivide
  then [ Fmt.strf "@[<v>@,%a@]" pp_math_sig int_divide_type ]
...
let add_ternary name = add_unqualified (name, ReturnType UReal, [ UReal; UReal; UReal ])
...
let () =
  ...
  for i = 0 to bare_types_size - 1 do
    add_unqualified ("add", ReturnType (bare_types i), [ bare_types i; bare_types i ])
  done;
  ...
  add_unqualified
    ("bernoulli_logit_glm_lpmf", ReturnType UReal, [ UInt; UMatrix; UReal; UVector ]);
  add_unqualified
    ("bernoulli_logit_glm_lpmf", ReturnType UReal, [ UInt; UMatrix; UVector; UVector ]);
  add_unqualified
    ( "bernoulli_logit_glm_lpmf"
    , ReturnType UReal
  ...
  for_vector_types (fun t ->
      for_all_vector_types (fun u ->
          add_unqualified
            ("beta_proportion_rng", ReturnType (rng_return_type UReal [ t; u ]), [ t; u ])));
  add_unqualified ("binary_log_loss", ReturnType UReal, [ UInt; UReal ]);
  add_binary "binomial_coefficient_log";
  add_unqualified ("block", ReturnType UMatrix, [ UMatrix; UInt; UInt; UInt; UInt ]);
  ...
  add_unqualified
    ("categorical_logit_glm_lpmf", ReturnType UReal, [ UInt; UMatrix; UVector; UMatrix ]);
  add_unqualified
    ( "categorical_logit_glm_lpmf"
    , ReturnType UReal
  ...
  add_unqualified ("append_col", ReturnType URowVector, [ URowVector; URowVector ]);
  ...
  add_unqualified ("columns_dot_product", ReturnType URowVector, [ UVector; UVector ]);
  add_unqualified
    ("columns_dot_product", ReturnType URowVector, [ URowVector; URowVector ]);
  add_unqualified ("columns_dot_product", ReturnType URowVector, [ UMatrix; UMatrix ]);
  ...
  add_unqualified
    ("cov_exp_quad", ReturnType UMatrix, [ bare_array_type (UReal, 1); UReal; UReal ]);
  add_unqualified
    ("cov_exp_quad", ReturnType UMatrix, [ bare_array_type (UVector, 1); UReal; UReal ]);
  add_unqualified
    ("cov_exp_quad", ReturnType UMatrix, [ bare_array_type (URowVector, 1); UReal; UReal ]);
  add_unqualified
    ( "cov_exp_quad"
    , ReturnType UMatrix
  ...
  add_unqualified
    ( "cov_exp_quad"
    , ReturnType UMatrix
    , [ bare_array_type (URowVector, 1); bare_array_type (URowVector, 1); UReal; UReal ]
    );
  ...
  add_unqualified
    ( "csr_to_dense_matrix"
    , ReturnType UMatrix
    , [ UInt; UInt; UVector; bare_array_type (UInt, 1); bare_array_type (UInt, 1) ] );
  add_unqualified ("csr_extract_w", ReturnType UVector, [ UMatrix ]);
  add_unqualified ("csr_extract_v", ReturnType (bare_array_type (UInt, 1)), [ UMatrix ]);
  add_unqualified ("csr_extract_u", ReturnType (bare_array_type (UInt, 1)), [ UMatrix ]);
  add_unqualified
    ( "cumulative_sum"
  ...
  add_unqualified ("diag_post_multiply", ReturnType UMatrix, [ UMatrix; URowVector ]);
  add_unqualified ("diag_pre_multiply", ReturnType UMatrix, [ UVector; UMatrix ]);
  add_unqualified ("diag_pre_multiply", ReturnType UMatrix, [ URowVector; UMatrix ]);
  ...
  for i = 0 to 8 - 1 do
    add_unqualified
      ("dims", ReturnType (bare_array_type (UInt, 1)), [ bare_array_type (UInt, i + 1) ]);
    add_unqualified
      ("dims", ReturnType (bare_array_type (UInt, 1)), [ bare_array_type (UReal, i + 1) ]);
    add_unqualified
      ( "dims"
  ...
  add_unqualified ("elt_divide", ReturnType URowVector, [ URowVector; URowVector ]);
  ...
  add_unqualified ("elt_multiply", ReturnType URowVector, [ URowVector; URowVector ]);
  ...
  add_unqualified
    ...
    , [ bare_array_type (UReal, 1); bare_array_type (UReal, 1); UReal ] );
  add_unqualified
    ("gp_dot_prod_cov", ReturnType UMatrix, [ bare_array_type (UVector, 1); UReal ]);
  add_unqualified
    ( "gp_dot_prod_cov"
    , ReturnType UMatrix
    , [ bare_array_type (UVector, 1); bare_array_type (UVector, 1); UReal ] );
  add_unqualified
    ("gp_exp_quad_cov", ReturnType UMatrix, [ bare_array_type (UReal, 1); UReal; UReal ]);
  add_unqualified
    ( "gp_exp_quad_cov"
    , ReturnType UMatrix
    , [ bare_array_type (UReal, 1); bare_array_type (UReal, 1); UReal; UReal ] );
  add_unqualified
    ("gp_exp_quad_cov", ReturnType UMatrix, [ bare_array_type (UVector, 1); UReal; UReal ]);
  add_unqualified
    ( "gp_exp_quad_cov"
    , ReturnType UMatrix
    , [ bare_array_type (UVector, 1); bare_array_type (UVector, 1); UReal; UReal ] );
  ...
  add_unqualified
    ("gp_matern32_cov", ReturnType UMatrix, [ bare_array_type (UReal, 1); UReal; UReal ]);
  add_unqualified
    ( "gp_matern32_cov"
    , ReturnType UMatrix
    , [ bare_array_type (UReal, 1); bare_array_type (UReal, 1); UReal; UReal ] );
  add_unqualified
    ("gp_matern32_cov", ReturnType UMatrix, [ bare_array_type (UVector, 1); UReal; UReal ]);
  add_unqualified
    ( "gp_matern32_cov"
    , ReturnType UMatrix
    , [ bare_array_type (UVector, 1); bare_array_type (UVector, 1); UReal; UReal ] );
  ...
  add_unqualified
    ("gp_matern52_cov", ReturnType UMatrix, [ bare_array_type (UReal, 1); UReal; UReal ]);
  add_unqualified
    ( "gp_matern52_cov"
    , ReturnType UMatrix
    , [ bare_array_type (UReal, 1); bare_array_type (UReal, 1); UReal; UReal ] );
  add_unqualified
    ("gp_matern52_cov", ReturnType UMatrix, [ bare_array_type (UVector, 1); UReal; UReal ]);
  add_unqualified
    ( "gp_matern52_cov"
    , ReturnType UMatrix
    , [ bare_array_type (UVector, 1); bare_array_type (UVector, 1); UReal; UReal ] );
  ...
  add_unqualified ("hmm_marginal", ReturnType UReal, [ UMatrix; UMatrix; UVector ]);
  add_qualified
  ...
  add_unqualified ("hypergeometric_log", ReturnType UReal, [ UInt; UInt; UInt; UInt ]);
  add_unqualified ("hypergeometric_lpmf", ReturnType UReal, [ UInt; UInt; UInt; UInt ]);
  ...
  add_unqualified ("inv_wishart_log", ReturnType UReal, [ UMatrix; UReal; UMatrix ]);
  add_unqualified ("inv_wishart_lpdf", ReturnType UReal, [ UMatrix; UReal; UMatrix ]);
  ...
  add_unqualified ("lkj_cov_log", ReturnType UReal, [ UMatrix; UVector; UVector; UReal ]);
  ...
  for i = 1 to vector_types_size - 1 do
    for j = 1 to vector_types_size - 1 do
      add_unqualified ("log_mix", ReturnType UReal, [ vector_types i; vector_types j ])
    done;
    add_unqualified
      ("log_mix", ReturnType UReal, [ vector_types i; bare_array_type (UVector, 1) ]);
    add_unqualified
      ("log_mix", ReturnType UReal, [ vector_types i; bare_array_type (URowVector, 1) ])
  done;
  add_binary "log_rising_factorial";
  add_unqualified ("log_softmax", ReturnType UVector, [ UVector ]);
  add_unqualified ("log_sum_exp", ReturnType UReal, [ bare_array_type (UReal, 1) ]);
  ...
  add_unqualified ("matrix_exp_multiply", ReturnType UMatrix, [ UMatrix; UMatrix ]);
  ...
  add_unqualified ("mdivide_left_tri_low", ReturnType UMatrix, [ UMatrix; UMatrix ]);
  add_unqualified ("mdivide_left_tri_low", ReturnType UVector, [ UMatrix; UVector ]);
  add_unqualified ("mdivide_right", ReturnType URowVector, [ URowVector; UMatrix ]);
  add_unqualified ("mdivide_right_spd", ReturnType UMatrix, [ UMatrix; UMatrix ]);
  add_unqualified ("mdivide_right_spd", ReturnType URowVector, [ URowVector; UMatrix ]);
  add_unqualified ("mdivide_right", ReturnType UMatrix, [ UMatrix; UMatrix ]);
  add_unqualified ("mdivide_right_tri_low", ReturnType URowVector, [ URowVector; UMatrix ]);
  add_unqualified ("mdivide_right_tri_low", ReturnType UMatrix, [ UMatrix; UMatrix ]);
  ...
  add_unqualified ("modified_bessel_first_kind", ReturnType UReal, [ UInt; UReal ]);
  add_unqualified ("modified_bessel_second_kind", ReturnType UReal, [ UInt; UReal ]);
  ...
  add_unqualified ("multi_normal_rng", ReturnType UVector, [ URowVector; UMatrix ]);
  add_unqualified
    ( "multi_normal_rng"
    , ReturnType (bare_array_type (UVector, 1))
    , [ bare_array_type (URowVector, 1); UMatrix ] );
  add_unqualified ("multi_normal_cholesky_rng", ReturnType UVector, [ UVector; UMatrix ]);
  add_unqualified
  ...
  add_unqualified
    ...
    , ReturnType (bare_array_type (UVector, 1))
    , [ bare_array_type (URowVector, 1); UMatrix ] );
  add_unqualified ("multi_student_t_rng", ReturnType UVector, [ UReal; UVector; UMatrix ]);
  add_unqualified
    ( "multi_student_t_rng"
    , ReturnType (bare_array_type (UVector, 1))
  ...
  add_unqualified ("multiply_lower_tri_self_transpose", ReturnType UMatrix, [ UMatrix ]);
  add_unqualified
  ...
  add_unqualified
    ("normal_id_glm_lpdf", ReturnType UReal, [ UVector; UMatrix; UReal; UVector; UReal ]);
  add_unqualified
    ("normal_id_glm_lpdf", ReturnType UReal, [ UVector; UMatrix; UVector; UVector; UReal ]);
  add_unqualified
    ("normal_id_glm_lpdf", ReturnType UReal, [ UReal; UMatrix; UReal; UVector; UVector ]);
  add_unqualified
    ("normal_id_glm_lpdf", ReturnType UReal, [ UReal; UMatrix; UVector; UVector; UVector ]);
  add_unqualified
    ( "normal_id_glm_lpdf"
    , ReturnType UReal
  ...
  for i = 1 to 10 - 1 do
    add_unqualified ("num_elements", ReturnType UInt, [ bare_array_type (UInt, i) ]);
    add_unqualified ("num_elements", ReturnType UInt, [ bare_array_type (UReal, i) ]);
    add_unqualified ("num_elements", ReturnType UInt, [ bare_array_type (UMatrix, i) ]);
    add_unqualified ("num_elements", ReturnType UInt, [ bare_array_type (URowVector, i) ]);
    add_unqualified ("num_elements", ReturnType UInt, [ bare_array_type (UVector, i) ])
  done;
  ...
  add_unqualified
    ...
    , [ bare_array_type (UInt, 1); URowVector; UVector; UVector ] );
  add_unqualified
    ("ordered_logistic_glm_lpmf", ReturnType UReal, [ UInt; UMatrix; UVector; UVector ]);
  add_unqualified
    ("ordered_logistic_glm_lpmf", ReturnType UReal, [ UInt; URowVector; UVector; UVector ]);
  add_unqualified ("ordered_logistic_log", ReturnType UReal, [ UInt; UReal; UVector ]);
  add_unqualified
  ...
  add_unqualified ("ordered_logistic_lpmf", ReturnType UReal, [ UInt; UReal; UVector ]);
  add_unqualified
  ...
  add_unqualified ("ordered_probit_log", ReturnType UReal, [ UInt; UReal; UVector ]);
  add_unqualified
    ( "ordered_probit_log"
    , ReturnType UReal
    , [ bare_array_type (UInt, 1); UVector; UVector ] );
  add_unqualified
    ...
    , ReturnType UReal
    , [ bare_array_type (UInt, 1); UVector; bare_array_type (UVector, 1) ] );
  add_unqualified ("ordered_probit_lpmf", ReturnType UReal, [ UInt; UReal; UVector ]);
  add_unqualified
    ( "ordered_probit_lpmf"
    , ReturnType UReal
  ...
  add_unqualified
    ("poisson_log_glm_lpmf", ReturnType UReal, [ UInt; UMatrix; UVector; UVector ]);
  add_unqualified
    ( "poisson_log_glm_lpmf"
    , ReturnType UReal
    , [ bare_array_type (UInt, 1); URowVector; UReal; UVector ] );
  ...
  for i = 0 to bare_types_size - 1 do
    add_unqualified
      ("rep_array", ReturnType (bare_array_type (bare_types i, 1)), [ bare_types i; UInt ]);
    add_unqualified
      ( "rep_array"
      , ReturnType (bare_array_type (bare_types i, 2))
  ...
  for i = 1 to 7 do
    add_unqualified
      ("reverse", ReturnType (bare_array_type (UInt, i)), [ bare_array_type (UInt, i) ]);
    add_unqualified
      ("reverse", ReturnType (bare_array_type (UReal, i)), [ bare_array_type (UReal, i) ]);
    add_unqualified
      ( "reverse"
      , ReturnType (bare_array_type (UMatrix, i))
  ...
  add_unqualified ("rows_dot_product", ReturnType UVector, [ URowVector; URowVector ]);
  ...
  add_unqualified
    ("sort_asc", ReturnType (bare_array_type (UInt, 1)), [ bare_array_type (UInt, 1) ]);
  add_unqualified
    ("sort_asc", ReturnType (bare_array_type (UReal, 1)), [ bare_array_type (UReal, 1) ]);
  add_unqualified ("sort_asc", ReturnType UVector, [ UVector ]);
  add_unqualified ("sort_asc", ReturnType URowVector, [ URowVector ]);
  add_unqualified
    ("sort_desc", ReturnType (bare_array_type (UInt, 1)), [ bare_array_type (UInt, 1) ]);
  add_unqualified
    ("sort_desc", ReturnType (bare_array_type (UReal, 1)), [ bare_array_type (UReal, 1) ]);
  ...
  add_unqualified
    ...
    , ReturnType (bare_array_type (UInt, 1))
    , [ bare_array_type (UReal, 1) ] );
  add_unqualified ("sort_indices_asc", ReturnType (bare_array_type (UInt, 1)), [ UVector ]);
  add_unqualified
  ...
  add_unqualified ("squared_distance", ReturnType UReal, [ URowVector; URowVector ]);
  ...
  add_unqualified ("sub_row", ReturnType URowVector, [ UMatrix; UInt; UInt; UInt ]);
  for i = 0 to bare_types_size - 1 do
    add_unqualified ("subtract", ReturnType (bare_types i), [ bare_types i; bare_types i ])
  done;
  ...
  add_unqualified ("to_array_1d", ReturnType (bare_array_type (UReal, 1)), [ UMatrix ]);
  add_unqualified ("to_array_1d", ReturnType (bare_array_type (UReal, 1)), [ UVector ]);
  add_unqualified ("to_array_1d", ReturnType (bare_array_type (UReal, 1)), [ URowVector ]);
  for i = 1 to 10 - 1 do
    add_unqualified
      ( "to_array_1d"
      , ReturnType (bare_array_type (UReal, 1))
  ...
  add_unqualified ("to_array_2d", ReturnType (bare_array_type (UReal, 2)), [ UMatrix ]);
  ...
  add_unqualified ("to_matrix", ReturnType UMatrix, [ URowVector; UInt; UInt; UInt ]);
  add_unqualified
    ("to_matrix", ReturnType UMatrix, [ bare_array_type (UReal, 1); UInt; UInt ]);
  add_unqualified
    ("to_matrix", ReturnType UMatrix, [ bare_array_type (UReal, 1); UInt; UInt; UInt ]);
  add_unqualified
    ("to_matrix", ReturnType UMatrix, [ bare_array_type (UInt, 1); UInt; UInt ]);
  add_unqualified
    ("to_matrix", ReturnType UMatrix, [ bare_array_type (UInt, 1); UInt; UInt; UInt ]);
  add_unqualified ("to_matrix", ReturnType UMatrix, [ bare_array_type (UReal, 2) ]);
  ...
  add_unqualified ("to_row_vector", ReturnType URowVector, [ bare_array_type (UReal, 1) ]);
  add_unqualified ("to_row_vector", ReturnType URowVector, [ bare_array_type (UInt, 1) ]);
  ...
  add_unqualified ("to_vector", ReturnType UVector, [ bare_array_type (UReal, 1) ]);
  add_unqualified ("to_vector", ReturnType UVector, [ bare_array_type (UInt, 1) ]);
  add_unqualified ("trace", ReturnType UReal, [ UMatrix ]);
  add_unqualified ("trace_gen_quad_form", ReturnType UReal, [ UMatrix; UMatrix; UMatrix ]);
  ...
  Hashtbl.iteri manual_stan_math_signatures ~f:(fun ~key ~data ->
      List.iter data ~f:(fun data -> Hashtbl.add_multi stan_math_signatures ~key ~data))
;;
