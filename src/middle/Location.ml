...
let context_to_string file =
  try Some (Fmt.to_to_string pp_context_exn file) with
  | _ -> None
;;
...
let rec of_string_opt str =
  let split_str =
    Str.bounded_split (Str.regexp ", line \\|, column \\|, included from\n") str 4
  in
...
let of_position_opt { Lexing.pos_fname; pos_lnum; pos_cnum; pos_bol } =
  let split_fname = Str.bounded_split (Str.regexp ", included from\n") pos_fname 2 in
...
let%expect_test "location string equivalence 2" =
  let loc : t =
    ...
    ; included_from =
        Some
          { filename = "yyy.stan"; line_num = 345; col_num = 214; included_from = None }
    }
  in
