  ...
  module Pattern = struct
    ...
    let pp pp_e pp_s ppf = function
      | Assignment ((assignee, _, idcs), rhs) ->
        Fmt.pf ppf {|@[<h>%a =@ %a;@]|} (Index.pp_indexed pp_e) (assignee, idcs) pp_e rhs
      | TargetPE expr -> Fmt.pf ppf {|@[<h>%a +=@ %a;@]|} pp_keyword "target" pp_e expr
...
module Labelled = struct
  module Meta = struct
    ...
    let empty = create ~loc:Location_span.empty ~label:Label.Int_label.(prev init) ()
    let pp _ _ = ()
  end
  ...
  let empty = { exprs = Label.Int_label.Map.empty; stmts = Label.Int_label.Map.empty }

  let rec associate ?init:(assocs = empty) ({ pattern; _ } as stmt : t) =
    associate_pattern
      { assocs with
        stmts = Label.Int_label.Map.add_exn assocs.stmts ~key:(label_of stmt) ~data:stmt
      }
      pattern
  ...
  and associate_pattern assocs = function
    ...
    | Assignment ((_, _, idxs), rhs) ->
      let exprs =
        Expr.Labelled.(
          associate rhs ~init:(List.fold ~f:associate_index ~init:assocs.exprs idxs))
      in
      { assocs with exprs }
    ...
    | For { lower; upper; body; _ } ->
      let exprs =
        Expr.Labelled.(associate ~init:(associate ~init:assocs.exprs lower) upper)
      in
      let assocs' = { assocs with exprs } in
  ...
  and associate_possibly_sized_type assocs = function
    | Type.Sized st -> { assocs with exprs = SizedType.associate ~init:assocs.exprs st }
    | Unsized _ -> assocs
  ;;
end
...
module Helpers = struct
  let ensure_var bodyfn (expr : Expr.Typed.t) meta =
    ...
    | _ ->
      ...
      let assign =
        { body with
          Fixed.pattern = Assignment ((symbol, Expr.Typed.type_of expr, []), expr)
        }
      in
      reset ();
  ...
  let internal_nrfunapp fn args meta =
    { Fixed.pattern = NRFunApp (CompilerInternal, Internal_fun.to_string fn, args); meta }
  ;;
  ...
  let mkfor upper bodyfn iteratee meta =
    let idx s =
      let meta = Expr.Typed.Meta.create ~type_:UInt ~loc:meta ~adlevel:DataOnly () in
    ...
    let stmt =
      Fixed.Pattern.Block [ bodyfn (Expr.Helpers.add_int_index iteratee (idx loopvar)) ]
    in
    reset ();
  ...
  let rec for_each bodyfn iteratee smeta =
    ...
    | UMatrix ->
      ...
      let emeta' = { emeta with Expr.Typed.Meta.type_ = UInt } in
      let rows =
        Expr.Fixed.{ meta = emeta'; pattern = FunApp (StanLib, "rows", [ iteratee ]) }
      in
