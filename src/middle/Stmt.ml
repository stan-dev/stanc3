...
*)
  let rec for_eigen st bodyfn var smeta =
    match st with
    | SizedType.SInt | SReal | SComplex | SVector _ | SVectorCL _ | SRowVector _ | SMatrix _
      ->
        bodyfn var
    | SArray (t, d) ->
...
*)
  let rec for_scalar st bodyfn var smeta =
    ...
    | SVector (_, d) | SVectorCL (_, d) | SRowVector (_, d) -> mk_for_iteratee d bodyfn var smeta
