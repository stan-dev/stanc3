...
let rec flow ?init:(accu = []) stmt =
  ...
  | While (_, body) ->
    ...
    let finals =
      List.map ~f:(fun l' -> l', cur_label) @@ Int_label.Set.to_list finals_body
    in
...
module Basic :
  Flowgraph.Basic
  with type t = Labelled.t
   and module Label = Int_label = struct
