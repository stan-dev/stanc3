...
module Labelled = struct
  ...
  *)
  ...
  and associate_pattern assocs = function
    ...
    | EAnd (e1, e2) | EOr (e1, e2) -> associate ~init:(associate ~init:assocs e2) e1
    | TernaryIf (e1, e2, e3) ->
...
module Helpers = struct
  ...
  let float i = { Fixed.meta = Typed.Meta.empty; pattern = Lit (Real, string_of_float i) }
  ...
  let internal_funapp fn args meta =
    { Fixed.meta; pattern = FunApp (CompilerInternal, Internal_fun.to_string fn, args) }
  ;;
  ...
  let%test "expr contains fn" = internal_funapp FnReadData [] () |> contains_fn FnReadData
  ...
  let%expect_test "infer type of indexed" =
    ...
    ; UArray UMatrix, [ Single loop_bottom; Single loop_bottom; Single loop_bottom ]
    ; UArray UMatrix, [ Upfrom loop_bottom; Single loop_bottom; Single loop_bottom ]
    ]
