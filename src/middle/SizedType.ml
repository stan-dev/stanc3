...
let rec pp pp_e ppf = function
  ...
  | SMatrix (d1_expr, d2_expr) ->
    Fmt.pf ppf {|matrix%a|} Fmt.(pair ~sep:comma pp_e pp_e |> brackets) (d1_expr, d2_expr)
  | SArray (st, expr) ->
    Fmt.pf
      ppf
      {|array%a|}
...
let rec associate ?init:(assocs = Label.Int_label.Map.empty) = function
  ...
  | SMatrix (e1, e2) -> Expr.Labelled.(associate ~init:(associate ~init:assocs e1) e2)
  | SArray (st, e) -> associate ~init:(Expr.Labelled.associate ~init:assocs e) st
;;
...
let rec inner_type = function
  | SArray (t, _) -> inner_type t
  | t -> t
;;
...
let%expect_test "dims" =
  let open Fmt in
  strf "@[%a@]" (list ~sep:comma string) (get_dims (SArray (SMatrix ("x", "y"), "z")))
  |> print_endline;
  [%expect {| z, x, y |}]
;;
