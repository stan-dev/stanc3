...
let propto_suffices =
  List.map ~f:(fun x -> proportional_to_distribution_infix ^ x) distribution_suffices
;;

let is_user_ident = Fn.non (String.is_suffix ~suffix:"__")

let is_distribution_name s =
  (not (String.is_suffix s ~suffix:"_cdf_log" || String.is_suffix s ~suffix:"_ccdf_log"))
  && List.exists ~f:(fun suffix -> String.is_suffix s ~suffix) distribution_suffices
;;
