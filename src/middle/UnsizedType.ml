...
let rec pp ppf = function
  ...
  | UFun (argtypes, rt) ->
    Fmt.pf
      ...
      pp_returntype
      rt
  | UMathLibraryFunction -> (pp_angle_brackets Fmt.string) ppf "Stan Math function"
...
let check_of_same_type_mod_conv name t1 t2 =
  if String.is_prefix name ~prefix:"assign_"
  then t1 = t2
  else (
    match t1, t2 with
    | UReal, UInt -> true
...
let rec check_of_same_type_mod_array_conv name t1 t2 =
  match t1, t2 with
  | UArray t1elt, UArray t2elt -> check_of_same_type_mod_array_conv name t1elt t2elt
...
let rec common_type = function
  ...
  | UArray t1, UArray t2 -> common_type (t1, t2) |> Option.map ~f:(fun t -> UArray t)
