...
let rec reduce l f x = if List.length(l) = 0 then x else reduce (List.tl l) f (f x (List.hd l))

let reducearray (sbt, l) = reduce l (fun y -> fun z -> SArray (y, z)) sbt
...
let construct_var_decl sbt id d ae = let sizes = match d with None -> [] | Some l -> l in
                                     match ae with Some a -> [VDecl (reducearray (sbt, sizes), id); Stmt (Assignment ((id, []), Assign, snd a))]   
                                                 | _ -> [VDecl (reducearray (sbt, sizes), id)]
                                                       
let construct_top_var_decl_no_assign tvt id d = let sizes = match d with None -> [] | Some l -> l
                                                in ((reducearray (fst tvt, sizes), snd tvt), id)

let construct_top_var_decl tvt id d ass = let sizes = match d with None -> [] | Some l -> l in
                                          match ass with Some a -> [TVDecl ((reducearray (fst tvt, sizes), snd tvt), id); TStmt (Assignment ((id, []), Assign, snd a))]   
