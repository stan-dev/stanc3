...
module Symbol : SYMBOL = struct
  type context_flags_record = {rng: bool; lp: bool} (* TODO: fix this to be more useful. Also add stack for this. *)

  type 'a state =
    { table: (string, 'a) Hashtbl.t
    ; stack: string Stack.t
    ; context_flags: context_flags_record ref }

  let initialize _ =
    { table= Hashtbl.create 123456
    ; stack= Stack.create ()
    ; context_flags= ref {rng= false; lp= false} }
