...
struct 
  ...
  let enter tab str ty =  Hashtbl.add (fst tab) str ty ; (snd tab) := str :: !(snd tab) (* recall that OCaml hash tables store a stack of all the values for each key; this would allow us to use shadowing; if we don't want shadowing we can add an extra check here. *)
  let look tab str = Hashtbl.find_opt (fst tab) str

  let begin_scope tab =  (snd tab) := "-" :: !(snd tab)  (* using a string '-' here that can never be used as an identifier to indicate that new scope is entered *)
  let end_scope tab =  while ((List.hd !(snd tab) ) <> "-" ) do (* we pop the stack down to where we entered the current scope and remove all variables defined since from the var map *)
