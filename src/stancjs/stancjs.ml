...
let print_warn_uninitialized (uninit_vars : (Location_span.t * string) Set.Poly.t) =
...
let stan2cpp model_name model_string =
  ...
  let semantic_err_to_string = function
    ...
    | Result.Error [] ->
      Result.Error
        "Semantic check failed but reported no errors. This should never happen."
  in
  Result.bind
    ast
    ~f:(Fn.compose semantic_err_to_string Semantic_check.semantic_check_program)
