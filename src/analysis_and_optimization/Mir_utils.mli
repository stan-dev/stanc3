...
val map_rec_expr_state
  :  ('s -> Expr.Typed.t Expr.Fixed.Pattern.t -> Expr.Typed.t Expr.Fixed.Pattern.t * 's)
  -> 's
  -> Expr.Typed.t
  -> Expr.Typed.t * 's
...
(**
   ...
   For use in RHS sets, not LHS assignment sets, except in a target term
*)
val index_var_set : Expr.Typed.t Index.t -> (vexpr * Expr.Typed.Meta.t) Set.Poly.t
...
val subst_expr : (string, Expr.Typed.t) Map.Poly.t -> Expr.Typed.t -> Expr.Typed.t
...
val subst_stmt : (string, Expr.Typed.t) Map.Poly.t -> Stmt.Located.t -> Stmt.Located.t

(** Substitute subexpressions occurring anywhere in a statement according to the provided Map. *)
val name_subst_stmt : (string, string) Map.Poly.t -> Stmt.Located.t -> Stmt.Located.t
...
val expr_subst_expr : Expr.Typed.t Expr.Typed.Map.t -> Expr.Typed.t -> Expr.Typed.t

(** Substitute subexpressions occurring anywhere in a statement according to the provided Map. *)
val expr_subst_stmt : Expr.Typed.t Expr.Typed.Map.t -> Stmt.Located.t -> Stmt.Located.t
...
val cleanup_empty_stmts : ('e, 's) Stmt.Fixed.t list -> ('e, 's) Stmt.Fixed.t list
