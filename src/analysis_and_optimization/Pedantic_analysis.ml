...
let list_hard_constrained (mir : Program.Typed.t)
  ...
  let constrained (e : bound_values) =
    match e with
    | { lower = `Lit 0.; upper = `Lit 1. } | { lower = `Lit -1.; upper = `Lit 1. } -> None
  ...
  Set.Poly.filter_map
    ~f:(fun (name, trans) ->
      Option.map ~f:(fun c -> name, c) (constrained (trans_bounds_values trans)))
    (parameter_set mir)
;;
...
let list_multi_twiddles (mir : Program.Typed.t)
  ...
  let collect_twiddle_stmt (stmt : Stmt.Located.t)
    ...
    | Stmt.Fixed.Pattern.TargetPE
        { pattern = Expr.Fixed.Pattern.FunApp (_, _, { pattern = Var vname; _ } :: _); _ }
  ...
  let multi_twiddles = Map.Poly.filter ~f:(fun s -> Set.Poly.length s <> 1) twiddles in
  Map.fold
    ~init:Set.Poly.empty
...
let var_deps
    info_map
    label
    ?expr:(expr_opt : Expr.Typed.t option = None)
...
let list_target_dependant_cf
    (info_map :
      (label, (Expr.Typed.t, label) Stmt.Fixed.Pattern.t * node_dep_info) Map.Poly.t)
    (targets : string Set.Poly.t)
  ...
  let cf_labels =
    Set.Poly.of_list
      (Map.Poly.keys (Map.Poly.filter info_map ~f:(fun (stmt, _) -> is_ctrl_flow stmt)))
  in
  Set.Poly.filter_map
    ~f:(fun label ->
      let deps = var_deps info_map label targets in
      if Set.Poly.is_empty deps
      then None
      else (
...
let list_arg_dependant_fundef_cf (mir : Program.Typed.t) (fun_def : 'a Program.fun_def)
  ...
  union_map cf_deps ~f:(fun (loc, names) ->
      Set.Poly.map names ~f:(fun name ->
          let ix, _ =
            Option.value_exn
              ~message:
                "INTERNAL ERROR: Pedantic mode found CF dependent on an arg,but the arg \
                 is mismatched. Please report a bug.\n"
...
let list_param_dependant_fundef_cf
                   ...
                   union_map (stmt_rhs stmt) ~f:(fun rhs_expr ->
                       expr_collect_exprs rhs_expr ~f:(fun rhs_subexpr ->
                           ...
                           | Expr.Fixed.Pattern.FunApp (UserDefined, fname, _)
                             when fname = fun_def.fdname -> Some (rhs_subexpr, label)
                           | _ -> None))
                 in
  ...
  let arg_exprs (fcall_expr : Expr.Typed.t) =
    ...
    | _ ->
      raise
        (Failure
           "In finding searching for parameter dependent functionarguments, mismatched \
            function. Please report a bug.\n")
  in
  ...
  union_map fun_calls ~f:(fun (fcall_expr, label) ->
      Set.Poly.filter_map (arg_exprs fcall_expr) ~f:(fun (cf_loc, arg_expr, arg_name) ->
...
let list_param_dependant_fundefs_cf (mir : Program.Typed.t)
    : (string * Location_span.t * string Set.Poly.t * string * Location_span.t) Set.Poly.t
  =
  let info_map = log_prob_build_dep_info_map mir in
...
let list_non_one_priors (fg : factor_graph) (mir : Program.Typed.t)
  ...
  let prior_set =
    Map.Poly.fold priors ~init:Set.Poly.empty ~f:(fun ~key:(VVar v) ~data:factors_opt s ->
...
let list_distributions (mir : Program.Typed.t) : dist_info Set.Poly.t =
  ...
  in
  stmts_collect_exprs
    (List.append mir.log_prob (List.map ~f:(fun f -> f.fdbody) mir.functions_block))
    ~f:take_dist
;;
...
let list_unscaled_constants (distributions_list : dist_info Set.Poly.t)
  ...
  let collect_unscaled_expr (arg : compiletime_val * Expr.Typed.Meta.t) =
    ...
    | Number (num, num_str), meta ->
      if is_unscaled_value num
      then Set.Poly.singleton (meta.loc, num_str)
      else Set.Poly.empty
    | _ -> Set.Poly.empty
  in
  union_map
...
let pp_warning ppf (loc, msg) =
  let loc_str =
    if loc = Location_span.empty then "" else " at " ^ Location_span.to_string loc
  in
...
let unscaled_constants_message (name : string) : string =
  Printf.sprintf
    "Argument %s suggests there may be parameters that are not unit scale; consider \
...
let nonsense_constrained_message (pname : string) : string =
  Printf.sprintf
    "Parameter %s has constraints that don't make sense. The lower bound should be \
...
let hard_constrained_message (pname : string) : string =
  Printf.sprintf
    "Your Stan program has a parameter %s with a lower and upper bound in its \
     declaration. These hard constraints are not recommended, for two reasons: (a) \
     Except when there are logical or physical constraints, it is very unusual for you \
     to be sure that a parameter will fall inside a specified range, and (b) The \
     infinite gradient induced by a hard constraint can cause difficulties for Stan's \
     sampling algorithm. As a consequence, we recommend soft constraints rather than \
     hard constraints; for example, instead of constraining an elasticity parameter to \
     fall between 0, and 1, leave it unconstrained and give it a normal(0.5,0.5) prior \
     distribution."
    pname
;;
...
let hard_constrained_warnings (mir : Program.Typed.t) =
  ...
  Set.Poly.map
    ~f:(fun (pname, c) ->
      ...
      | `NonsenseConstraint -> Location_span.empty, nonsense_constrained_message pname)
    pnames
;;
...
let multi_twiddles_warnings (mir : Program.Typed.t) =
  ...
  Set.Poly.map
    ~f:(fun (vname, locs) -> Set.Poly.min_elt_exn locs, multi_twiddles_message vname)
    twds
;;

let param_dependant_cf_message (plist : string Set.Poly.t) : string =
  let plistStr = String.concat ~sep:", " (Set.Poly.to_list plist) in
  Printf.sprintf "A control flow statement depends on parameter(s): %s." plistStr
;;

let param_dependant_cf_warnings (mir : Program.Typed.t) =
  let cfs = list_param_dependant_cf mir in
  Set.Poly.map ~f:(fun (loc, plist) -> loc, param_dependant_cf_message plist) cfs
;;
...
let param_dependant_fundef_cf_message
  ...
  let plistStr = String.concat ~sep:", " (Set.Poly.to_list plist) in
  Printf.sprintf
    "A control flow statement inside function %s depends on argument %s. At %s, the \
     value of %s depends on parameter(s): %s."
    fname
    arg_name
...
let non_one_priors_message (pname : string) (n : int) : string =
  if n = 0
  then Printf.sprintf "The parameter %s has no priors." pname
...
let uninitialized_warnings (mir : Program.Typed.t) =
  ...
  in
  Set.Poly.map ~f:(fun (loc, vname) -> loc, uninitialized_message vname) uninit_vars
;;

(* Print uninitialized warnings
   In case a user wants only this warning *)
let print_warn_uninitialized mir = print_warning_set (uninitialized_warnings mir)
...
let print_warn_pedantic (mir_unopt : Program.Typed.t) =
  ...
  let mir = Optimize.optimization_suite ~settings:settings_constant_prop mir_unopt in
