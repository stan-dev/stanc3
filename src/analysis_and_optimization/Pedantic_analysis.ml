...
let list_multi_twiddles (mir : Program.Typed.t) :
  ...
  let collect_twiddle_stmt (stmt : Stmt.Located.t) :
    ...
    | Stmt.Fixed.Pattern.TargetPE
        { pattern= Expr.Fixed.Pattern.FunApp (_, _, {pattern= Var vname; _} :: _)
        ; _ } ->
...
let var_deps info_map label ?expr:(expr_opt : Expr.Typed.t option = None)
  ...
  let dep_labels =
    ...
    | Some expr ->
        ...
        node_vars_dependencies info_map vars label in
...
let pp_warning ppf (loc, msg) =
  let loc_str =
    ...
    else " at " ^ Location_span.to_string loc in
...
let print_warn_pedantic (mir_unopt : Program.Typed.t) =
  ...
  let warning_set =
    Set.Poly.union_list
      ...
      ; distribution_warnings distributions_info ] in
