...
let is_int i Expr.Fixed.{ pattern; _ } =
  ...
  match pattern with
  | (Lit (Int, i) | Lit (Real, i)) when List.mem nums i ~equal:String.equal -> true
  | _ -> false
;;
    ...
    pattern =
      ...
      | FunApp (t, f, l) ->
        ...
        let get_fun_or_op_rt_opt name l' =
          ...
          |> Option.value_map
               ~f:(fun op ->
                 Stan_math_signatures.operator_stan_math_return_type op argument_types)
               ~default:(Stan_math_signatures.stan_math_returntype name argument_types)
        in
        ...
        try_partially_evaluate_to
          ...
          | ( "bernoulli_lpmf"
                    ...
                    FunApp
                      ...
                      , [ { pattern =
                              FunApp
                                ...
                                , "Plus__"
                                , [ alpha
                                  ; { pattern = FunApp (StanLib, "Times__", [ x; beta ])
                                    ; _
                                    }
                                  ] )
                          ; _
            ...
            FunApp (StanLib, "bernoulli_logit_glm_lpmf", [ y; x; alpha; beta ])
          | ( "bernoulli_lpmf"
            , [ y
              ; { pattern =
                    FunApp
          ...
          | ( "bernoulli_lpmf"
            ...
            when Expr.Typed.type_of x = UMatrix ->
            FunApp (StanLib, "bernoulli_logit_glm_lpmf", [ y; x; Expr.Helpers.zero; beta ])
          ...
          | "bernoulli_rng", [ { pattern = FunApp (StanLib, "inv_logit", [ alpha ]); _ } ]
          ...
          | ( "log"
            , [ { pattern =
                    FunApp
                      ( StanLib
                      , "Minus__"
                      , [ y; { pattern = FunApp (StanLib, "exp", [ x ]); _ } ] )
                ; _
                }
              ] )
          ...
          | "log", [ { pattern = FunApp (StanLib, "Minus__", [ y; x ]); _ } ]
            when is_int 1 y && not preserve_stability -> FunApp (StanLib, "log1m", [ x ])
          | ( "log"
            , [ { pattern =
                    FunApp
                      ( StanLib
          ...
          | "log", [ { pattern = FunApp (StanLib, "Plus__", [ y; x ]); _ } ]
            when is_int 1 y && not preserve_stability -> FunApp (StanLib, "log1p", [ x ])
          | ( "log"
            , [ { pattern =
                    FunApp
                      ( StanLib
            ...
            , [ { pattern =
                    FunApp
                      ...
                      , [ { pattern = FunApp (StanLib, "exp", [ x ]); _ }
                        ; { pattern = FunApp (StanLib, "exp", [ y ]); _ }
                        ] )
                ; _
                }
          ...
          | ( "log"
            , [ { pattern =
                    FunApp
                      (StanLib, "sum", [ { pattern = FunApp (StanLib, "exp", l); _ } ])
                ; _
                }
          ...
          | ( "neg_binomial_2_lpmf"
            ...
            when Expr.Typed.type_of x = UMatrix ->
            FunApp (StanLib, "neg_binomial_2_log_glm_lpmf", [ y; x; alpha; beta; sigma ])
          | ( "neg_binomial_2_lpmf"
            , [ y
              ; { pattern =
                    FunApp
          ...
          | ( "neg_binomial_2_log_lpmf"
            ...
            FunApp (StanLib, "neg_binomial_2_log_glm_lpmf", [ y; x; alpha; beta; sigma ])
          | ( "neg_binomial_2_log_lpmf"
            , [ y; { pattern = FunApp (StanLib, "Times__", [ x; beta ]); _ }; sigma ] )
            when Expr.Typed.type_of x = UMatrix ->
            FunApp
              ( StanLib
              , "neg_binomial_2_log_glm_lpmf"
          ...
          | ( "normal_lpdf"
            ...
            when Expr.Typed.type_of x = UMatrix ->
            FunApp (StanLib, "normal_id_glm_lpdf", [ y; x; alpha; beta; sigma ])
          | ( "normal_lpdf"
            , [ y
              ; { pattern =
                    FunApp
          ...
          | ( "normal_lpdf"
            ...
            when Expr.Typed.type_of x = UMatrix ->
            FunApp
              (StanLib, "normal_id_glm_lpdf", [ y; x; Expr.Helpers.zero; beta; sigma ])
          | ( "poisson_lpmf"
            , [ y
              ; { pattern =
                    FunApp
          ...
          | ( "poisson_lpmf"
            ...
            when Expr.Typed.type_of x = UMatrix ->
            FunApp (StanLib, "poisson_log_glm_lpmf", [ y; x; Expr.Helpers.zero; beta ])
          | ( "poisson_log_lpmf"
            , [ y
              ; { pattern =
                    FunApp
          ...
          | ( "poisson_log_lpmf"
            ...
            when Expr.Typed.type_of x = UMatrix ->
            FunApp (StanLib, "poisson_log_glm_lpmf", [ y; x; Expr.Helpers.zero; beta ])
          | "poisson_lpmf", [ y; { pattern = FunApp (StanLib, "exp", [ eta ]); _ } ] ->
          ...
          | "Minus__", [ x; { pattern = FunApp (StanLib, "erf", l); _ } ] when is_int 1 x
            -> FunApp (StanLib, "erfc", l)
          | "Minus__", [ x; { pattern = FunApp (StanLib, "erfc", l); _ } ] when is_int 1 x
            -> FunApp (StanLib, "erf", l)
          | "Minus__", [ { pattern = FunApp (StanLib, "exp", l'); _ }; x ]
            when is_int 1 x && not preserve_stability -> FunApp (StanLib, "expm1", l')
          | "Plus__", [ { pattern = FunApp (StanLib, "Times__", [ x; y ]); _ }; z ]
          | "Plus__", [ z; { pattern = FunApp (StanLib, "Times__", [ x; y ]); _ } ]
            when not preserve_stability -> FunApp (StanLib, "fma", [ x; y; z ])
          | "Minus__", [ x; { pattern = FunApp (StanLib, "gamma_p", l); _ } ]
            when is_int 1 x -> FunApp (StanLib, "gamma_q", l)
          | "Minus__", [ x; { pattern = FunApp (StanLib, "gamma_q", l); _ } ]
            when is_int 1 x -> FunApp (StanLib, "gamma_p", l)
          | ( "Times__"
            , [ { pattern =
                    FunApp
                      ( StanLib
          ...
          | "Times__", [ { pattern = FunApp (StanLib, "log", [ y ]); _ }; x ]
            when not preserve_stability -> FunApp (StanLib, "lmultiply", [ x; y ])
          | ( "Times__"
            , [ { pattern = FunApp (StanLib, "diag_matrix", [ v ]); _ }
              ; { pattern = FunApp (StanLib, "diag_post_multiply", [ a; w ]); _ }
              ] )
            when Expr.Typed.equal v w -> FunApp (StanLib, "quad_form_diag", [ a; v ])
          | ( "Times__"
          ...
          | ( "Times__"
            ...
            when Expr.Typed.equal b c -> FunApp (StanLib, "quad_form", [ a; b ])
          | ( "Times__"
            , [ { pattern =
                    FunApp
                      ( StanLib
                      , "Times__"
                      , [ { pattern = FunApp (StanLib, "transpose", [ b ]); _ }; a ] )
                ; _
                }
              ; c
              ] )
            when Expr.Typed.equal b c -> FunApp (StanLib, "quad_form", [ a; b ])
          | "Times__", [ e1'; { pattern = FunApp (StanLib, "diag_matrix", [ v ]); _ } ] ->
            FunApp (StanLib, "diag_post_multiply", [ e1'; v ])
          | "Times__", [ { pattern = FunApp (StanLib, "diag_matrix", [ v ]); _ }; e2' ] ->
          ...
          | op, [ { pattern = Lit (Real, r); _ } ] ->
            (match op with
            | "PPlus__" | "PMinus__" -> apply_prefix_operator_real op (Float.of_string r)
            | _ -> FunApp (t, op, l))
          | op, [ { pattern = Lit (Int, i1); _ }; { pattern = Lit (Int, i2); _ } ] ->
            ...
            | "Leq__"
            | "Greater__"
            | "Geq__" -> apply_operator_int op (Int.of_string i1) (Int.of_string i2)
            | _ -> FunApp (t, op, l))
          ...
          | op, [ { pattern = Lit (Real, i1); _ }; { pattern = Lit (Int, i2); _ } ] ->
            ...
            | "Plus__" | "Minus__" | "Times__" | "Divide__" ->
              apply_arithmetic_operator_real op (Float.of_string i1) (Float.of_string i2)
            | "Or__"
            | "And__"
            | "Equals__"
            | "NEquals__"
            ...
            | "Greater__"
            | "Geq__" ->
              apply_logical_operator_real op (Float.of_string i1) (Float.of_string i2)
            | _ -> FunApp (t, op, l))
          | _ -> FunApp (t, f, l))
      | TernaryIf (e1, e2, e3) ->
  ...
  Expr.Fixed.(
    ...
    | Pattern.Indexed
      ...
      | inner_singles, All :: inner_tl ->
        ...
        simplify_index_expr
          (Indexed
             ( { pattern = Indexed (obj, inner_singles @ [ single ] @ inner_tl); meta }
             , outer_tl ))
