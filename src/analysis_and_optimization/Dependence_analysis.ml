...
*)
let reaching_defn_lookup (rds : reaching_defn Set.Poly.t) (var : vexpr) : label Set.Poly.t
  =
...
let node_immediate_dependencies
    ...
    ?(blockers : vexpr Set.Poly.t = Set.Poly.empty)
    (label : label)
    : label Set.Poly.t
  =
...
let all_node_dependencies
  ...
  let step_node label m =
    ...
    let updated =
      Set.union (union_map immediate ~f:(fun label -> Map.find_exn m label)) immediate
    in
...
let mir_reaching_definitions (mir : Program.Typed.t) (stmt : Stmt.Located.t)
  ...
  =
  let flowgraph, flowgraph_to_mir = Monotone_framework.forward_flowgraph_of_stmt stmt in
  let (module Flowgraph) = flowgraph in
  let rd_map = reaching_definitions_mfp mir (module Flowgraph) flowgraph_to_mir in
  let to_rd_set set =
    Set.Poly.map set ~f:(fun (s, label_opt) -> VVar s, Option.value label_opt ~default:1)
  in
...
let stmt_uninitialized_variables (exceptions : string Set.Poly.t) (stmt : Stmt.Located.t)
...
let mir_uninitialized_variables (mir : Program.Typed.t)
  ...
  let globals =
    Set.Poly.union (Set.Poly.of_list flag_variables) (Set.Poly.singleton "target")
  in
  let parameters =
    Set.Poly.of_list
      (List.map
  ...
  Set.Poly.union_list
    ...
    ; Set.Poly.union_list
        (List.map mir.functions_block ~f:(fun { fdbody; fdargs; _ } ->
             let arg_vars =
               Set.Poly.of_list (List.map fdargs ~f:(fun (_, arg_name, _) -> arg_name))
             in
