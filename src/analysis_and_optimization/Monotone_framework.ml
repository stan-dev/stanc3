...
let print_mfp
    to_string
    (mfp : (int, 'a entry_exit) Map.Poly.t)
  ...
  in
  let print_stmt s = [%sexp (s : Stmt.Located.Non_recursive.t)] |> Sexp.to_string_hum in
...
let rec free_vars_expr (e : Expr.Typed.t) =
  ...
  | Indexed (e, l) -> Set.Poly.union_list (free_vars_expr e :: List.map ~f:free_vars_idx l)
...
let rec free_vars_stmt (s : (Expr.Typed.t, Stmt.Located.t) Stmt.Fixed.Pattern.t) =
  match s with
  | Assignment ((_, _, []), e) | Return (Some e) | TargetPE e -> free_vars_expr e
  | Assignment ((_, _, l), e) ->
  ...
  | For { lower = e1; upper = e2; body = b; _ } ->
    Set.Poly.union_list [ free_vars_expr e1; free_vars_expr e2; free_vars_stmt b.pattern ]
  | Block l | SList l ->
...
let top_free_vars_stmt
  ...
  =
  match s with
  | Assignment _ | Return _ | TargetPE _ | NRFunApp _ | Decl _ | Break | Continue | Skip
    -> free_vars_stmt (statement_stmt_loc_of_statement_stmt_loc_num flowgraph_to_mir s)
...
let forward_flowgraph_of_stmt ?(flatten_loops = false) ?(blocks_after_body = true) stmt =
  let inv_flowgraph = inverse_flowgraph_of_stmt ~flatten_loops ~blocks_after_body stmt in
...
let dual_powerset_lattice (type v) (module S : INITIALTOTALTYPE with type vals = v) =
  (module struct
    type properties = S.vals Set.Poly.t
...
let dual_powerset_lattice_expressions
    (initial : Expr.Typed.Set.t)
...
let dual_partial_function_lattice
    (type dv cv)
...
let dual_partial_function_lattice_with_bot
    (type dv cv)
    (module Dom : TOTALTYPE with type vals = dv)
    (module Codom : TYPE with type vals = cv)
  =
...
let dual_powerset_lattice_empty_initial (type v) (module T : TOTALTYPE with type vals = v)
  =
...
let reaching_definitions_lattice
    (type v l)
...
let autodiff_level_lattice autodiff_variables =
  powerset_lattice
    (module struct
      ...
      let initial = autodiff_variables
    end)
;;
      ...
      | Some m ->
        ...
        Some
          ...
          | Assignment ((s, t, []), e) ->
            ...
            | _ -> Map.remove m s)
          | Decl { decl_id = s; _ } | Assignment ((s, _, _ :: _), _) -> Map.remove m s
          | TargetPE _
          | NRFunApp (_, _, _)
          | Break | Continue | Return _ | Skip
          | IfElse (_, _, _)
          | While (_, _)
          | For _ | Block _ | SList _ -> m)
    ;;
    ...
    let transfer_function l p =
      ...
      | Some m ->
        ...
        Some
          ...
          | Decl { decl_id = s; _ } | Assignment ((s, _, _ :: _), _) -> kill_var m s
          | Block b ->
            let kills =
...
let assigned_vars_stmt (s : (Expr.Typed.t, 'a) Stmt.Fixed.Pattern.t) =
  ...
  | NRFunApp (_, s, _) when String.suffix s 3 = "_lp" -> Set.Poly.singleton "target"
...
let reaching_definitions_transfer
  ...
  (module struct
    ...
    let transfer_function l p =
      let mir_node = (Map.find_exn flowgraph_to_mir l).pattern in
      let gen =
        Set.Poly.map ~f:(fun x -> x, Some l) (assigned_or_declared_vars_stmt mir_node)
      in
      let kill =
        match mir_node with
        ...
        | IfElse (_, _, _)
        | While (_, _)
        | Block _ | SList _ | Assignment _ -> Set.Poly.empty
      in
...
let live_variables_transfer
  ...
  (module struct
    ...
    let transfer_function l p =
      ...
      let kill =
        match mir_node with
        | Assignment ((x, _, []), _) | Decl { decl_id = x; _ } -> Set.Poly.singleton x
        | TargetPE _
        | NRFunApp (_, _, _)
        | Break | Continue | Return _ | Skip
        | IfElse (_, _, _)
        ...
        | Assignment ((_, _, _ :: _), _) -> Set.Poly.empty
      in
...
let rec used_expressions_stmt_help
    f
...
let used_subexpressions_stmt = used_expressions_stmt_help used_subexpressions_expr
...
let top_used_expressions_stmt_help f (s : (Expr.Typed.t, int) Stmt.Fixed.Pattern.t) =
  match s with
  | Assignment ((_, _, []), e) | TargetPE e | Return (Some e) -> f e
  | Assignment ((_, _, l), e) ->
    Expr.Typed.Set.union
...
let top_used_subexpressions_stmt = top_used_expressions_stmt_help used_subexpressions_expr

(** Calculate the set of expressions at the top level in a statement *)
let top_used_expressions_stmt = top_used_expressions_stmt_help used_expressions_expr
...
let used (flowgraph_to_mir : (int, Stmt.Located.Non_recursive.t) Map.Poly.t) =
  Map.Poly.fold flowgraph_to_mir ~init:Map.Poly.empty ~f:(fun ~key ~data accum ->
...
let earliest
  ...
  =
  Map.fold anticipated_expressions ~init:Map.Poly.empty ~f:(fun ~key ~data accum ->
      Map.set
        accum
        ~key
        ~data:(Set.diff data.exit (Map.find_exn available_expressions key).entry))
;;
...
let latest
    (successors : (int, int Set.Poly.t) Map.Poly.t)
    (earliest : (int, Expr.Typed.Set.t) Map.Poly.t)
...
(** The central definition of a monotone dataflow analysis framework.
    ...
    *)
let monotone_framework
    (type l p)
    (module F : FLOWGRAPH with type labels = l)
  ...
  (module struct
    ...
    let mfp () =
      ...
      while Stack.length workstack <> 0 do
        ...
        let new_analysis_in_l' = T.transfer_function l (Hashtbl.find_exn analysis_in l) in
        if not (L.leq new_analysis_in_l' old_analysis_in_l')
        then (
          let () =
            Hashtbl.set
              analysis_in
...
let rec declared_variables_stmt (s : (Expr.Typed.t, Stmt.Located.t) Stmt.Fixed.Pattern.t) =
  match s with
  | Decl { decl_id = x; _ } -> Set.Poly.singleton x
  | Assignment (_, _)
  | TargetPE _
  | NRFunApp (_, _, _)
  | Break | Continue | Return _ | Skip -> Set.Poly.empty
  | IfElse (_, b1, Some b2) ->
    Set.Poly.union
  ...
  | For { loopvar = s; body = b; _ } -> Set.Poly.add (declared_variables_stmt b.pattern) s
  | Block l | SList l ->
    Set.Poly.union_list (List.map ~f:(fun x -> declared_variables_stmt x.pattern) l)
;;

let propagation_mfp
    (prog : Program.Typed.t)
  ...
  let codomain =
    (module struct
      type vals = Expr.Typed.t
    end : TYPE
      with type vals = Expr.Typed.t)
  in
  let (module Lattice) = dual_partial_function_lattice_with_bot domain codomain in
...
(** Monotone framework instance for live_variables analysis. Expects reverse
    flowgraph. *)
let live_variables_mfp
    (prog : Program.Typed.t)
    (module Rev_Flowgraph : Monotone_framework_sigs.FLOWGRAPH with type labels = int)
    (flowgraph_to_mir : (int, Stmt.Located.Non_recursive.t) Map.Poly.t)
  =
...
let lazy_expressions_mfp
  ...
  let all_expressions =
    used_subexpressions_stmt
      (stmt_loc_of_stmt_loc_num flowgraph_to_mir (Map.Poly.find_exn flowgraph_to_mir 1))
        .pattern
  in
  ...
  let (module Transfer1) = anticipated_expressions_transfer flowgraph_to_mir used_expr in
  let (module Mf1) =
  ...
  let earliest_expr = earliest anticipated_expressions_mfp available_expressions_mfp in
  let (module Transfer3) = postponable_expressions_transfer earliest_expr used_expr in
  let (module Mf3) =
    monotone_framework (module Flowgraph) (module Lattice1) (module Transfer3)
  in
  let postponable_expressions_mfp = Mf3.mfp () in
  let latest_expr =
    latest Flowgraph.successors earliest_expr postponable_expressions_mfp used_expr
  in
  let (module Transfer4) = used_not_latest_expressions_transfer used_expr latest_expr in
  let (module Mf4) =
    monotone_framework (module Rev_Flowgraph) (module Lattice2) (module Transfer4)
  in
