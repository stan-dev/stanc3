...
let optimize_ad_levels (mir : Program.Typed.t) =
  ...
  let update_stmt stmt_pattern variable_set =
    ...
    | Stmt.Fixed.Pattern.Decl {decl_id; decl_type; _}
      when Set.mem variable_set decl_id ->
        Stmt.Fixed.Pattern.Decl
          {decl_adtype= UnsizedType.AutoDiffable; decl_id; decl_type}
    | Decl {decl_id; decl_type; _} when not (Set.mem variable_set decl_id) ->
