...
let transform_program
    (mir : Program.Typed.t)
    (transform : Stmt.Located.t -> Stmt.Located.t)
    : Program.Typed.t
  =
  ...
  let transformed_functions =
    List.map mir.functions_block ~f:(fun fs -> { fs with fdbody = transform fs.fdbody })
  in
  ...
  | _ -> raise (Failure "Something went wrong with program transformation packing!")
;;

(**
   Apply the transformation to each function body and to each program block separately.
*)
let transform_program_blockwise
    (mir : Program.Typed.t)
    (transform :
...
let handle_early_returns opt_var b =
  ...
  let f = function
    | Stmt.Fixed.Pattern.Return opt_ret ->
      ...
      | Some _, None ->
        raise_s
          [%message
            ("Function should return a value but found an empty return statement."
              : string)]
      | None, Some _ ->
        raise_s
          [%message
            ("Expected a void function but found a non-empty return statement." : string)])
  ...
  Stmt.Fixed.Pattern.SList
    [ Stmt.Fixed.
        { pattern =
            Decl { decl_adtype = DataOnly; decl_id = returned; decl_type = Sized SInt }
        ; meta = Location_span.empty
        }
    ; Stmt.Fixed.
        { pattern =
...
let rec inline_function_expression adt fim (Expr.Fixed.{ pattern; _ } as e) =
  ...
  | EAnd (e1, e2) ->
    ...
    let sl2 =
      [ Stmt.Fixed.(
          Pattern.IfElse
            (e1, { pattern = Block (map_no_loc sl2); meta = Location_span.empty }, None))
      ]
    in
  ...
  Stmt.Fixed.
    { pattern =
        ...
        | Assignment ((x, ut, l), e2) ->
          ...
          in
          slist_concat_no_loc (dl2 @ dl1 @ sl2 @ sl1) (Assignment ((x, ut, l), e2))
        | TargetPE e ->
          ...
          slist_concat_no_loc
            ...
            (While
               ...
               , match s' with
                 ...
                 | _ ->
                   { pattern =
                       Block ([ inline_function_statement adt fim s ] @ map_no_loc s')
                   ; meta = Location_span.empty
                   } ))
        | For { loopvar; lower; upper; body } ->
          let d_lower, s_lower, lower = inline_function_expression adt fim lower in
          let d_upper, s_upper, upper = inline_function_expression adt fim upper in
          slist_concat_no_loc
...
let function_inlining (mir : Program.Typed.t) =
  ...
  ; generate_quantities = dataonly_inline_function_statements mir.generate_quantities
  }
;;
...
let unroll_static_loops_statement _ =
  let f stmt =
    ...
    | Stmt.Fixed.Pattern.For { loopvar; lower; upper; body } ->
      ...
      (match contains_top_break_or_continue body, lower.pattern, upper.pattern with
      | false, Lit (Int, low_str), Lit (Int, up_str) ->
        let low = Int.of_string low_str in
        let up = Int.of_string up_str in
        if up - low > unroll_static_limit
        then stmt
        else (
          let range =
            List.map
              ~f:(fun i ->
                Expr.Fixed.
                  ...
                  ; meta =
                      Expr.Typed.Meta.
                        { type_ = UInt; loc = Location_span.empty; adlevel = DataOnly }
                  })
...
let unroll_loop_one_step_statement _ =
  let f stmt =
    match stmt with
    | Stmt.Fixed.Pattern.For { loopvar; lower; upper; body } ->
      if contains_top_break_or_continue body
      then stmt
      else
        IfElse
          ( Expr.Fixed.{ lower with pattern = FunApp (StanLib, "Geq__", [ upper; lower ]) }
          , { pattern =
                (let body_unrolled =
                   subst_args_stmt
                     [ loopvar ]
    ...
    (propagation_transfer :
      ...
      -> (module Monotone_framework_sigs.TRANSFER_FUNCTION
            with type labels = int
             and type properties = (string, Middle.Expr.Typed.t) Map.Poly.t option))
    (mir : Program.Typed.t)
  =
  let transform s =
    let flowgraph, flowgraph_to_mir = Monotone_framework.forward_flowgraph_of_stmt s in
    let (module Flowgraph) = flowgraph in
    let values =
      Monotone_framework.propagation_mfp
        mir
        (module Flowgraph)
        flowgraph_to_mir
        propagation_transfer
...
let constant_propagation = propagation Monotone_framework.constant_propagation_transfer
...
and idx_any pred (i : Expr.Typed.t Index.t) = Index.fold (accum_any pred) false i
...
let dead_code_elimination (mir : Program.Typed.t) =
  ...
  let transform s =
    ...
    let live_variables =
      Monotone_framework.live_variables_mfp mir (module Rev_Flowgraph) flowgraph_to_mir
    in
    let dead_code_elim_stmt_base i stmt =
      ...
      | Stmt.Fixed.Pattern.Assignment ((x, _, []), rhs) ->
        if Set.Poly.mem live_variables_s x || cannot_remove_expr rhs then stmt else Skip
      | Assignment ((x, _, is), rhs) ->
        if Set.Poly.mem live_variables_s x
      ...
      | Decl _ | TargetPE _ | NRFunApp (_, _, _) | Break | Continue | Return _ | Skip ->
        stmt
      | IfElse (e, b1, b2) ->
        if (* TODO: check if e has side effects, like print, reject, then don't optimize? *)
           (not (cannot_remove_expr e))
           && b1.Stmt.Fixed.pattern = Skip
           && (Option.map ~f:(fun Stmt.Fixed.{ pattern; _ } -> pattern) b2 = Some Skip
              || Option.map ~f:(fun Stmt.Fixed.{ pattern; _ } -> pattern) b2 = None)
        then Skip
        else (
          match e.pattern with
          ...
          | _ -> IfElse (e, b1, b2))
      | While (e, b) ->
        if (not (cannot_remove_expr e)) && b.pattern = Break
        then Skip
        else (
          match e.pattern with
          ...
          | _ -> While (e, b))
      | For { loopvar; lower; upper; body } ->
        if (not (cannot_remove_expr lower))
...
let lazy_code_motion (mir : Program.Typed.t) =
      ...
      | IfElse (e, b1, Some b2) ->
        Stmt.Fixed.(
          Pattern.IfElse
            ...
            , Some
                { pattern = Block [ b2; { pattern = Skip; meta = Location_span.empty } ]
                ; meta = Location_span.empty
                } ))
      | IfElse (e, b, None) ->
        IfElse
  ...
  let transform s =
    ...
    let latest_expr, used_not_latest_expressions_mfp =
      Monotone_framework.lazy_expressions_mfp fwd_flowgraph rev_flowgraph flowgraph_to_mir
    in
    let expression_map =
      ...
      Set.fold
        ...
        ~init:Expr.Typed.Map.empty
        ~f:collect_expressions
    in
    ...
    let lazy_code_motion_base i stmt =
      ...
      let to_assign_in_s =
        ...
        |> Set.to_list
        |> List.sort ~compare:(fun e e' -> compare_int (expr_depth e) (expr_depth e'))
      in
      ...
      let assignments_to_add_to_s =
        List.map
          ~f:(fun e ->
            Stmt.Fixed.
              { pattern = Assignment ((Map.find_exn expression_map e, e.meta.type_, []), e)
              ; meta = Location_span.empty
              })
          to_assign_in_s
      in
      let expr_subst_stmt_except_initial_assign m =
        let f stmt =
          ...
          | Stmt.Fixed.Pattern.Assignment ((x, _, []), e')
            when Map.mem m e'
                 && Expr.Typed.equal { e' with pattern = Var x } (Map.find_exn m e') ->
      ...
      let expr_map =
        Map.filter_keys
          ...
          (Map.mapi expression_map ~f:(fun ~key ~data -> { key with pattern = Var data }))
      in
      let f = expr_subst_stmt_except_initial_assign expr_map in
      if List.length assignments_to_add_to_s = 0
      then (f Stmt.Fixed.{ pattern = stmt; meta = Location_span.empty }).pattern
      else
        SList
          (List.map
             ~f
  ...
  let cleanup =
    let cleanup_base (stmt : (Expr.Typed.t, Stmt.Located.t) Stmt.Fixed.Pattern.t)
      ...
      | IfElse
          ...
          , Some { pattern = Skip; _ } ) -> IfElse (e, b, None)
    ...
    let optimize_ad_levels_stmt_base i stmt =
      ...
      match
        Stmt.Fixed.Pattern.map
          ...
          stmt
      with
      | Decl { decl_id; decl_type; _ } when Set.mem autodiffable_variables decl_id ->
...
let optimization_suite ?(settings = all_optimizations) mir =
  let maybe_optimizations =
    ...
    ; constant_propagation, settings.constant_propagation (* Book: Loop simplification *)
