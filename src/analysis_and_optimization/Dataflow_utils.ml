...
let build_cf_graphs ?(flatten_loops = false) ?(blocks_after_body = true)
  ...
  let rec build_cf_graph_rec (cf_parent : label option)
    ...
    let substmt_state, predecessors =
      match stmt with
      | For _ | While _ ->
          (* Loop statements are preceded by: 1. The statements that come before
             the loop 2. The natural exit points of the loop body 3. Continue
             statements in the loop body This comment mangling brought to you by
             the autoformatter *)
          ...
          in
          (* Loop exits are: 1. The loop node itself, since the last action of a
             typical loop execution is to check if there are any iterations
             remaining 2. Break statements in the loop body, since broken loops
             don't execute the loop statement *)
          let loop_exits =
