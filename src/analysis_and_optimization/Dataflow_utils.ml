...
let branching_fold_statement stmt ~join ~init ~f =
  fst (branching_traverse_statement stmt ~join ~init ~f:(fun s a -> f s a, ()))
;;
...
let build_statement_map extract metadata stmt =
  let rec build_statement_map_rec next_label map stmt =
    ...
    in
    ( ( next_label''
      , union_maps_left map (Map.Poly.singleton this_label (built, metadata stmt)) )
    , this_label )
  in
...
*)
let build_cf_graphs ?(flatten_loops = false) ?(blocks_after_body = true) statement_map =
  let rec build_cf_graph_rec
      (cf_parent : label option)
      ((in_state, in_map) : cf_state * (label, cf_edges) Map.Poly.t)
      (label : label)
      : cf_state * (label, cf_edges) Map.Poly.t
    =
    ...
    let substmt_state, predecessors =
      ...
      | For _ | While _ ->
        ...
        *)
        let loop_exits =
          if flatten_loops
          then substmt_state_unlooped.exits
          else
            Set.Poly.union_list
              [ (*1*)
                Set.Poly.singleton label
              ; (*2*)
    ...
    , Map.Poly.add_exn substmt_map ~key:label ~data:{ parents = cf_parents; predecessors }
    )
  in
  let state, edges =
    build_cf_graph_rec
