            ...
            | StanLib (f, suffix, mem_type) ->
                ...
                try_partially_evaluate_stanlib
                  ...
                  | "columns_dot_product", [x; y] when Expr.Typed.equal x y ->
                      FunApp
                        (StanLib ("columns_dot_self", suffix, mem_type), [x])
                  ...
                  | ( "log"
                    , [ { pattern=
                            FunApp
                              (StanLib ("falling_factorial", FnPlain, mem), l)
                        ; _ } ] ) ->
                      FunApp
                        ( StanLib
                  ...
                  | ( "log"
                    , [ { pattern=
                            FunApp
                              (StanLib ("rising_factorial", FnPlain, mem), l)
                        ; _ } ] ) ->
                      FunApp
                        ( StanLib
                  ...
                  | ( "log"
                    , [ { pattern=
                            FunApp (StanLib ("inv_logit", FnPlain, mem), l)
                        ; _ } ] ) ->
                      FunApp
                        (StanLib ("log_inv_logit", suffix, lub_mem_pat [mem]), l)
                  | ( "log"
                    , [ { pattern= FunApp (StanLib ("softmax", FnPlain, mem), l)
                        ; _ } ] ) ->
                      FunApp
                  ...
                  | ( "neg_binomial_2_lpmf"
                    ...
                    when Expr.Typed.type_of x = UMatrix ->
                      ...
                      FunApp
                        ( StanLib
                            ("neg_binomial_2_log_glm_lpmf", suffix, lub_mem)
                  ...
                  | ( "neg_binomial_2_lpmf"
                    , [ y
                      ; { pattern= FunApp (StanLib ("exp", FnPlain, mem), [eta])
                        ; _ }; phi ] ) ->
                      let lub_mem = lub_mem_pat [mem] in
                      FunApp
                  ...
                  | ( "poisson_rng"
                    , [ { pattern= FunApp (StanLib ("exp", FnPlain, mem), [eta])
                      ...
                      let lub_mem = lub_mem_pat [mem] in
                      FunApp
                        (StanLib ("poisson_log_rng", suffix, lub_mem), [eta])
                  ...
                  | ( "Minus__"
                    , [ x
                      ; {pattern= FunApp (StanLib ("erf", FnPlain, mem), l); _}
                      ] )
                    when is_int 1 x ->
                      let lub_mem = lub_mem_pat [mem] in
                  ...
                  | ( "Minus__"
                    , [ x
                      ; { pattern= FunApp (StanLib ("gamma_p", FnPlain, mem), l)
                        ; _ } ] )
                    when is_int 1 x ->
                      let lub_mem = lub_mem_pat [mem] in
                  ...
                  | ( "Minus__"
                    , [ x
                      ; { pattern= FunApp (StanLib ("gamma_q", FnPlain, mem), l)
                        ; _ } ] )
                    when is_int 1 x ->
                      let lub_mem = lub_mem_pat [mem] in
                  ...
                  | ( "Times__"
                    , [ { pattern=
                      ...
                      let lub_mem = lub_mem_pat [mem] in
                      FunApp
                        ( StanLib ("matrix_exp_multiply", suffix, lub_mem)
                        , [a; b] )
                  | ( "Times__"
                    , [ x
                  ...
                  | ( "Times__"
                    ...
                    when Expr.Typed.equal v w ->
                      let lub_mem = lub_mem_pat [mem1; mem2] in
                      FunApp
                        (StanLib ("quad_form_diag", suffix, lub_mem), [a; v])
                  | ( "Times__"
                    , [ { pattern=
                            FunApp
                  ...
                  | ( "Times__"
                    , [ { pattern=
                      ...
                      let lub_mem = lub_mem_pat [mem] in
                      FunApp
                        ( StanLib ("diag_pre_multiply", suffix, lub_mem)
                        , [v; e2'] )
