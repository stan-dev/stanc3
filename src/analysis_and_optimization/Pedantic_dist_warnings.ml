...
let cholesky_covariance = { name = "Cholesky factor of covariance"; constr = CholeskyCov }
...
let transform_mismatch_constraint
    (constr : var_constraint)
    (trans : Expr.Typed.t Program.transformation)
    : bool
  =
  match constr with
...
let constr_mismatch_message
  ...
  | Arg (argn, arg_name) ->
    Printf.sprintf
      "A %s distribution is given parameter %s as %s (argument %d), but %s was not \
       constrained to be %s."
      dist_name
      param_name
      arg_name
  ...
  | Variate ->
    ...
    Printf.sprintf
      "Parameter %s is given a %s distribution, which has %s support, but %s was not \
       constrained to be %s."
      param_name
      dist_name
      constr_name
...
let constr_literal_mismatch_message
    (dist_name : string)
    (num_str : string)
    (arg : arg_info)
    (constr_name : string)
  ...
  | Arg (argn, arg_name) ->
    Printf.sprintf
      "A %s distribution is given value %s as %s (argument %d), but %s is not %s."
      dist_name
      num_str
      arg_name
      argn
  ...
  | Variate ->
    (* Possibly: Either change the distribution or change the constraints. *)
    Printf.sprintf
      "Value %s is given a %s distribution, which has %s support, but %s is not %s."
      num_str
      dist_name
      constr_name
      num_str
...
let constr_mismatch_warning
    (constr : var_constraint_named)
    (arg : arg_info)
    ({ args; name; loc } : dist_info)
  ...
  let v =
    ...
    | None ->
      let arg_fail_msg =
        Printf.sprintf
          "Distribution %s at %s expects more arguments."
          name
  ...
  | Param (pname, trans), meta ->
    if transform_mismatch_constraint constr.constr trans
    then Some (meta.loc, constr_mismatch_message name pname arg constr.name)
    else None
  | Number (num, num_str), meta ->
    if value_mismatch_constraint constr.constr num
    then Some (meta.loc, constr_literal_mismatch_message name num_str arg constr.name)
    else None
  | _ -> None
;;
...
let uniform_dist_message (pname : string) : string =
  Printf.sprintf
    "Parameter %s is given a uniform distribution. The uniform distribution is not \
     recommended, for two reasons: (a) Except when there are logical or physical \
     constraints, it is very unusual for you to be sure that a parameter will fall \
     inside a specified range, and (b) The infinite gradient induced by a uniform \
     density can cause difficulties for Stan's sampling algorithm. As a consequence, we \
     recommend soft constraints rather than hard constraints; for example, instead of \
     giving an elasticity parameter a uniform(0,1) distribution, try normal(0.5,0.5)."
    pname
;;
...
let lkj_corr_message : string =
  "It is suggested to reparameterize your model to replace lkj_corr with \
   lkj_corr_cholesky, the Cholesky factor variant. lkj_corr tends to run slower, consume \
   more memory, and has higher risk of numerical errors."
;;

(* Warn about all non-Cholesky lkj_corr distributions *)
let lkj_corr_dist_warning (dist_info : dist_info) : (Location_span.t * string) option =
  Some (dist_info.loc, lkj_corr_message)
;;

let gamma_arg_dist_message : string =
  "There is a gamma or inverse-gamma distribution with parameters that are equal to each \
   other and set to values less than 1. This is mathematically acceptable and can make \
   sense in some problems, but typically we see this model used as an attempt to assign \
   a noninformative prior distribution. In fact, priors such as inverse-gamma(.001,.001) \
   can be very strong, as explained by Gelman (2006). Instead we recommend something \
   like a normal(0,1) or student_t(4,0,1), with parameter constrained to be positive."
;;
...
let distribution_warning (dist_info : dist_info) : (Location_span.t * string) List.t =
  ...
  *)
  let warning_fns =
    ...
    | "normal_id_glm" -> [ constr_mismatch_warning positive_range (Arg (4, scale_name)) ]
    | "exp_mod_normal" ->
      [ constr_mismatch_warning positive_range (Arg (2, scale_name))
      ; constr_mismatch_warning positive_range (Arg (3, shape_name))
      ]
    | "skew_normal" -> [ constr_mismatch_warning positive_range (Arg (2, scale_name)) ]
    | "student_t" ->
    ...
    | "double_exponential" ->
      [ constr_mismatch_warning positive_range (Arg (2, scale_name)) ]
    | "logistic" -> [ constr_mismatch_warning positive_range (Arg (2, scale_name)) ]
    ...
    | "von_mises" -> [ constr_mismatch_warning positive_range (Arg (2, scale_name)) ]
