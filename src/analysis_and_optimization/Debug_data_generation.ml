...
let dotproduct xs ys =
  List.fold_left2 xs ys ~init:0. ~f:(fun accum x y -> accum +. (x *. y))
    ...
    | Ordered | PositiveOrdered | Simplex | UnitVector | SumToZero
     ...
     |StochasticColumn | TupleTransformation _ ->
        Common.ICE.internal_errorf "Unknown transformation for real: %t"
          [dprint_transform t] [@coverage off] in
  low +. Random.float (up -. low)

let repeat n e = List.init ~len:n ~f:(Fun.const e)
let repeat_th n f = List.init ~len:n ~f:(fun _ -> f ())
...
let gen_ul_bounded m n gen e1 e2 =
  let create_bounds l u =
    List.map2 ~f:(fun x y -> gen (Transformation.LowerUpper (x, y))) l u in
...
let sum_to_zero_floats n =
  ...
  let sum = List.fold_left l ~init:0. ~f:( +. ) in
...
let gen_vector m n t =
  let gen_ordered n =
    let l = random_floats n in
    List.fold_left_map l ~init:0. ~f:(fun accum elt ->
  ...
  | UnitVector ->
      ...
      let sum =
        Float.sqrt
          (List.fold_left l ~init:0. ~f:(fun accum elt -> accum +. (elt ** 2.)))
      in
...
let gen_diag_mat l =
  ...
  List.map
    ...
    ~f:(fun k ->
      repeat (min (k - 1) n) 0.
      @ (if k <= n then [List.nth l (k - 1)] else [])
      @ repeat (n - k) 0.)

let fill_lower_triangular m =
  let fill_row i l =
    let tl = List.drop i l in
...
let gen_corr_cholesky_unwrapped n =
  ...
  let row_normalizer l =
    let row_norm =
      Float.sqrt
        (List.fold_left ~init:0. ~f:(fun accum x -> accum +. (x *. x)) l) in
...
let gen_sum_to_zero_matrix m n =
  ...
  let col_sums =
    List.fold_left rows ~init:(repeat n 0.) ~f:(fun accum row ->
        List.map2 accum row ~f:( +. )) in
...
let gen_complex_unwrapped () =
  ( gen_num_real String.Map.empty Transformation.Identity
  , gen_num_real String.Map.empty Transformation.Identity )
...
let json_to_mir (decls : (Expr.Typed.t SizedType.t * 'a * string) list)
  ...
  let rec create_expr (type_ : UnsizedType.t) (json : Basic.t) =
    ...
    | `Assoc l, UTuple ts ->
        ...
        |> List.map2 ~f:(fun typ_ (_, json) -> create_expr typ_ json) ts
        |> Option.all
        |> Option.map ~f:(fun l -> Expr.Helpers.tuple_expr l)
    | _ -> None in
  let map =
    match json with `Assoc l -> String.Map.of_list l | _ -> String.Map.empty
  in
...
let generate_json_entries (name, expr) : string * t =
  let rec expr_to_json e : t =
    ...
    | Lit (Real, s) when String.ends_with s ~suffix:"." -> `Floatlit (s ^ "0")
    | Lit (Int, s) -> `Intlit s
...
let gen_values_json_exn ?(new_only = false) ?(context = String.Map.empty) decls
    =
  ...
  let json_entries =
    if new_only then
      List.filter ~f:(fun (name, _) -> String.Map.mem name context) json_entries
    else json_entries in
  let json = `Assoc json_entries in
  pretty_to_string json

let gen_values_json ?(new_only = false) ?(context = String.Map.empty) decls =
  try Ok (gen_values_json_exn ~new_only ~context decls)
  with Partial_evaluator.Rejected (loc, msg) ->
    Error
      (Frontend.Errors.DebugDataError
         (loc, msg, not (String.Map.is_empty context)))
