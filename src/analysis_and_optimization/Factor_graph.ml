...
let extract_factors_statement stmt =
  ...
  | NRFunApp (_, f, _) when Internal_fun.of_string_opt f = Some FnReject -> [ Reject ]
  | NRFunApp (_, s, args) when String.suffix s 3 = "_lp" -> [ LPFunction (s, args) ]
  | Assignment (_, _)
  | NRFunApp (_, _, _)
...
let build_adjacency_maps (factors : (label * factor * vexpr Set.Poly.t) List.t)
  ...
  let factor_map =
    List.fold
      ~f:merge_set_maps
      ~init:Map.Poly.empty
      (List.map ~f:(fun (l, fac, vars) -> Map.Poly.singleton (fac, l) vars) factors)
  in
  let var_map =
    List.fold
      ~f:merge_set_maps
...
let fg_remove_var (var : vexpr) (fg : factor_graph) : factor_graph =
  let factor_map = Map.Poly.map fg.factor_map ~f:(fun vars -> Set.Poly.remove vars var) in
...
let remove_touching vars fg =
  ...
  let without_facs = Set.fold ~f:(fun g f -> fg_remove_fac f g) ~init:without_vars facs in
  without_facs
;;
...
let prog_factor_graph ?(exclude_data_facs : bool = false) prog : factor_graph =
  ...
  if exclude_data_facs
  then remove_touching (Set.Poly.map ~f:(fun v -> VVar v) data_vars) fg
  else fg
;;

(* BFS on 'fg' with initial frontier 'starts' and terminating at any
   element of 'goals' *)
let fg_reaches (starts : vexpr Set.Poly.t) (goals : vexpr Set.Poly.t) (fg : factor_graph)
    : bool
  =
  let vneighbors v =
  ...
  let rec step (frontier : vexpr List.t) (visited : vexpr Set.Poly.t) =
    match frontier with
    | next :: frontier' ->
      if Set.mem visited next
      then step frontier' visited
      else (
        let visited' = Set.Poly.add visited next in
        let expansion = vneighbors next in
        if not (Set.Poly.is_empty (Set.Poly.inter expansion goals))
        then true
        else step (List.append frontier' (Set.Poly.to_list expansion)) visited')
    | [] -> false
  in
  step (Set.Poly.to_list starts) Set.Poly.empty
;;

let fg_factor_reaches
    (start : factor * label)
    (goals : vexpr Set.Poly.t)
    (fg : factor_graph)
    : bool
  =
...
let fg_factor_is_prior
    (var : vexpr)
    (fac : factor * label)
    (data : vexpr Set.Poly.t)
    (fg : factor_graph)
    : bool
  =
  (* build G'=G\V *)
...
let fg_var_priors (var : vexpr) (data : vexpr Set.Poly.t) (fg : factor_graph)
  ...
  | Some factors ->
    Some (Set.Poly.filter factors ~f:(fun fac -> fg_factor_is_prior var fac data fg))
  | None -> None
;;
...
let string_of_vexpr (vexpr : vexpr) : string =
  match vexpr with
  | VVar s -> s
;;
