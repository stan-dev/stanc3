...
let fold_stmts ~take_expr ~take_stmt ~(init : 'c) (stmts : Stmt.Located.t List.t) : 'c =
...
let trans_bounds_values (trans : Expr.Typed.t transformation) : bound_values =
  ...
  | LowerUpper (lower, upper) -> { lower = bound_value lower; upper = bound_value upper }
...
let rec var_declarations Stmt.Fixed.{ pattern; _ } : string Set.Poly.t =
  ...
  | Block slist | SList slist -> Set.Poly.union_list (List.map ~f:var_declarations slist)
  | _ -> Set.Poly.empty
;;
...
let map_rec_stmt_loc_num flowgraph_to_mir f s =
  let rec map_rec_stmt_loc_num' (cur_node : int) (stmt : Stmt.Located.Non_recursive.t) =
    ...
    Stmt.Fixed.
      { pattern = f cur_node (Pattern.map Fn.id recurse stmt.pattern); meta = stmt.meta }
  in
...
let map_rec_state_stmt_loc_num
    ...
    (state : 's)
    (s : Stmt.Located.Non_recursive.t)
    : Stmt.Located.t * 's
  =
  let cur_state = ref state in
  let g i stmt =
...
and index_var_set ix =
  ...
  | Between (expr1, expr2) -> Set.Poly.union (expr_var_set expr1) (expr_var_set expr2)
...
let stmt_rhs stmt =
  ...
  | Assignment (_, rhs)
  | TargetPE rhs
  | Return (Some rhs) -> Set.Poly.singleton rhs
  | Return None | Break | Continue | Skip | Decl _ | Block _ | SList _ -> Set.Poly.empty
;;
...
let rec expr_depth Expr.Fixed.{ pattern; _ } =
  ...
  | TernaryIf (e1, e2, e3) ->
    ...
    + Option.value
        ~default:0
        (List.max_elt ~compare:compare_int (List.map ~f:expr_depth [ e1; e2; e3 ]))
  | Indexed (e, l) ->
    1
    + max
        (expr_depth e)
...
let ad_level_sup l =
  if List.exists l ~f:(fun x -> Expr.Typed.adlevel_of x = AutoDiffable)
  then UnsizedType.AutoDiffable
  else DataOnly
;;

let rec update_expr_ad_levels autodiffable_variables (Expr.Fixed.{ pattern; _ } as e) =
  match pattern with
  | Var x ->
    if Set.Poly.mem autodiffable_variables x
    then Expr.Typed.{ e with meta = Meta.{ e.meta with adlevel = AutoDiffable } }
  ...
  | EAnd (e1, e2) ->
    let e1 = update_expr_ad_levels autodiffable_variables e1 in
    let e2 = update_expr_ad_levels autodiffable_variables e2 in
    { pattern = EAnd (e1, e2); meta = { e.meta with adlevel = ad_level_sup [ e1; e2 ] } }
  | EOr (e1, e2) ->
    let e1 = update_expr_ad_levels autodiffable_variables e1 in
    let e2 = update_expr_ad_levels autodiffable_variables e2 in
  ...
  | Indexed (ixed, i_list) ->
    ...
    let i_list = List.map ~f:(update_idx_ad_levels autodiffable_variables) i_list in
...
let cleanup_empty_stmts stmts =
  ...
  let flatten_block s =
    match s.pattern with
    | SList ls | Block ls -> if List.for_all ~f:(Fn.non is_decl) ls then ls else [ s ]
    | _ -> [ s ]
  in
  let ellide_skip s =
    match s.pattern with
