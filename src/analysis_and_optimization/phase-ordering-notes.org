...
** DONE vectorize_loops < one_step_loop_unrolling, static_loop_unrolling
   vectorization consumes whole loops, so it must run before anything peels or
   unrolls them
** DONE vectorize_loops < partial_evaluation
   so the vectorized density calls it emits can be further fused, e.g. into
   the GLM primitives
