...
let rec pp_expr ppf { Expr.Fixed.pattern; _ } =
  ...
  | FunApp (StanLib, f, args) when Operator.of_string_opt f |> Option.is_some ->
    (match Operator.of_string_opt f |> Option.value_exn |> pystring_of_operator, args with
    | op, [ lhs; rhs ] -> pf ppf "%a %s %a" pp_paren lhs op pp_paren rhs
    | op, [ unary ] -> pf ppf "%s%a" op pp_paren unary
    | op, args -> raise_s [%message "Need to implement" op (args : Expr.Typed.t list)])
...
let rec pp_stmt ppf s =
  ...
  | Return rhs -> pf ppf "return %a" (option ~none:(const string "None") pp_expr) rhs
...
let pp_init ppf p =
  let pp_save_data ppf (name, st) = pf ppf "self.%s = %a" name (pp_cast "") (name, st) in
...
let get_params p =
  List.filter
    ~f:(function
      | _, { Program.out_block = Parameters; _ } -> true
      | _ -> false)
...
let pp_bijector ppf trans =
  ...
  let components =
    ...
    | Upper ub -> [ "Exp", []; "Scale", [ Expr.Helpers.float (-1.) ]; "Shift", [ ub ] ]
    ...
    | _ -> raise_s [%message "Unsupported " (trans : Expr.Typed.t Program.transformation)]
  in
...
let pp_prog ppf (p : Program.Typed.t) =
  pf
    ppf
    "@[<v>%s@,%a@,class %s(tfd__.Distribution):@,@[<v 2>%a@]@]"
    imports
    (list ~sep:cut pp_fundef)
    p.functions_block
    p.prog_name
    pp_methods
    p;
