...
let capitalize_fnames =
  String.Set.of_list
    ["normal"; "cauchy"; "gumbel"; "exponential"; "gamma"; "beta"; "poisson"; "wishart"]
...
let map_functions fname args =
  ...
  | "binomial" -> (
      match args with
      | [y; n; p] -> ( "Binomial", [ y; n; {Fixed.pattern= Var "None"; meta= Typed.Meta.empty}; p ] )
      | _ -> raise_s [%message " Binomial argument should contain exactly three elements."] )
  | "bernoulli" -> (
      match args with
      | [y; p] -> ( "Bernoulli", [y; {Fixed.pattern= Var "None"; meta= Typed.Meta.empty}; p])
      | _ -> raise_s [%message " Binomial argument should contain exactly two elements."] )
  | "categorical" -> (
      match args with
      | [y; p] -> ( "Categorical", [y; {Fixed.pattern= Var "None"; meta= Typed.Meta.empty}; p])
      | _ -> raise_s [%message " Categorical argument should contain exactly two elements."] )
  | "poisson_log" -> (
      match args with
      | [y; log_lambda] -> ( "Poisson", [y; {Fixed.pattern= Var "None"; meta= Typed.Meta.empty}; log_lambda])
      | _ -> raise_s [%message " Poisson argument should contain exactly two elements."] )
  | "pareto" ->  (
      match args with
      | [y; y_min; alpha] -> ( "Categorical", [y; alpha; y_min])
      | _ -> raise_s [%message " Pareto argument should contain exactly three elements."] )
