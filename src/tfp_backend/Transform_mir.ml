...
let translate_funapps_and_kwrds e =
  ...
  let f ({pattern; _} as expr) =
    match pattern with
    | FunApp (StanLib (fname, suffix, mem_type), args) ->
        let prefix =
        ...
        { expr with
          pattern= FunApp (StanLib (prefix ^ fname, suffix, mem_type), args) }
...
let%expect_test "nested dist prefixes translated" =
  ...
  let f =
    FunApp
      ( Fun_kind.StanLib ("normal_lpdf", FnLpdf false, Common.Helpers.AoS)
      , [ FunApp
            ( Fun_kind.StanLib ("normal_lpdf", FnLpdf false, Common.Helpers.SoA)
            , [] )
          |> e ] )
  ...
  [%expect
    ...
    ((pattern
      (FunApp (StanLib tfd__.Normal (FnLpdf false) AoS)
       (((pattern (FunApp (StanLib tfd__.Normal (FnLpdf false) SoA) ()))
         (meta
          ((type_ UInt)
           (loc
            ((begin_loc
