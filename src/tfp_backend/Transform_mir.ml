...
let append_kwrds_suffix x =
  ...
  Fmt.epr "Identifier %s is a reserved word in python, renamed to %s@," x x_with_suffix;
  x_with_suffix
;;
...
let add_suffix_to_kwrds s = if Set.mem python_kwrds s then append_kwrds_suffix s else s
...
let map_functions fname args =
  ...
  | (("neg_binomial_2" | "neg_binomial_2_log") as l), _ ->
    raise_s [%message l " is not supported, consider using neg_binomial instead."]
  | f, _ when Operator.of_string_opt f |> Option.is_some -> fname, args
  | _ ->
    if Set.mem capitalize_fnames fname
    then String.capitalize fname, args
...
let translate_funapps_and_kwrds e =
  ...
  let f ({ pattern; _ } as expr) =
    ...
    | FunApp (UserDefined, fname, args) ->
      { expr with pattern = FunApp (UserDefined, add_suffix_to_kwrds fname, args) }
  ...
  let pattern =
    ...
    | Assignment (_, { Expr.Fixed.pattern = FunApp (CompilerInternal, f, _); _ })
      when Internal_fun.to_string FnConstrain = f
           || Internal_fun.to_string FnUnconstrain = f -> Stmt.Fixed.Pattern.Skip
    | Decl _ -> Stmt.Fixed.Pattern.Skip
    | NRFunApp (CompilerInternal, name, _) when Internal_fun.to_string FnCheck = name ->
...
let rec change_kwrds_stmts s =
  ...
  let pattern =
    ...
    | Assignment ((s, t, e1), e2) -> Assignment ((add_suffix_to_kwrds s, t, e1), e2)
