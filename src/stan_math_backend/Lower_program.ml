...
let math_fwd_include = Preprocessor (Include "stan/math/fwd.hpp")
...
let lower_program ?printed_filename (p : Program.Typed.t) : Cpp.program =
  ...
  let includes =
    if needs_mix_header then
      [ !//"Including the fwd and mix header for embedded laplace usage"
        (* TODO should this be a #define and subsumed into the model header? A second model header? *)
      ; math_fwd_include; model_header_include; math_mix_include ]
