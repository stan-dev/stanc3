...
let rec pp_statement (ppf : Format.formatter)
  ...
  let pp_statement_body ppf stmt =
    match stmt with
    ...
    | Assignment ((assignee, idcs), rhs) ->
        let rec maybe_deep_copy vident = function
          ...
          | {expr; emeta} ->
              {expr= map_expr (maybe_deep_copy vident) expr; emeta}
        in
        let rhs =
          match rhs.expr with
    ...
    | NRFunApp
        (CompilerInternal, fname, {expr= Lit (Str, check_name); _} :: args)
      when fname = string_of_internal_fn FnCheck ->
        let args = {expr= Var "function__"; emeta= internal_meta} :: args in
        pp_statement ppf
          { stmt= NRFunApp (CompilerInternal, "check_" ^ check_name, args)
          ; smeta }
