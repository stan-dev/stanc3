...
let rec pp_dims ppf st =
  match st with
  | SizedType.SInt -> pf ppf ""
  | SReal -> pf ppf ""
  | SVector d1 | SRowVector d1 -> pf ppf ", %a" pp_expr d1
  | SMatrix (d1, d2) -> pf ppf ", %a, %a" pp_expr d1 pp_expr d2
  | SArray (st, d) -> pf ppf ", %a %a" pp_expr d pp_dims st
