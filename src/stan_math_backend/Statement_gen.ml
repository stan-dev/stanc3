...
let pp_set_size ppf (decl_id, st, adtype) =
  ...
  if contains_eigen (SizedType.to_unsized st)
  then pf ppf "@[<hov 2>stan::math::fill(%s, %s);@]@," decl_id real_nan
;;
...
let pp_decl ppf (vident, ut, adtype) =
  let pp_type =
    if Transform_Mir.is_opencl_var vident
    then
      fun ppf _ ->
      match ut with
...
let rec pp_statement (ppf : Format.formatter) (Stmt.Fixed.{ pattern; meta } as stmt) =
  ...
  | (Assignment ((assignee, UInt, idcs), rhs) | Assignment ((assignee, UReal, idcs), rhs))
    when List.for_all ~f:is_single_index idcs ->
    pf ppf "@[<hov 4>%a = %a;@]" pp_indexed_simple (assignee, idcs) pp_expr rhs
  | Assignment ((assignee, _, idcs), rhs) ->
    ...
    let rec maybe_deep_copy e =
      let recurse (e : 'a Expr.Fixed.t) =
        { e with Expr.Fixed.pattern = Expr.Fixed.Pattern.map maybe_deep_copy e.pattern }
      in
      match e.pattern with
      | _ when UnsizedType.is_scalar_type (Expr.Typed.type_of e) -> e
      | FunApp (CompilerInternal, _, _) -> e
      | (Indexed ({ Expr.Fixed.pattern = Var v; _ }, _) | Var v) when v = assignee ->
        { e with
          Expr.Fixed.pattern = FunApp (CompilerInternal, "stan::model::deep_copy", [ e ])
        }
      | _ -> recurse e
    in
    let rhs =
      ...
      | FunApp (CompilerInternal, f, _)
        when f = Internal_fun.to_string FnConstrain
             || f = Internal_fun.to_string FnUnconstrain -> rhs
      | _ -> maybe_deep_copy rhs
    in
    pf
      ppf
      "@[<hov 2>assign(@,%s,@ %a,@ %a,@ %S@]);"
      assignee
      pp_indexes
      idcs
      pp_expr
  ...
  | NRFunApp (CompilerInternal, fname, args) when fname = Internal_fun.to_string FnPrint
    ->
  ...
  | NRFunApp (CompilerInternal, fname, args) when fname = Internal_fun.to_string FnReject
    ->
  ...
  | NRFunApp (CompilerInternal, fname, { pattern = Lit (Str, check_name); _ } :: args)
    when fname = Internal_fun.to_string FnCheck ->
    let args =
      { Expr.Fixed.pattern = Var "function__"; meta = Expr.Typed.Meta.empty } :: args
    in
    pp_statement
      ppf
  ...
  | NRFunApp (CompilerInternal, fname, args) ->
    ...
    pf ppf "%s(@[<hov>%a@]);" fname (list ~sep:comma pp_expr) (extra_args @ args)
  | NRFunApp (StanLib, fname, args) ->
    pf ppf "%s(@[<hov>%a@]);" fname (list ~sep:comma pp_expr) args
  | NRFunApp (UserDefined, fname, args) -> pf ppf "%a;" pp_user_defined_fun (fname, args)
  ...
  | For
      { body =
          { pattern = Assignment (_, { pattern = FunApp (CompilerInternal, f, _); _ })
          ; _
          } as body
      ; _
      }
