      ...
      | x::[] -> (
          ...
          | Assignment((ident,idxs),rhs) -> (
            ...
            | Indexed(e,_) -> (
                ...
                | FunApp(CompilerInternal,name,_) when name = Internal_fun.to_string FnReadData ->
                  let pos_var = Expr.(var Typed.Meta.empty pos) in 
                  let stmts = 
                    [ Stmt.assign meta ident ~idxs Expr.(indexed (Fixed.meta e) e [index_single pos_var])
...
let%expect_test "xform_readdata" =
  ...
  let indexed = 
    Expr.(indexed Typed.Meta.empty read idxs) in
  Stmt.mock_for 7
    (Stmt.mock_for 8
...
let add_jacobians stmt = 
  Stmt.Fixed.map_pattern stmt 
    ...
    ~g:(fun meta pattern -> 
      ...
      | Assignment((ident,idxs),rhs) -> (
        match Expr.Fixed.pattern rhs with 
        | FunApp(CompilerInternal,f,args) when f = Internal_fun.to_string FnConstrain -> 
          let emeta = Expr.Typed.Meta.empty in
          let extra_arg = Expr.var emeta "lp__" in 
          let new_rhs = Expr.internal_fun emeta FnConstrain (args @ [extra_arg]) in 
          Stmt.(
            if_  meta Expr.(var emeta "jacobian__")
...
let add_read_data_vestigial_indices stmt = 
  Stmt.Fixed.map_pattern stmt 
    ...
    ~g:(fun meta pattern -> 
      ...
      | Assignment((ident,idxs),rhs) -> (
        match Expr.Fixed.pattern rhs with 
        | FunApp(CompilerInternal,f,_) when f = Internal_fun.to_string FnReadData -> 
            let emeta = Expr.Fixed.meta rhs in
            let with_vestigial_idx = 
              Expr.(indexed emeta rhs [index_single loop_bottom]) in 
...
let fix_data_reads = function 
  | stmt :: stmts -> 
    ...
    let new_stmt = 
      Stmt.(declare_sized meta DataOnly pos SInt )
    in 
    new_stmt ::stmt ::stmts 
