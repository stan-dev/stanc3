...
let gen_unconstrained_write (decl_id, Program.{out_constrained_st; _}) =
  if SizedType.is_recursive_container out_constrained_st then
    let bodyfn var =
      Stmt.Helpers.internal_nrfunapp
        (FnWriteParam {constraint_= None; var})
    ...
    let decl_var =
      ...
      ; meta=
          Expr.Typed.Meta.
            ...
            ; adlevel= DataOnly } } in
    Stmt.Helpers.internal_nrfunapp
      (FnWriteParam {constraint_= None; var= decl_var})
      [] Location_span.empty
...
let trans_prog (p : Program.Typed.t) =
  ...
  let param_writes, tparam_writes, gq_writes =
    List.partition3_map p.output_vars ~f:(fun (name, outvar) ->
        match outvar.Program.out_block with
        | Parameters -> `Fst (gen_unconstrained_write (name, outvar))
        | TransformedParameters -> `Snd (gen_write (name, outvar))
        | GeneratedQuantities -> `Trd (gen_write (name, outvar)) ) in
