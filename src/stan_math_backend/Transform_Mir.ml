...
let rec var_context_read_internal
  ...
  | SArray _ when SizedType.contains_tuple st ->
      (* The IO format for tuples is complicated in this case. Therefore, we
         need to do the following 1. Make "_flat__" decls for everything 2.
         Declare a temp for each item of this tuple 3. in a loop: i. call
         [var_context_read_inside_tuple] with the temp variable as the
         ...
         arrays-of-tuples. ii. assign those temps (forwarding as tuple) to this
         variable, properly indexed. *)
      let tupl, dims = SizedType.get_array_dims st in
      let flat_decls =
...
let rec ensure_body_in_block (Stmt.{pattern; _} as stmt) =
  ...
  let pattern =
    ensure_body_in_block_base
      Stmt.(Pattern.map Fn.id ensure_body_in_block pattern) in
  {stmt with pattern}
