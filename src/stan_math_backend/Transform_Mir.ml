...
let rec switch_expr_to_opencl available_cl_vars (Expr.Fixed.{ pattern; _ } as e) =
  ...
  | x ->
    { e with
      pattern = Expr.Fixed.Pattern.map (switch_expr_to_opencl available_cl_vars) x
    }
;;
...
let data_read smeta (decl_id, st) =
  ...
  | UFun _ | UMathLibraryFunction -> raise_s [%message "Cannot read a function type."]
  | UVector | URowVector | UMatrix | UArray _ ->
    let decl, assign, flat_var =
      ...
      , Assignment ((decl_id, flat_type, []), readfnapp decl_var) |> swrap
      , { Expr.Fixed.pattern = Var decl_id
        ; meta = Expr.Typed.Meta.{ loc = smeta; type_ = flat_type; adlevel = DataOnly }
        } )
    in
    let bodyfn var =
      let pos_increment =
        [ Assignment ((pos, UInt, []), Expr.Helpers.(binop pos_var Plus one)) |> swrap ]
      in
      let read_indexed _ =
    ...
    in
    let pos_reset =
      Stmt.Fixed.Pattern.Assignment ((pos, UInt, []), Expr.Helpers.loop_bottom) |> swrap
    in
    [ Block
        [ decl; assign; pos_reset; Stmt.Helpers.for_scalar_inv st bodyfn decl_var smeta ]
      |> swrap
    ]
;;
...
let rec base_ut_to_string = function
  ...
  | t -> raise_s [%message "Another place where it's weird to get " (t : UnsizedType.t)]
;;

let param_read
    smeta
    ( decl_id
    , Program.{ out_constrained_st = cst; out_unconstrained_st = ucst; out_block; _ } )
  =
  if not (out_block = Parameters)
  then []
  else (
    let decl_id, decl =
      match cst = ucst with
    ...
    let bodyfn var =
      ...
      in
      Stmt.Helpers.assign_indexed (SizedType.to_unsized cst) decl_id smeta readfnapp var
    in
...
let rec add_jacobians Stmt.Fixed.{ meta = smeta; pattern } =
  ...
  | Assignment (lhs, { pattern = FunApp (CompilerInternal, f, args); meta = emeta })
    ...
    let assign rhs = Stmt.{ Fixed.pattern = Assignment (lhs, rhs); meta = smeta } in
    { Stmt.Fixed.pattern =
        IfElse
          ( var "jacobian__"
          , assign
              { Expr.Fixed.pattern = FunApp (CompilerInternal, f, args @ [ var "lp__" ])
              ; meta = emeta
              }
          , Some
              (assign
                 { Expr.Fixed.pattern = FunApp (CompilerInternal, f, args); meta = emeta })
          )
    ; meta = smeta
    }
  | ptn -> Stmt.Fixed.{ pattern = Pattern.map Fn.id add_jacobians ptn; meta = smeta }
;;
...
let rec ensure_body_in_block (Stmt.Fixed.{ pattern; _ } as stmt) =
  ...
  in
  let pattern =
    ensure_body_in_block_base Stmt.Fixed.(Pattern.map Fn.id ensure_body_in_block pattern)
  in
  { stmt with pattern }
;;
  ...
  let rec flatten_slists_stmt stmt =
    let pattern =
      Stmt.Fixed.(
        ...
        | Block ls ->
          Pattern.Block
            (List.concat_map ~f:(Fn.compose flatten_slist flatten_slists_stmt) ls)
  ...
  let stmt =
    Stmt.Fixed.Pattern.(
      [ SList
          [ Block
              [ SList
                  [ While (e (Var "hi"), Block [ SList [ Break |> s ] |> s ] |> s) |> s ]
                |> s
              ]
            |> s
          ]
...
let gen_write (decl_id, sizedtype) =
  ...
  in
  let meta = { Expr.Typed.Meta.empty with type_ = SizedType.to_unsized sizedtype } in
...
let rec contains_var_expr is_vident accum Expr.Fixed.{ pattern; _ } =
  ...
  | pattern -> Expr.Fixed.Pattern.fold (contains_var_expr is_vident) false pattern
;;
...
let constrain_in_params outvars stmts =
  let is_target_var = function
    | ( name
      , { Program.out_unconstrained_st; out_constrained_st; out_block = Parameters; _ } )
      when not (out_unconstrained_st = out_constrained_st) -> Some name
    | _ -> None
  in
  let target_vars = List.filter_map outvars ~f:is_target_var |> String.Set.of_list in
  let rec change_constrain_target (Stmt.Fixed.{ pattern; _ } as s) =
    ...
    | Assignment (_, { pattern = FunApp (CompilerInternal, f, args); _ })
      when (Internal_fun.of_string_opt f = Some FnConstrain
           ...
           && List.exists args ~f:(contains_var_expr (Set.mem target_vars) false) ->
      let rec change_var_expr (Expr.Fixed.{ pattern; _ } as e) =
        ...
        | pattern -> { e with pattern = Expr.Fixed.Pattern.map change_var_expr pattern }
      in
      let rec change_var_stmt s =
        Stmt.Fixed.
          { s with pattern = Pattern.map change_var_expr change_var_stmt s.pattern }
      in
      change_var_stmt s
    | pattern ->
      Stmt.Fixed.{ s with pattern = Pattern.map Fn.id change_constrain_target pattern }
  in
  List.map ~f:change_constrain_target stmts
;;

let fn_name_map = String.Map.of_alist_exn [ "integrate_ode", "integrate_ode_rk45" ]
...
let rec insert_before f to_insert = function
  | [] -> to_insert
  | hd :: tl ->
    if f hd then to_insert @ (hd :: tl) else hd :: insert_before f to_insert tl
;;
...
let collect_opencl_vars s =
  let rec go accum s =
    Stmt.Fixed.(Pattern.fold (collect_vars_expr is_opencl_var) go accum s.pattern)
  in
...
let%expect_test "collect vars expr" =
  ...
  let fnapp =
    Expr.{ Fixed.pattern = FunApp (StanLib, "print", args); meta = Typed.Meta.empty }
  in
...
let validate_sized decl_id meta transform st =
  let check fn x =
    Stmt.Helpers.internal_nrfunapp
      fn
      Expr.Helpers.[ str decl_id; str (Fmt.strf "%a" Expression_gen.pp_expr x); x ]
      meta
  in
...
let rec add_validate_dims outvars stmts =
  ...
  let with_size_checks = function
    | Stmt.Fixed.{ pattern = Decl { decl_id; decl_type = Sized st; _ }; meta } as decl ->
...
and validate_dims_stmt stmt =
  let pattern =
    match stmt.pattern with
    | Stmt.Fixed.Pattern.Block s -> Stmt.Fixed.Pattern.Block (add_validate_dims [] s)
...
let trans_prog (p : Program.Typed.t) =
  ...
  let init_pos =
    ...
    |> List.map ~f:(fun pattern -> Stmt.Fixed.{ pattern; meta = Location_span.empty })
  in
  ...
  let get_pname_ust = function
    | ( name
      , { Program.out_block = Parameters; out_constrained_st; out_unconstrained_st; _ } )
      when SizedType.to_unsized out_constrained_st
           = SizedType.to_unsized out_unconstrained_st -> Some (name, out_unconstrained_st)
  ...
  let param_writes, tparam_writes, gq_writes =
    List.map p.output_vars ~f:(fun (name, { out_constrained_st = st; out_block; _ }) ->
  ...
  let tparam_start stmt =
    Stmt.Fixed.(
      match stmt.pattern with
      | IfElse (cond, _, _)
        when contains_var_expr (( = ) "emit_transformed_parameters__") false cond -> true
      | _ -> false)
  in
  let gq_start Stmt.Fixed.{ pattern; _ } =
    match pattern with
    | IfElse
        ( { pattern = FunApp (_, _, [ { pattern = Var "emit_generated_quantities__"; _ } ])
          ; _
          }
        , _
        , _ ) -> true
  ...
  let translate_to_open_cl stmts =
    ...
    then (
      ...
      let rec trans_stmt_to_opencl s =
        Stmt.Fixed.
          { s with pattern = Pattern.map switch_expr trans_stmt_to_opencl s.pattern }
      in
  ...
  let to_matrix_cl_stmts =
    List.concat_map opencl_vars ~f:(fun vident ->
        let vident_sans_opencl = String.chop_suffix_exn ~suffix:opencl_suffix vident in
  ...
  let p =
    ...
    ; prepare_data =
        init_pos
        @ (add_validate_dims [] p.prepare_data |> add_reads p.input_vars data_read)
        @ to_matrix_cl_stmts
    ; transform_inits =
        init_pos
