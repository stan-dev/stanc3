...
let transform_for stmt =
  ...
  | meta, For {body; _}
    when Stmt.(
           contains_internal_fun ~fn:FnReadData body
...
let%expect_test "invert write fors" =
  let stmt =
    mock_for 8 (mock_for 9 (Stmt.internal_nrfun () FnWriteParam []))
  in
    ...
    | block_meta, Block [(assign_meta, Assignment ((ident, idxs), rhs))] -> (
      ...
      | ( index_meta
        ...
        when fn_name = Internal_fun.to_string FnReadData ->
          ...
          let rhs1 =
            Expr.index_single index_meta (Expr.Fixed.inj fun_app) ~idx:pos_var
    ...
    | assign_meta, Assignment ((ident, idxs), rhs) -> (
      ...
      | fun_meta, FunApp (CompilerInternal, fn_name, args)
        when fn_name = Internal_fun.to_string FnConstrain ->
          let new_rhs =
            Expr.internal_fun fun_meta FnConstrain
              (args @ [Expr.var fun_meta "lp__"])
          in
...
let ensure_body_in_block stmt =
  ...
  let transform stmt =
    match Stmt.Fixed.proj stmt with
    | meta, While (pred, body) -> Stmt.while_ meta pred @@ wrap body
