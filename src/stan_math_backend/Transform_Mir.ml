...
let rec switch_expr_to_opencl available_cl_vars (Expr.Fixed.{pattern; _} as e) =
  ...
  let move_cl_args cl_args index arg =
    if List.mem ~equal:( = ) cl_args index then to_cl arg else arg in
  ...
  let check_if_data args ind =
    ...
    match pattern with Var s when is_avail s -> true | _ -> false in
  let req_met args (data_arg, type_arg) =
    List.for_all ~f:(check_if_data args) data_arg
    && List.for_all ~f:(check_type args) type_arg in
  ...
  | FunApp (StanLib, f, args)
    when Map.mem opencl_triggers (Utils.stdlib_distribution_name f) ->
      let trigger =
        Map.find_exn opencl_triggers (Utils.stdlib_distribution_name f) in
  ...
  if not (out_block = Parameters) then []
    ...
    let decl_id, decl =
      ...
      | false ->
          ...
          let d =
            Stmt.Fixed.Pattern.Decl
              {decl_adtype= AutoDiffable; decl_id; decl_type= Sized ucst} in
          (decl_id, [Stmt.Fixed.{meta= smeta; pattern= d}]) in
    let unconstrained_decl_var =
      let meta =
        Expr.Typed.Meta.create ~loc:smeta
          ~type_:SizedType.(to_unsized cst)
          ~adlevel:AutoDiffable () in
      Expr.Fixed.{meta; pattern= Var decl_id} in
    let bodyfn var =
      let readfnapp (var : Expr.Typed.t) =
        Expr.(
          Helpers.(
            ...
            Typed.Meta.{var.meta with type_= base_type ucst}) in
...
let rec add_jacobians Stmt.Fixed.{meta= smeta; pattern} =
  ...
  | Assignment (lhs, {pattern= FunApp (CompilerInternal, f, args); meta= emeta})
    when Internal_fun.of_string_opt f = Some FnConstrain ->
      ...
      let assign rhs =
        Stmt.{Fixed.pattern= Assignment (lhs, rhs); meta= smeta} in
      { Stmt.Fixed.pattern=
          IfElse
            ( var "jacobian__"
            , assign
...
let gen_write (decl_id, sizedtype) =
  let bodyfn var =
    Stmt.Helpers.internal_nrfunapp FnWriteParam [var] Location_span.empty in
  let meta =
    {Expr.Typed.Meta.empty with type_= SizedType.to_unsized sizedtype} in
...
let gen_write_unconstrained (decl_id, sizedtype) =
  ...
  let writefn var =
    Stmt.Helpers.for_scalar_inv
      ...
      bodyfn var Location_span.empty in
...
let constrain_in_params outvars stmts =
  let is_target_var = function
    ...
    | _ -> None in
  let target_vars =
    List.filter_map outvars ~f:is_target_var |> String.Set.of_list in
...
let validate_sized decl_id meta transform st =
  ...
  let nrfunapp fname args =
    Stmt.Fixed.{pattern= NRFunApp (CompilerInternal, fname, args); meta} in
          ...
          | Some CholeskyCov ->
              nrfunapp "check_greater_or_equal"
                Expr.Helpers.
                  ...
                  ; str "num rows (must be greater or equal to num cols)"; rows
                  ; cols ]
          | _ -> check FnValidateSize rows in
        [validate_rows; check FnValidateSize cols] in
  dims_check st
...
let rec add_validate_dims outvars stmts =
  ...
  let with_size_checks = function
    ...
    | stmt -> [validate_dims_stmt stmt] in
...
let trans_prog (p : Program.Typed.t) =
  ...
  let gq_start Stmt.Fixed.{pattern; _} =
    ...
    | IfElse
        ( { pattern=
              FunApp (_, _, [{pattern= Var "emit_generated_quantities__"; _}])
          ; _ }
        , _
        , _ ) ->
        true
  ...
  let translate_to_open_cl stmts =
    if !use_opencl then
      let decl Stmt.Fixed.{pattern; _} =
        match pattern with Decl d -> Some d.decl_id | _ -> None in
