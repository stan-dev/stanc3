...
and gen_distribution_app f =
  let sfx = "_unnormalized" in
  if
    String.is_suffix f ~suffix:("_lpmf" ^ sfx)
    || String.is_suffix f ~suffix:("_lpdf" ^ sfx)
    || String.is_suffix f ~suffix:("_log" ^ sfx)
  then
    Some
      (fun ppf ->
        let f = String.chop_suffix_exn ~suffix:sfx f in
