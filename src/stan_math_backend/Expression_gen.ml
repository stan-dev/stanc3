...
let types_match e1 e2 =
  ...
  && UnsizedType.compare_autodifftype
       (Expr.Typed.adlevel_of e1)
...
let%expect_test "promote_unsized" =
  ...
  in
  print_s [%sexp (tests |> List.map ~f:promote_unsizedtype : UnsizedType.t list)];
...
let pp_expr_type ppf e = pp_unsizedtype_local ppf Expr.Typed.(adlevel_of e, type_of e)
...
let is_user_dist s =
  ends_with_any user_dist_suffices s && not (ends_with_any [ "_cdf_log"; "_ccdf_log" ] s)
;;
...
let demangle_propto_name udf f =
  if f = "multiply_log" || f = "binomial_coefficient_log"
  then f
  else if Utils.is_propto_distribution f
  then Utils.stdlib_distribution_name f ^ "<propto__>"
  else if Utils.is_distribution_name f || (udf && (is_user_dist f || is_user_lp f))
  then f ^ "<false>"
  else f
;;
...
let functor_suffix_select hof =
  if Stan_math_signatures.is_reduce_sum_fn hof
  then reduce_sum_functor_suffix
  else functor_suffix
;;
...
let rec pp_index ppf = function
  ...
  | Between (e_low, e_high) -> pf ppf "index_min_max(%a, %a)" pp_expr e_low pp_expr e_high
  | MultiIndex e -> pf ppf "index_multi(%a)" pp_expr e

and pp_indexes ppf = function
  | [] -> pf ppf "nil_index_list()"
  | idx :: idxs -> pf ppf "@[<hov 2>cons_list(@,%a,@ %a)@]" pp_index idx pp_indexes idxs

and pp_logical_op ppf op lhs rhs =
  pf ppf "(primitive_value(@,%a)@ %s@ primitive_value(@,%a))" pp_expr lhs op pp_expr rhs
...
and pp_binary_f ppf f es = pf ppf "%s(@,%a,@ %a)" f pp_expr (first es) pp_expr (second es)
...
and pp_scalar_binary ppf scalar_fmt generic_fmt es =
  pp_binary
    ppf
    (if is_scalar (first es) && is_scalar (second es) then scalar_fmt else generic_fmt)
    es
...
and gen_operator_app = function
  ...
  | PMinus ->
    fun ppf es ->
      pp_unary ppf (if is_scalar (List.hd_exn es) then "-%a" else "minus(@,%a)") es
  | PPlus -> fun ppf es -> pp_unary ppf "%a" es
  | Transpose ->
    fun ppf es ->
      pp_unary ppf (if is_scalar (List.hd_exn es) then "%a" else "transpose(@,%a)") es
  | PNot -> fun ppf es -> pp_unary ppf "logical_negation(@,%a)" es
  | Minus -> fun ppf es -> pp_scalar_binary ppf "(%a@ -@ %a)" "subtract(@,%a,@ %a)" es
  | Times -> fun ppf es -> pp_scalar_binary ppf "(%a@ *@ %a)" "multiply(@,%a,@ %a)" es
  | Divide | IntDivide ->
    fun ppf es ->
      if is_matrix (second es) && (is_matrix (first es) || is_row_vector (first es))
  ...
  | And | Or -> raise_s [%message "And/Or should have been converted to an expression"]
  | EltTimes ->
...
and gen_misc_special_math_app f =
  ...
  | "ceil" ->
    let std_prefix_data_scalar f = function
      | [ Expr.
            { Fixed.meta = Typed.Meta.{ adlevel = DataOnly; type_ = UInt | UReal; _ }; _ }
        ] -> "std::" ^ f
      | _ -> f
    in
    Some
...
and read_data ut ppf es =
  let i_or_r =
    ...
    | UFun (_, _)
    | UMathLibraryFunction -> raise_s [%message "Can't ReadData of " (ut : UnsizedType.t)]
  in
...
and gen_fun_app ppf fname es =
  let default ppf es =
    ...
    let convert_hof_vars = function
      | { Expr.Fixed.pattern = Var name; meta = { Expr.Typed.Meta.type_ = UFun _; _ } } as
        e -> { e with pattern = FunApp (StanLib, name ^ functor_suffix_select fname, []) }
      | e -> e
    in
    ...
    *)
    let fname, args =
      ...
      | true, "integrate_ode_bdf", f :: y0 :: t0 :: ts :: theta :: x :: x_int :: tl
      | true, "integrate_ode_adams", f :: y0 :: t0 :: ts :: theta :: x :: x_int :: tl
      | true, "integrate_ode_rk45", f :: y0 :: t0 :: ts :: theta :: x :: x_int :: tl ->
...
and pp_compiler_internal_fn ut f ppf es =
  let pp_array_literal ppf es =
    ...
    pf
      ppf
      "stan::math::array_builder<%a>()@,.add(%a)@,.array()"
  ...
  | Some FnMakeRowVec ->
    ...
    | _ ->
      raise_s [%message "Unexpected type for row vector literal" (ut : UnsizedType.t)])
  ...
  | Some FnReadParam ->
    ...
    | { Expr.Fixed.pattern = Lit (Str, base_type); _ } :: dims ->
      pf ppf "@[<hov 2>in__.%s(@,%a)@]" base_type (list ~sep:comma pp_expr) dims
...
and pp_indexed_simple ppf (obj, idcs) =
  let idx_minus_one = function
    ...
    | All ->
      raise_s
        [%message "No non-Single indices allowed" ~obj (idcs : Expr.Typed.t Index.t list)]
  in
  pf
    ppf
    "%s%a"
...
and pp_expr ppf Expr.Fixed.({ pattern; meta } as e) =
  ...
  | TernaryIf (ec, et, ef) ->
    ...
    then tform ppf pp_expr ec pp_expr et pp_expr ef
    else tform ppf pp_expr ec promoted (e, et) promoted (e, ef)
  | Indexed (e, []) -> pp_expr ppf e
  | Indexed (e, idx) ->
    (match e.pattern with
    | FunApp (CompilerInternal, f, _)
      when Some Internal_fun.FnReadParam = Internal_fun.of_string_opt f -> pp_expr ppf e
    | FunApp (CompilerInternal, f, _)
...
let dummy_locate pattern =
  Expr.(
    Fixed.
      { pattern
      ; meta = Typed.Meta.{ type_ = UInt; adlevel = DataOnly; loc = Location_span.empty }
      })
;;
