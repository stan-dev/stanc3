...
let pp_function__ ppf (prog_name, fname) =
  pf ppf "static const char* function__ = %S;@ " (strf "%s_namespace::%s" prog_name fname);
  ...
  | _ ->
    let rec promote_args_chunked ppf args =
      let go ppf tl =
        match tl with
        | [] -> ()
        | _ -> pf ppf ", %a" promote_args_chunked tl
      in
      match args with
      | [] -> pf ppf "double"
      | hd :: tl -> pf ppf "stan::promote_args_t<%a%a>" (list ~sep:comma string) hd go tl
    in
...
let pp_located_error_b ppf body_stmts =
  pp_located_error
    ppf
    (pp_statement, Stmt.Fixed.{ pattern = Block body_stmts; meta = Locations.no_span_num })
;;
...
let get_templates_and_args fdargs =
  ...
  , List.map ~f:(fun a -> strf "%a" pp_arg a) (List.zip_exn argtypetemplates fdargs) )
;;
...
let pp_template_decorator ppf = function
  | [] -> ()
  | templates -> pf ppf "@[<hov>template <%a>@]@ " (list ~sep:comma string) templates
;;
...
*)
let pp_fun_def ppf Program.{ fdrt; fdname; fdargs; fdbody; _ } funs_used_in_reduce_sum =
  ...
  let extra, extra_templates =
    if is_lp
    then [ "lp__"; "lp_accum__" ], [ "T_lp__"; "T_lp_accum__" ]
    else if is_rng
    then [ "base_rng__" ], [ "RNG" ]
    else [], []
  in
  ...
  let pp_sig_rs ppf name =
    if is_dist
    then pp_template_decorator ppf (List.tl_exn templates)
  ...
  | _ ->
    ...
    if String.Set.mem funs_used_in_reduce_sum fdname
    then (
      ...
      | (_, slice, _) :: (_, start, _) :: (_, end_, _) :: rest ->
        pf
          ...
          , (slice
            ...
            @ extra
            @ [ "pstream__" ] )
      | _ -> raise_s [%message "Ill-formed reduce_sum call! This is bug in the compiler."])
;;
...
let pp_validate_data ppf (name, st) =
  if String.is_suffix ~suffix:"__" name
  then ()
  else
    pf
      ppf
      "@[<hov 4>context__.validate_dims(@,%S,@,%S,@,%S,@,%a);@]@ "
      "data initialization"
      name
    ...
    | ( decl_id
      ...
      let meta =
        ...
        |> List.find ~f:(function
               | { Stmt.Fixed.pattern = Decl { decl_id = id; _ }; _ } when id = decl_id ->
                 true
               | _ -> false)
  ...
  pp_block
    ...
    ( (fun ppf { Program.prog_name; prepare_data; output_vars; _ } ->
        ...
        pp_located_error
          ppf
          ( pp_block
          , (list ~sep:cut pp_num_param, List.filter_map ~f:get_param_st output_vars) ))
    , p )
;;

let rec top_level_decls Stmt.Fixed.{ pattern; _ } =
  match pattern with
  | Decl d -> [ Some (d.decl_id, Type.to_unsized d.decl_type, UnsizedType.DataOnly) ]
...
let pp_get_dims ppf { Program.output_vars; _ } =
  ...
  let pp_pack ppf inner_dims =
    pf ppf "std::vector<size_t>{@[<hov>@,%a@]}" (list ~sep:comma pp_cast) inner_dims
  in
...
let rec pp_for_loop_iteratee ?(index_ids = []) ppf (iteratee, dims, pp_body) =
  ...
  | dim :: dims ->
    iter dim (fun ppf (i, idcs) ->
        pf ppf "%a" pp_block (pp_for_loop_iteratee ~index_ids:idcs, (i, dims, pp_body)))
;;
...
let pp_constrained_param_names ppf { Program.output_vars; _ } =
  ...
  pp_method
    ...
    (fun ppf ->
      ...
      pf
        ppf
        "@,if (emit_transformed_parameters__) %a@,"
        pp_block
        (list ~sep:cut pp_param_names, tparamvars);
      pf
        ppf
        "@,if (emit_generated_quantities__) %a@,"
        pp_block
...
let pp_transform_inits ppf { Program.transform_inits; _ } =
  let params =
    [ "const stan::io::var_context& context__"
    ; "std::vector<int>& params_i__"
    ; "std::vector<double>& vars__"
    ; "std::ostream* pstream__"
    ]
  ...
  let rec find_functors_expr accum Expr.Fixed.{ pattern; _ } =
    String.Set.union
      ...
      | FunApp (StanLib, x, { pattern = Var f; _ } :: _)
        when Stan_math_signatures.is_reduce_sum_fn x -> String.Set.of_list [ f ]
      | x -> Expr.Fixed.Pattern.fold find_functors_expr accum x)
  in
  let rec find_functors_stmt accum stmt =
    Stmt.Fixed.(Pattern.fold find_functors_expr find_functors_stmt accum stmt.pattern)
  in
...
let pp_prog ppf (p : Program.Typed.t) =
  ...
  pf
    ppf
    "@[<v>@ %s@ %s@ namespace %s {@ %s@ %s@ %a@ %s@ %a@ %a@ }@ @]"
    version
    includes
    (namespace p)
    custom_functions
    usings
    Locations.pp_globals
    s
