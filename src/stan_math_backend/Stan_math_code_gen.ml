...
|}
  ...
  | None -> pf ppf ";@ "
  | Some _ ->
      pf ppf "@,%s@,%s %s%a @,{@,%s %s%s::%a;@,}@," rcpp_comment
        return_type fdname pp_sig_standalone "" rcpp_accum return_stmt 
        namespace_fun pp_call_str
        ( ( if is_user_dist fdname || is_user_lp fdname then fdname ^ "<false>"
          else fdname )
        , List.map ~f:(fun (_, name, _) -> name) fdargs @ extra @ lp_accum_opt @ ["pstream__"]
        )
