...
|}
  ...
  let lp_accum_opt =
    if !rcpp_friendly && is_user_lp fdname then ["lp_accum__"] else []
  in
  match fdbody with
  | None -> pf ppf ";@ "
  | Some _ ->
      pf ppf "@,%s@,%s %s%a @,{@,%s %s%s::%a;@,}@," rcpp_comment return_type
        fdname pp_sig_standalone "" rcpp_accum return_stmt namespace_fun
        pp_call_str
        ( ( if is_user_dist fdname || is_user_lp fdname then fdname ^ "<false>"
          else fdname )
        , List.map ~f:(fun (_, name, _) -> name) fdargs
          @ extra @ lp_accum_opt @ ["pstream__"] )

let version = "// Code generated by %%NAME%% %%VERSION%%"

let includes_standalone_functions rcpp =
  if rcpp then
    {|
...
|}
  else "#include <stan/model/model_header.hpp>"
...
let pp_prog ppf (p : Program.Typed.t) =
  ...
  in
  let pp_standalone_funs_def ppf fblock =
    pp_standalone_fun_def ppf fblock (namespace p) !rcpp_friendly
  in
  ...
  if !standalone_functions then
    pf ppf "@[<v>@ %s@ %s@ namespace %s {@ %s@ %a@ %s@ %a@}}@ %a@ @]" version
      (includes_standalone_functions !rcpp_friendly)
      (namespace p) usings Locations.pp_globals s
