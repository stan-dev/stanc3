...
let pp_standalone_fun_def ppf Program.({fdname; fdargs; fdbody; _}) namespace_fun =
  let is_lp = is_user_lp fdname in
  let is_dist = is_user_dist fdname in
  let is_rng = String.is_suffix fdname ~suffix:"_rng" in
  let extra, extra_templates =
    if is_lp then (["lp__"; "lp_accum__"], ["double"; "stan::math::accumulator<double>"])
    else if is_rng then (["base_rng__"], ["boost::ecuyer1988"])
    else ([], [])
  in
  let mk_extra_args templates args =
    List.map ~f:(fun (t, v) -> t ^ "& " ^ v) (List.zip_exn templates args)
  ...
  | Skip -> pf ppf ";@ "
  | _ -> (
      pf ppf "@,@,auto %s%a @,{@,return %s::%a;@,}@," fdname
         pp_sig_standalone "" namespace_fun pp_call_str
        ( (if is_dist || is_lp then fdname ^ "<false>" else fdname)
        , List.map ~f:(fun (_, name, _) -> name) fdargs @ extra @ ["pstream__"]
        ))
...
let pp_prog ppf (p : Program.Typed.t) =
  ...
  in
   let pp_standalone_funs_def ppf fblock =
    pp_standalone_fun_def ppf fblock (namespace p)
  in
