...
*)
let pp_fun_def ppf Program.({fdrt; fdname; fdargs; fdbody; _}) _funs_used_in_ode_bdf_ =
  ...
  in
  let pp_sig_ob ppf name =
    pp_template_decorator ppf templates ;
    pp_returntype ppf fdargs fdrt ;
    let first_three, rest = List.split_n args 2 in
    let arg_strs =
      first_three
      @ rest
  ...
  | _ ->
      ...
      (* Produces the ode_bdf functors that has the pstream argument
      as the third and not last argument *)
      let first_two, rest_fdargs = List.split_n fdargs 2 in
      (* if List.mem funs_used_in_ode_bdf_ fdname ~equal:( = ) then *)
      pf ppf "@,@,struct %s%s {@,%a const @,{@,return %a;@,}@,};@," fdname
        ode_bdf_functor_suffix pp_sig_ob "operator()" pp_call_str
        ( fdname
        , List.map ~f:(fun (_, name, _) -> name) first_two
...
let pp_prog ppf (p : Program.Typed.t) =
  ...
  let pp_fun_def_with_ob_list ppf fblock =
    pp_fun_def ppf fblock (fun_used_in_ode_bdf p)
  in
  pf ppf "@[<v>@ %s@ %s@ namespace %s {@ %s@ %s@ %a@ %a@ %a@ }@ @]" version
    includes (namespace p) custom_functions usings Locations.pp_globals s
    (list ~sep:cut pp_fun_def_with_ob_list)
