...
let pp_fun_def ppf Program.({fdrt; fdname; fdargs; fdbody; _}) =
  ...
  | _ ->
      ...
      let first_two_types, _ = List.split_n fdargs 2 in
      let types_of_args = List.map ~f:(fun (_, _, t) -> t) first_two_types in 
      match types_of_args with
      | [UnsizedType.UInt; UnsizedType.UInt] -> 
        pf ppf "@,@,struct %s%s {@,%a const @,{@,return %a;@,}@,};@," fdname
