...
let pp_fun_def ppf Program.({fdrt; fdname; fdargs; fdbody; _}) =
  ...
  let pp_sig_rs ppf name =
    pp_template_decorator ppf templates ;
    pp_returntype ppf fdargs fdrt ;
    let first_three_args = List.mapi ~f:(fun i x -> if i < 3 then x else "") args
    in
    let rest_args = List.mapi ~f:(fun i x -> if i >= 3 then x else "") args
    in
    let arg_strs =
      mk_extra_args prefix_extra_templates prefix_extra_args
  ...
  | _ ->
      ...
      pf ppf "@,@,struct %s%s {@,%a const @,{@,return %a;@,}@,};@," fdname
        ...
        , prefix_extra_args
          ...
          @ extra @ ["pstream__"] );
      pf ppf "@,@,struct %s%s {@,%a const @,{@,return %a;@,}@,};@," fdname
        functor_reduce_sum_suffix pp_sig_rs "operator()" pp_call_str
        ( fdname
        , prefix_extra_args
          @ List.map ~f:(fun (_, name, _) -> name) fdargs
          @ ["pstream__"]
