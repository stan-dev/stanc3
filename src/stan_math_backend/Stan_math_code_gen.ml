...
let pp_prog ppf (p : Program.Typed.t) =
  ...
  let pp_fun_def_with_rs_list ppf fblock = pp_fun_def ppf fblock (fun_used_in_reduce_sum p) in
  pf ppf "@[<v>@ %s@ %s@ namespace %s {@ %s@ %s@ %a@ %a@ %a@ }@ @]" version
    includes (namespace p) custom_functions usings Locations.pp_globals s
    (list ~sep:cut pp_fun_def_with_rs_list) p.functions_block pp_model p ;
