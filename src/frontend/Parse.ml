    ...
    | (lazy Nil) ->
      let message =
        "Expected \"functions {\" or \"data {\" or \"transformed data {\" or \
         \"parameters {\" or \"transformed parameters {\" or \"model {\" or \"generated \
         quantities {\".\n"
      in
      Errors.Parsing
        ( message
    ...
    | (lazy (Cons (Interp.Element (state, _, start_pos, end_pos), _))) ->
      let message =
        try
          ...
          ^
          if !Debugging.grammar_logging
          then "(Parse error state " ^ string_of_int (Interp.number state) ^ ")"
          else ""
        with
        | Not_found_s _ ->
          if !Debugging.grammar_logging
      ...
      in
      Errors.Parsing (message, Location_span.of_positions_exn start_pos end_pos)
      |> Result.Error
  in
