...
let probability_distribution_name_variants id =
  ...
  List.map
    ...
    then [ name ]
    else if is_suffix ~suffix:"_lpmf" name
    then [ name; drop_suffix name 5 ^ "_lpdf"; drop_suffix name 5 ^ "_log" ]
    else if is_suffix ~suffix:"_lpdf" name
    then [ name; drop_suffix name 5 ^ "_lpmf"; drop_suffix name 5 ^ "_log" ]
    else if is_suffix ~suffix:"_lcdf" name
    then [ name; drop_suffix name 5 ^ "_cdf_log" ]
    else if is_suffix ~suffix:"_lccdf" name
    then [ name; drop_suffix name 6 ^ "_ccdf_log" ]
    else if is_suffix ~suffix:"_cdf_log" name
    then [ name; drop_suffix name 8 ^ "_lcdf" ]
    else if is_suffix ~suffix:"_ccdf_log" name
    then [ name; drop_suffix name 9 ^ "_lccdf" ]
    else if is_suffix ~suffix:"_log" name
    then [ name; drop_suffix name 4 ^ "_lpmf"; drop_suffix name 4 ^ "_lpdf" ]
    else [ name ])
;;
...
let lub_rt loc rt1 rt2 =
  ...
  | ReturnType UInt, ReturnType UReal -> Validate.ok (UnsizedType.ReturnType UReal)
...
let check_fresh_variable_basic id is_nullary_function =
  Validate.(
    ...
    (* For some strange reason, Stan allows user declared identifiers that are
       ...
       achieve that. *)
    if (Stan_math_signatures.is_stan_math_function_name id.name
       && (is_nullary_function
...
let check_fresh_variable id is_nullary_function =
  List.fold
    ~init:(Validate.ok ())
    ~f:(fun v0 name ->
      check_fresh_variable_basic name is_nullary_function |> Validate.apply_const v0)
...
let semantic_check_identifier id =
  Validate.(
    ...
    then Semantic_error.ident_is_model_name id.id_loc id.name |> error
    else if String.is_suffix id.name ~suffix:"__"
...
let semantic_check_fn_map_rect ~loc id es =
  Validate.(
    ...
    | "map_rect", { expr = Variable arg1; _ } :: _
      when String.(
             is_suffix arg1.name ~suffix:"_lp" || is_suffix arg1.name ~suffix:"_rng") ->
...
let semantic_check_fn_conditioning ~loc id =
  Validate.(
    if List.exists [ "_lpdf"; "_lpmf"; "_lcdf"; "_lccdf" ] ~f:(fun x ->
...
let semantic_check_fn_target_plus_equals cf ~loc id =
  Validate.(
    if String.is_suffix id.name ~suffix:"_lp"
...
let semantic_check_fn_rng cf ~loc id =
  Validate.(
    if String.is_suffix id.name ~suffix:"_rng" && cf.in_toplevel_decl
    then Semantic_error.invalid_decl_rng_fn loc |> error
    else if String.is_suffix id.name ~suffix:"_rng"
...
let semantic_check_fn_normal ~is_cond_dist ~loc id es =
  Validate.(
    ...
    | Some (_, UnsizedType.UFun (_, Void)) ->
      Semantic_error.returning_fn_expected_nonreturning_found loc id.name |> error
...
let semantic_check_fn_stan_math ~is_cond_dist ~loc id es =
  match Stan_math_signatures.stan_math_returntype id.name (get_arg_types es) with
  | Some UnsizedType.Void ->
    Semantic_error.returning_fn_expected_nonreturning_found loc id.name |> Validate.error
...
let semantic_check_reduce_sum ~is_cond_dist ~loc id es =
  ...
  | { emeta =
    ...
    then
      ...
      Semantic_error.illtyped_reduce_sum
        loc
        id.name
...
let fn_kind_from_application id es =
  ...
  if Stan_math_signatures.stan_math_returntype
       id.name
...
let semantic_check_ternary_if loc (pe, te, fe) =
  Validate.(
    let err =
      Semantic_error.illtyped_ternary_if loc pe.emeta.type_ te.emeta.type_ fe.emeta.type_
    in
    if pe.emeta.type_ = UInt
    then (
...
let semantic_check_binop loc op (le, re) =
  Validate.(
    let err = Semantic_error.illtyped_binary_op loc op le.emeta.type_ re.emeta.type_ in
    [ le; re ]
    |> List.map ~f:arg_type
...
let check_consistent_types ad_level type_ es =
  let f state e =
    ...
    | Ok (ad, ty) ->
      let ad =
        if UnsizedType.autodifftype_can_convert e.emeta.ad_level ad
        then e.emeta.ad_level
        else ad
      in
...
let semantic_check_rowvector ~loc es =
  Validate.(
    ...
    | { emeta = { ad_level; type_ = UnsizedType.URowVector; _ }; _ } :: elements ->
      ...
      | Ok (ad_level, _) ->
        mk_typed_expression ~expr:(RowVectorExpr es) ~ad_level ~type_:UMatrix ~loc |> ok
      | Error (_, meta) ->
        Semantic_error.invalid_matrix_types meta.loc meta.type_ |> error)
    | _ ->
      (match check_consistent_types DataOnly UReal es with
      | Ok (ad_level, _) ->
        mk_typed_expression ~expr:(RowVectorExpr es) ~ad_level ~type_:URowVector ~loc
        |> ok
      | Error (_, meta) ->
  ...
  let rec aux k ut xs =
    ...
    | _, next :: rest ->
      ...
      | _ ->
        (match ut with
        | UArray inner_ty ->
          let k' = Fn.compose k (Validate.map ~f:(fun t -> UnsizedType.UArray t)) in
          aux k' inner_ty rest
  ...
  UnsizedType.lub_ad_type
    ...
    :: List.map
         ~f:(function
           ...
           | Between (ue1, ue2) ->
             UnsizedType.lub_ad_type [ at; ue1.emeta.ad_level; ue2.emeta.ad_level ])
         uindices)
;;
...
let rec semantic_check_indexed ~loc ~cf e indices =
  Validate.(
    ...
    |> map ~f:(fun ut ->
           mk_typed_expression ~expr:(Indexed (ue, uindices)) ~ad_level:at ~type_:ut ~loc))
...
and semantic_check_index cf = function
  ...
  | Single e ->
    Validate.(
      ...
      >>= fun ue ->
      if has_int_type ue || has_int_array_type ue
      then ok @@ Single ue
      else
        Semantic_error.int_intarray_or_range_expected ue.emeta.loc ue.emeta.type_ |> error)
  | Upfrom e ->
...
and semantic_check_expression cf ({ emeta; expr } : Ast.untyped_expression)
    ...
    and warn_int_division (x, y) =
      ...
      | UInt, UInt, Divide ->
        let hint ppf () =
          ...
          | IntNumeral x, _ -> Fmt.pf ppf "%s.0 / %a" x Pretty_printing.pp_expression y
          | _, Ast.IntNumeral y ->
          ...
          | _ ->
            Fmt.pf
              ppf
              "%a * 1.0 / %a"
              Pretty_printing.pp_expression
              x
              Pretty_printing.pp_expression
              y
        in
        Fmt.pr
          "@[<v>@[<hov 0>Info: Found int division at %s:@]@   @[<hov 2>%a@]@,\
           @[<hov>%a@]@   @[<hov 2>%a@]@,\
           @[<hov>%a@]@]"
  ...
  | RealNumeral s ->
    mk_typed_expression
      ...
      ~loc:emeta.loc
    |> Validate.ok
  | FunApp (_, id, es) -> semantic_check_funapp ~is_cond_dist:false id es cf emeta
  | CondDistApp (_, id, es) -> semantic_check_funapp ~is_cond_dist:true id es cf emeta
  | GetLP ->
    (* Target+= can only be used in model and functions with right suffix (same for tilde etc) *)
    if not (cf.in_lp_fun_def || cf.current_block = Model || cf.current_block = TParam)
    then
  ...
  | Paren e ->
    ...
    |> Validate.map ~f:(fun ue ->
           mk_typed_expression
             ~expr:(Paren ue)
             ~ad_level:ue.emeta.ad_level
             ~type_:ue.emeta.type_
             ~loc:emeta.loc)
  | Indexed (e, indices) -> semantic_check_indexed ~loc:emeta.loc ~cf e indices

and semantic_check_funapp ~is_cond_dist id es cf emeta =
  let name_check =
    if is_cond_dist then semantic_check_conddist_name else semantic_check_fn_conditioning
  in
  Validate.(
    es
    |> List.map ~f:(semantic_check_expression cf)
...
and semantic_check_expression_of_int_or_real_type cf e name =
  Validate.(
    semantic_check_expression cf e
    >>= fun ue ->
    if has_int_or_real_type ue
    then ok ue
    else Semantic_error.int_or_real_expected ue.emeta.loc name ue.emeta.type_ |> error)
;;
...
let semantic_check_expression_of_scalar_or_type cf t e name =
  Validate.(
    semantic_check_expression cf e
    >>= fun ue ->
    if UnsizedType.is_scalar_type ue.emeta.type_ || ue.emeta.type_ = t
    then ok ue
    else
      Semantic_error.scalar_or_type_expected ue.emeta.loc name t ue.emeta.type_ |> error)
;;
...
let semantic_check_transformation cf ut = function
  ...
  | LowerUpper (e1, e2) ->
    let ue1 = semantic_check_expression_of_scalar_or_type cf ut e1 "Lower bound"
    and ue2 = semantic_check_expression_of_scalar_or_type cf ut e2 "Upper bound" in
  ...
  | OffsetMultiplier (e1, e2) ->
    ...
    and ue2 = semantic_check_expression_of_scalar_or_type cf ut e2 "Multiplier" in
    Validate.liftA2 (fun ue1 ue2 -> Program.OffsetMultiplier (ue1, ue2)) ue1 ue2
...
let semantic_check_truncation cf = function
  ...
  | TruncateBetween (e1, e2) ->
    let ue1 = semantic_check_expression_of_int_or_real_type cf e1 "Truncation bound"
    and ue2 = semantic_check_expression_of_int_or_real_type cf e2 "Truncation bound" in
...
let semantic_check_nrfn_normal ~loc id es =
  Validate.(
    ...
    | Some (_, UFun (listedtypes, Void))
      when UnsizedType.check_compatible_arguments_mod_conv
             id.name
             listedtypes
             (get_arg_types es) ->
      mk_typed_statement
    ...
    | Some (_, UFun (listedtypes, Void)) ->
      es
      |> List.map ~f:type_of_expr_typed
      |> Semantic_error.illtyped_userdefined_fn_app loc id.name listedtypes Void
      |> error
    | Some (_, UFun (_, ReturnType _)) ->
      Semantic_error.nonreturning_fn_expected_returning_found loc id.name |> error
    | Some _ -> Semantic_error.nonreturning_fn_expected_nonfn_found loc id.name |> error
    | None ->
      Semantic_error.nonreturning_fn_expected_undeclaredident_found loc id.name |> error)
;;
...
let semantic_check_nrfn_stan_math ~loc id es =
  Validate.(
    match Stan_math_signatures.stan_math_returntype id.name (get_arg_types es) with
    | Some UnsizedType.Void ->
      mk_typed_statement ~stmt:(NRFunApp (StanLib, id, es)) ~return_type:NoReturnType ~loc
      |> ok
...
let semantic_check_assignment_read_only ~loc id =
  Validate.(
    if Symbol_table.get_read_only vm id.name
...
let mk_assignment_from_indexed_expr assop lhs rhs =
  Assignment { assign_lhs = Ast.lvalue_of_expr lhs; assign_op = assop; assign_rhs = rhs }
;;
...
let semantic_check_target_pe_expr_type ~loc e =
  ...
  | UFun _ | UMathLibraryFunction ->
    Semantic_error.int_or_real_container_expected loc e.emeta.type_ |> Validate.error
  | _ -> Validate.ok ()
;;

let semantic_check_target_pe_usage ~loc ~cf =
  if cf.in_lp_fun_def || cf.current_block = Model
  then Validate.ok ()
  else Semantic_error.target_plusequals_outisde_model_or_logprob loc |> Validate.error
;;
...
let semantic_check_sampling_pdf_pmf id =
  Validate.(
    if String.(is_suffix id.name ~suffix:"_lpdf" || is_suffix id.name ~suffix:"_lpmf")
...
let semantic_check_sampling_cdf_ccdf ~loc id =
  Validate.(
    if String.(is_suffix id.name ~suffix:"_cdf" || is_suffix id.name ~suffix:"_ccdf")
...
let semantic_check_valid_sampling_pos ~loc ~cf =
  Validate.(
    ...
    then error @@ Semantic_error.target_plusequals_outisde_model_or_logprob loc
    else ok ())
;;
...
let semantic_check_sampling_distribution ~loc id arguments =
  ...
  in
  Validate.(
    if is_reat_rt_for_suffix "_lpdf"
...
let can_truncate_distribution ~loc (arg : typed_expression) = function
  ...
  | _ ->
    if UnsizedType.is_scalar_type arg.emeta.type_
    then Validate.ok ()
...
let semantic_check_break ~loc ~cf =
  Validate.(
    if cf.loop_depth = 0
    then Semantic_error.break_outside_loop loc |> error
...
let semantic_check_continue ~loc ~cf =
  Validate.(
    (* Break and continue only occur in loops. *)
    if cf.loop_depth = 0
    then Semantic_error.continue_outside_loop loc |> error
...
let semantic_check_return ~loc ~cf e =
  Validate.(
    if not cf.in_returning_fun_def
    then Semantic_error.expression_return_outside_returning_fn loc |> error
    else
      semantic_check_expression cf e
      |> map ~f:(fun ue ->
             mk_typed_statement
               ~stmt:(Return ue)
               ~return_type:(Complete (ReturnType ue.emeta.type_))
               ~loc))
;;
...
let semantic_check_returnvoid ~loc ~cf =
  Validate.(
    if (not cf.in_fun_def) || cf.in_returning_fun_def
    then Semantic_error.void_ouside_nonreturning_fn loc |> error
    else mk_typed_statement ~stmt:ReturnVoid ~return_type:(Complete Void) ~loc |> ok)
;;
...
let try_compute_ifthenelse_statement_returntype loc srt1 srt2 =
  ...
  | AnyReturnType, NoReturnType | NoReturnType, AnyReturnType | NoReturnType, NoReturnType
  ...
  | Incomplete rt, NoReturnType -> Validate.ok @@ Incomplete rt
  | Complete rt, AnyReturnType | AnyReturnType, Complete rt -> Validate.ok @@ Complete rt
...
let rec semantic_check_if_then_else ~loc ~cf pred_e s_true s_false_opt =
  ...
  and ue =
    semantic_check_expression_of_int_or_real_type cf pred_e "Condition in conditional"
  in
  Validate.(
...
and semantic_check_while ~loc ~cf e s =
  ...
  and ue = semantic_check_expression_of_int_or_real_type cf e "Condition in while-loop" in
  Validate.liftA2
    (fun ue us ->
      mk_typed_statement ~stmt:(While (ue, us)) ~return_type:us.smeta.return_type ~loc)
    ue
    us
  ...
  Validate.(
    ...
    |> map ~f:(fun us ->
           mk_typed_statement
             ...
             ~return_type:us.smeta.return_type
             ~loc))
...
and list_until_escape xs =
  let rec aux accu = function
    | next :: next' :: _ when stmt_is_escape next' -> List.rev (next' :: next :: accu)
...
and try_compute_block_statement_returntype loc srt1 srt2 =
  ...
  | Incomplete rt, AnyReturnType
  | AnyReturnType, Complete rt -> Validate.ok @@ Complete rt
...
and semantic_check_var_decl_bounds ~loc is_global sized_ty trans =
  ...
  in
  Validate.(
    if is_global && sized_ty = SizedType.SInt && is_valid_transformation
    then Semantic_error.non_int_bounds loc |> error
    else ok ())
...
and semantic_check_var_decl ~loc ~cf sized_ty trans id init is_global =
  ...
  Validate.(
    let checked_trans =
      checked_stmt
      >>= fun ust -> semantic_check_transformation cf (SizedType.to_unsized ust) trans
    in
...
and semantic_check_fundef_overloaded ~loc id arg_tys rt =
  Validate.(
    ...
    if Symbol_table.check_is_unassigned vm id.name
    then (
      match Symbol_table.look vm id.name with
...
and semantic_check_fundef_decl ~loc id body =
  Validate.(
    ...
    | { stmt = Skip; _ } ->
      if Symbol_table.check_is_unassigned vm id.name
      then error @@ Semantic_error.fn_decl_without_def loc
      else (
...
and semantic_check_pdf_fundef_first_arg_ty ~loc id arg_tys =
  Validate.(
    ...
    let is_real_type = function
      ...
      | _ -> false
    in
    if String.is_suffix id.name ~suffix:"_lpdf"
    then
      List.hd arg_tys
      |> Option.value_map
           ~default:(error @@ Semantic_error.prob_density_non_real_variate loc None)
           ~f:(fun (_, rt) ->
             if is_real_type rt
             then ok ()
             else error @@ Semantic_error.prob_density_non_real_variate loc (Some rt))
    else ok ())
...
and semantic_check_pmf_fundef_first_arg_ty ~loc id arg_tys =
  Validate.(
    ...
    let is_int_type = function
      ...
      | _ -> false
    in
    if String.is_suffix id.name ~suffix:"_lpmf"
    then
      List.hd arg_tys
      |> Option.value_map
           ...
           ~f:(fun (_, rt) ->
             if is_int_type rt
             then ok ()
             else error @@ Semantic_error.prob_mass_non_int_variate loc (Some rt))
    else ok ())

(* All function arguments are distinct *)
and semantic_check_fundef_distinct_arg_ids ~loc arg_names =
  Validate.(
    if dup_exists arg_names
    then error @@ Semantic_error.duplicate_arg_names loc
    else ok ())
...
and semantic_check_statement cf (s : Ast.untyped_statement)
  ...
  | VarDecl { decl_type = Sized st; transformation; identifier; initial_value; is_global }
    ->
    semantic_check_var_decl ~loc ~cf st transformation identifier initial_value is_global
...
let check_fun_def_body_in_block = function
  ...
  | { stmt = FunDef { body = { stmt = Skip; _ }; _ }; _ } -> Validate.ok ()
...
let semantic_check_functions_have_defn function_block_stmts_opt =
  Validate.(
    if Symbol_table.check_some_id_is_unassigned vm
    ...
    then (
      ...
      | Some [] | None -> ok ()
      | Some ls ->
        List.map ~f:check_fun_def_body_in_block ls |> sequence |> map ~f:(fun _ -> ())))
;;
...
let semantic_check_program
  ...
  let ufb =
    Validate.(
      ...
      >>= fun xs -> semantic_check_functions_have_defn xs |> map ~f:(fun _ -> xs))
  in
  ...
  let check_correctness_invariant (decorated_ast : typed_program) : typed_program =
    if compare_untyped_program
         { functionblock = fb
         ; datablock = db
         ; transformeddatablock = tdb
         ; parametersblock = pb
