...
let reserved_keywords =
  ...
  ; "void"; "volatile"; "wchar_t"; "while"; "xor"; "xor_eq"; "functions"; "data"
  ; "parameters"; "model"; "return"; "if"; "else"; "while"; "for"; "in"; "break"
  ; "continue"; "void"; "int"; "real"; "vector"; "row_vector"; "matrix"
  ; "ordered"; "positive_ordered"; "simplex"; "unit_vector"
  ; "cholesky_factor_corr"; "cholesky_factor_cov"; "corr_matrix"; "cov_matrix"
  ; "print"; "reject"; "target"; "get_lp"; "profile" ]
...
let semantic_check_variadic_ode ~is_cond_dist ~loc id es =
  ...
  let fun_arg_match (x_ad, x_t) (y_ad, y_t) =
    ...
    && UnsizedType.autodifftype_can_convert x_ad y_ad in
  ...
  | {emeta= {type_= UnsizedType.UFun (fun_args, ReturnType return_type); _}; _}
    :: args ->
      ...
      let mandatory_args, variadic_args =
        List.split_n args num_of_mandatory_args in
            ...
            let hint ppf () =
              ...
              | _ ->
                  Fmt.pf ppf "%a * 1.0 / %a" Pretty_printing.pp_expression x
                    Pretty_printing.pp_expression y in
            Fmt.pr
...
let semantic_check_transformation cf ut = function
  ...
  | LowerUpper (e1, e2) ->
      ...
      and ue2 =
        semantic_check_expression_of_scalar_or_type cf ut e2 "Upper bound" in
  ...
  | OffsetMultiplier (e1, e2) ->
      ...
      and ue2 =
        semantic_check_expression_of_scalar_or_type cf ut e2 "Multiplier" in
      Validate.liftA2
...
and semantic_check_while ~loc ~cf e s =
  ...
  and ue =
    semantic_check_expression_of_int_or_real_type cf e "Condition in while-loop"
  in
  Validate.liftA2
    (fun ue us ->
      mk_typed_statement
...
and semantic_check_var_decl ~loc ~cf sized_ty trans id init is_global =
  ...
  Validate.(
    let checked_trans =
      checked_stmt
      >>= fun ust ->
      semantic_check_transformation cf (SizedType.to_unsized ust) trans in
...
and semantic_check_fundef_distinct_arg_ids ~loc arg_names =
  Validate.(
    if dup_exists arg_names then error @@ Semantic_error.duplicate_arg_names loc
    else ok ())
      ...
      raise_s
        [%message
          "Type checked AST does not match original AST. Please file a bug!"
            (decorated_ast : typed_program)] in
  let check_correctness_invariant_validate =
    Validate.map ~f:check_correctness_invariant in
  Validate.(
