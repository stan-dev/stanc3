...
let find_udf_log_suffix = function
  | { stmt=
        FunDef
          { funname= {name; _}
          ; arguments= (_, ((UReal | UInt) as type_), _) :: _; _ }
    ; smeta= _ }
...
let rec collect_deprecated_expr (acc : (Location_span.t * string) list)
  ...
  | FunApp ((StanLib | UserDefined), {name; _}, l) ->
      let w =
      ...
      let acc =
        ...
        @ [ ( loc
            , "Condition of type real is deprecated and will be disallowed in \
               Stan 2.34. Use an explicit != 0 comparison instead. This can \
               be automatically changed using the canonicalize flag for stanc"
            ) ]
      in
