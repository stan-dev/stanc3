...
(** Check that the functions in the list [requires_higher_order_autodiff] cannot
    (**transitively**) call stan math functions that don't have second
    derivative support.
...
(** Laplace functions are also special, in two ways: 1. The general forms accept
    two UDF callbacks, not just one 2. The callback arguments are passed as a
    tuple, not just as trailing arguments. (this is a requirement of the first
    point, but worth noting separately). *)
