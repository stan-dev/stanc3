...
module TypeError = struct
  type t =
    ...
    | IllTypedReduceSum of
        string * UnsizedType.t list * (UnsizedType.autodifftype * UnsizedType.t) list
  ...
  let pp ppf = function
    | MismatchedReturnTypes (rt1, rt2) ->
      Fmt.pf
        ppf
        "Branches of function definition need to have the same return type. Instead, \
         found return types %a and %a."
    ...
    | MismatchedArrayTypes (t1, t2) ->
      Fmt.pf
        ppf
        "Array expression must have entries of consistent type. Expected %a but found %a."
        UnsizedType.pp
        t1
        UnsizedType.pp
        t2
    | InvalidRowVectorTypes ty ->
      Fmt.pf
        ppf
        "Row_vector expression must have all int or real entries. Found type %a."
        UnsizedType.pp
        ty
    | InvalidMatrixTypes ty ->
      Fmt.pf
        ...
        ty
    | IntExpected (name, ut) ->
      Fmt.pf ppf "%s must be of type int. Instead found type %a." name UnsizedType.pp ut
    | IntOrRealExpected (name, ut) ->
      Fmt.pf
        ppf
        "%s must be of type int or real. Instead found type %a."
        name
        UnsizedType.pp
        ut
    | TypeExpected (name, (UInt | UReal), ut) ->
      Fmt.pf ppf "%s must be a scalar. Instead found type %a." name UnsizedType.pp ut
    | TypeExpected (name, et, ut) ->
      Fmt.pf
        ppf
        "%s must be a scalar or of type %a. Instead found type %a."
        name
        UnsizedType.pp
        et
        UnsizedType.pp
    ...
    | IntIntArrayOrRangeExpected ut ->
      Fmt.pf
        ppf
        "Index must be of type int or int[] or must be a range. Instead found type %a."
        UnsizedType.pp
        ut
    | ArrayVectorRowVectorMatrixExpected ut ->
      Fmt.pf
        ppf
        "Foreach-loop must be over array, vector, row_vector or matrix. Instead found \
    ...
    | IllTypedAssignment ((OperatorAssign op as assignop), lt, rt) ->
      Fmt.pf
        ppf
        "@[<h>Ill-typed arguments supplied to assignment operator %s: lhs has type %a \
         and rhs has type %a. Available signatures:@]%s"
    ...
    | IllTypedAssignment (assignop, lt, rt) ->
      Fmt.pf
        ppf
        "Ill-typed arguments supplied to assignment operator %s: lhs has type %a and rhs \
         has type %a"
        (Pretty_printing.pretty_print_assignmentoperator assignop)
        UnsizedType.pp
        lt
    ...
    | IllTypedTernaryIf (UInt, ut2, ut3) ->
      Fmt.pf
        ppf
        "Type mismatch in ternary expression, expression when true is: %a; expression \
         when false is: %a"
        UnsizedType.pp
        ut2
        UnsizedType.pp
    ...
    | IllTypedReduceSum (name, arg_tys, args) ->
      ...
      Fmt.pf
        ppf
        "Ill-typed arguments supplied to function '%s'. Expected arguments:@[<h>%a@]\n\
    ...
    | IllTypedReduceSumGeneric (name, arg_tys) ->
      ...
      let type_string (a, b, c, d) i =
        Fmt.strf
          "(T[%s], %a, %a, ...) => %a, T[%s], %a, ...\n"
          (n_commas (i - 1))
          Pretty_printing.pp_unsizedtype
          a
          Pretty_printing.pp_unsizedtype
          b
          Pretty_printing.pp_unsizedtype
      ...
      Fmt.pf
        ...
        "Ill-typed arguments supplied to function '%s'. Available arguments:\n\
         %sWhere T is any one of int, real, vector, row_vector or matrix.@[<h>Instead \
         supplied arguments of incompatible type: %a@]"
        name
    ...
    | NotIndexable ut ->
      Fmt.pf
        ppf
        "Only expressions of array, matrix, row_vector and vector type may be indexed. \
         Instead, found type %a."
        UnsizedType.pp
        ut
    | ReturningFnExpectedNonReturningFound fn_name ->
      Fmt.pf
        ppf
        "A returning function was expected but a non-returning function '%s' was \
         supplied."
        fn_name
    | NonReturningFnExpectedReturningFound fn_name ->
      Fmt.pf
        ppf
        "A non-returning function was expected but a returning function '%s' was \
         supplied."
        fn_name
    | ReturningFnExpectedNonFnFound fn_name ->
      Fmt.pf
        ppf
        "A returning function was expected but a non-function value '%s' was supplied."
        fn_name
    | NonReturningFnExpectedNonFnFound fn_name ->
      Fmt.pf
        ppf
        "A non-returning function was expected but a non-function value '%s' was \
         supplied."
        fn_name
    | ReturningFnExpectedUndeclaredIdentFound fn_name ->
      Fmt.pf
        ppf
        "A returning function was expected but an undeclared identifier '%s' was \
         supplied."
        fn_name
    | NonReturningFnExpectedUndeclaredIdentFound fn_name ->
      Fmt.pf
        ppf
        "A non-returning function was expected but an undeclared identifier '%s' was \
         supplied."
        fn_name
    ...
    | IllTypedUserDefinedFunctionApp (name, listed_tys, return_ty, arg_tys) ->
      Fmt.pf
        ppf
        "Ill-typed arguments supplied to function '%s'. Available signatures:%a\n\
         @[<h>Instead supplied arguments of incompatible type: %a.@]"
        name
        UnsizedType.pp
    ...
    | IllTypedBinaryOperator (op, lt, rt) ->
      Fmt.pf
        ppf
        "Ill-typed arguments supplied to infix operator %a. Available signatures: \
         %s@[<h>Instead supplied arguments of incompatible type: %a, %a.@]"
        Operator.pp
        op
    ...
    | IllTypedPrefixOperator (op, ut) ->
      Fmt.pf
        ppf
        "Ill-typed arguments supplied to prefix operator %a. Available signatures: \
         %s@[<h>Instead supplied argument of incompatible type: %a.@]"
        Operator.pp
        op
        (Stan_math_signatures.pretty_print_math_lib_operator_sigs op
        |> String.concat ~sep:"\n")
    ...
    | IllTypedPostfixOperator (op, ut) ->
      Fmt.pf
        ppf
        "Ill-typed arguments supplied to postfix operator %a. Available signatures: %s\n\
...
module IdentifierError = struct
  ...
  let pp ppf = function
    | IsStanMathName name ->
      Fmt.pf ppf "Identifier '%s' clashes with Stan Math library function." name
    | InUse name -> Fmt.pf ppf "Identifier '%s' is already in use." name
    | IsModelName name -> Fmt.pf ppf "Identifier '%s' clashes with model name." name
    | IsKeyword name -> Fmt.pf ppf "Identifier '%s' clashes with reserved keyword." name
...
module ExpressionError = struct
  ...
  let pp ppf = function
    | InvalidMapRectFn fn_name ->
      Fmt.pf
        ppf
        "Mapped function cannot be an _rng or _lp function, found function name: %s"
        fn_name
    | InvalidSizeDeclRng ->
      Fmt.pf
        ppf
    ...
    | InvalidRngFunction ->
      Fmt.pf
        ppf
        "Random number generators are only allowed in transformed data block, generated \
         quantities block or user-defined functions with names ending in _rng."
    | ConditionalNotationNotAllowed ->
      Fmt.pf
        ppf
        "Only functions with names ending in _lpdf, _lpmf, _lcdf, _lccdf can make use of \
         conditional notation."
    | ConditioningRequired ->
      Fmt.pf
        ppf
        "Probabilty functions with suffixes _lpdf, _lpmf, _lcdf, and _lccdf, require a \
         vertical bar (|) between the first two arguments."
    | NotPrintable -> Fmt.pf ppf "Functions cannot be printed."
    | EmptyArray -> Fmt.pf ppf "Array expressions must contain at least one element."
    | IntTooLarge -> Fmt.pf ppf "Integer literal cannot be larger than 2_147_483_647."
  ;;
end
...
module StatementError = struct
  ...
  let pp ppf = function
    | CannotAssignToReadOnly name ->
      Fmt.pf ppf "Cannot assign to function argument or loop identifier '%s'." name
    | CannotAssignToGlobal name ->
      Fmt.pf ppf "Cannot assign to global variable '%s' declared in previous blocks." name
    | TargetPlusEqualsOutsideModelOrLogProb ->
      Fmt.pf
        ppf
        "Target can only be accessed in the model block or in definitions of functions \
         with the suffix _lp."
    | InvalidSamplingPDForPMF ->
      Fmt.pf
        ppf
...
|}
    ...
    | InvalidSamplingNoSuchDistribution name ->
      Fmt.pf
        ppf
        "Ill-typed arguments to '~' statement. No distribution '%s' was found with the \
         correct signature."
        name
    | InvalidTruncationCDForCCDF ->
      Fmt.pf
        ppf
        "Truncation is only defined if distribution has _lcdf and _lccdf functions \
         implemented with appropriate signature."
    | MultivariateTruncation ->
      Fmt.pf ppf "Outcomes in truncated distributions must be univariate."
    | BreakOutsideLoop -> Fmt.pf ppf "Break statements may only be used in loops."
    | ContinueOutsideLoop -> Fmt.pf ppf "Continue statements may only be used in loops."
    | ExpressionReturnOutsideReturningFn ->
      Fmt.pf
        ppf
        "Expression return statements may only be used inside returning function \
         definitions."
    | VoidReturnOutsideNonReturningFn ->
      Fmt.pf
        ppf
        "Void return statements may only be used inside non-returning function \
         definitions."
    | NonDataVariableSizeDecl ->
      Fmt.pf ppf "Non-data variables are not allowed in top level size declarations."
    | NonIntBounds ->
      Fmt.pf ppf "Bounds of integer variable must be of type int. Found type real."
    | TransformedParamsInt -> Fmt.pf ppf "(Transformed) Parameters cannot be integers."
    | MismatchFunDefDecl (name, Some ut) ->
      Fmt.pf
        ppf
        "Function '%s' has already been declared to have type %a"
        name
        UnsizedType.pp
        ut
    | MismatchFunDefDecl (name, None) ->
      Fmt.pf
        ppf
        "Function '%s' has already been declared but type cannot be determined."
        name
    | FunDeclExists name ->
      Fmt.pf ppf "Function '%s' has already been declared. A definition is expected." name
    | FunDeclNoDefn ->
    ...
    | NonRealProbFunDef ->
      Fmt.pf
        ppf
        "Real return type required for probability functions ending in _log, _lpdf, \
         _lpmf, _lcdf, or _lccdf."
    | ProbDensityNonRealVariate (Some ut) ->
      Fmt.pf
        ppf
        "Probability density functions require real variates (first argument). Instead \
         found type %a."
        UnsizedType.pp
        ut
    | ProbDensityNonRealVariate _ ->
      Fmt.pf ppf "Probability density functions require real variates (first argument)."
    | ProbMassNonIntVariate (Some ut) ->
      Fmt.pf
        ppf
        "Probability mass functions require integer variates (first argument). Instead \
         found type %a."
        UnsizedType.pp
        ut
    | ProbMassNonIntVariate _ ->
      Fmt.pf ppf "Probability mass functions require integer variates (first argument)."
    | DuplicateArgNames ->
...
let invalid_row_vector_types loc ty = TypeError (loc, TypeError.InvalidRowVectorTypes ty)
let invalid_matrix_types loc ty = TypeError (loc, TypeError.InvalidMatrixTypes ty)
...
let ident_is_keyword loc name = IdentifierError (loc, IdentifierError.IsKeyword name)
let ident_is_model_name loc name = IdentifierError (loc, IdentifierError.IsModelName name)
...
let ident_not_in_scope loc name = IdentifierError (loc, IdentifierError.NotInScope name)

let invalid_map_rect_fn loc name =
  ExpressionError (loc, ExpressionError.InvalidMapRectFn name)
;;

let invalid_decl_rng_fn loc = ExpressionError (loc, ExpressionError.InvalidSizeDeclRng)
let invalid_rng_fn loc = ExpressionError (loc, ExpressionError.InvalidRngFunction)

let conditional_notation_not_allowed loc =
  ExpressionError (loc, ExpressionError.ConditionalNotationNotAllowed)
;;

let conditioning_required loc = ExpressionError (loc, ExpressionError.ConditioningRequired)
...
let break_outside_loop loc = StatementError (loc, StatementError.BreakOutsideLoop)
let continue_outside_loop loc = StatementError (loc, StatementError.ContinueOutsideLoop)
...
let transformed_params_int loc = StatementError (loc, StatementError.TransformedParamsInt)

let mismatched_fn_def_decl loc name ut_opt =
  StatementError (loc, StatementError.MismatchFunDefDecl (name, ut_opt))
;;

let fn_decl_exists loc name = StatementError (loc, StatementError.FunDeclExists name)
let fn_decl_without_def loc = StatementError (loc, StatementError.FunDeclNoDefn)
let fn_decl_needs_block loc = StatementError (loc, StatementError.FunDeclNeedsBlock)
...
let duplicate_arg_names loc = StatementError (loc, StatementError.DuplicateArgNames)
