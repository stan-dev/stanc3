    ...
    | IllTypedReduceSumGeneric (name, arg_tys) ->
        ...
        let type_string (a, b, c, d) i =
          Fmt.strf "(T[%s], %a, %a, ...) => %a, T[%s], %a, ...\n"
            ...
            (n_commas (i - 1))
            Pretty_printing.pp_unsizedtype d in
        let lines =
          List.map
...
module ExpressionError = struct
  ...
  let pp ppf = function
    ...
    | ConditionalNotationNotAllowed ->
        Fmt.pf ppf
          "Only functions with names ending in _lpdf, _lpmf, _lcdf, _lccdf can \
           make use of conditional notation."
    | ConditioningRequired ->
        Fmt.pf ppf
