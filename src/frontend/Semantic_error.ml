  ...
  let pp ppf = function
    ...
    | IllTypedAssignment ((OperatorAssign op as assignop), lt, rt) ->
        Fmt.pf ppf
          ...
          ( Stan_math.pretty_print_math_lib_assignmentoperator_sigs op
    ...
    | IllTypedTernaryIf (ut1, ut2, ut3) ->
        Fmt.pf ppf
          ...
          (Stan_math.pretty_print_all_math_lib_fn_sigs "if_else")
    ...
    | IllTypedStanLibFunctionApp (name, arg_tys) ->
        Fmt.pf ppf
          "Ill-typed arguments supplied to function '%s'. Available \
           ...
           type: %a.@]"
          name
          (Stan_math.pretty_print_all_math_lib_fn_sigs name)
