...
let enter s str ty =
  let (_ : [ `Duplicate | `Ok ]) =
    if !(s.scopedepth) = 0 then Hashtbl.add s.globals ~key:str ~data:() else `Ok
  in
...
let get_read_only s str =
  match Hashtbl.find s.readonly str with
  | Some () -> true
  | _ -> false
;;
...
let set_is_unassigned s str =
  let (_ : [ `Duplicate | `Ok ]) =
    if Hashtbl.mem s.isunassigned str
    then `Ok
...
let is_global s str =
  match Hashtbl.find s.globals str with
  | Some _ -> true
  | _ -> false
;;
