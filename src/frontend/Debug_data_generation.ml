...
let matprod x y =
  ...
  if List.length x <> List.length y_T
  then failwith "Matrix multiplication dim. mismatch"
...
let rec vect_to_mat l m =
  ...
  if len % m <> 0
  then failwith "the length has to be a whole multiple of the partition size"
  else if len = m
  then [ l ]
  else (
...
let gen_vector m n t =
  ...
  | Ordered ->
    ...
    let halfmax = Option.value_exn (List.max_elt l ~compare:compare_float) /. 2. in
  ...
  | UnitVector ->
    let l = repeat_th n (fun _ -> Random.float 1.) in
    let sum =
      Float.sqrt (List.fold l ~init:0. ~f:(fun accum elt -> accum +. (elt ** 2.)))
    in
    let l = List.map l ~f:(fun x -> x /. sum) in
    wrap_vector (List.map ~f:wrap_real l)
...
let wrap_real_mat m =
  let mat_wrapped =
    List.map ~f:wrap_row_vector (List.map ~f:(fun x -> List.map ~f:wrap_real x) m)
  in
...
let gen_matrix mm m n t =
  ...
  | _ ->
    { int_two with expr = RowVectorExpr (repeat_th m (fun () -> gen_row_vector mm n t)) }
;;
...
let rec generate_value m st t =
  ...
  | SMatrix (e1, e2) -> gen_matrix m (unwrap_int_exn m e1) (unwrap_int_exn m e2) t
  | SArray (st, e) ->
...
let print_data_prog s =
  ...
  let l, _ =
    List.fold data ~init:([], Map.Poly.empty) ~f:(fun (l, m) decl ->
        ...
        l @ [ var_decl_id decl, value ], Map.set m ~key:(var_decl_id decl) ~data:value)
  in
  let pp ppf (id, value) = Fmt.pf ppf {|@[<hov 2>"%s":@ %a@]|} id pp_value_json value in
