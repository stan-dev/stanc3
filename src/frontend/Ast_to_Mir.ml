...
let rec op_to_funapp op args =
  let argtypes = List.map ~f:(fun x -> x.Ast.emeta.Ast.ad_level, x.emeta.type_) args in
  let type_ =
    Stan_math_signatures.operator_stan_math_return_type op argtypes |> unwrap_return_exn
...
and trans_expr { Ast.expr; Ast.emeta } =
  ...
  | Ast.TernaryIf (cond, ifb, elseb) ->
    Expr.Fixed.Pattern.TernaryIf (trans_expr cond, trans_expr ifb, trans_expr elseb)
    |> ewrap
  ...
  | ArrayExpr eles ->
    FunApp (CompilerInternal, Internal_fun.to_string FnMakeArray, trans_exprs eles)
    |> ewrap
  | RowVectorExpr eles ->
    FunApp (CompilerInternal, Internal_fun.to_string FnMakeRowVec, trans_exprs eles)
    |> ewrap
  | Indexed (lhs, indices) ->
...
and trans_idx = function
  ...
  | Ast.Single e ->
    ...
    | _ -> raise_s [%message "Expecting int or array" (e.emeta.type_ : UnsizedType.t)])
...
let neg_inf =
  Expr.
    ...
    ; meta = Typed.Meta.{ type_ = UReal; loc = Location_span.empty; adlevel = DataOnly }
    }
;;
...
let truncate_dist ud_dists (id : Ast.identifier) ast_obs ast_args t =
  ...
  | TruncateDownFrom ub ->
    ...
    [ trunc Greater ub (targetme ub.emeta.loc (funapp ub.emeta fk fn (ub :: ast_args))) ]
...
let unquote s =
  ...
  then String.drop_suffix (String.drop_prefix s 1) 1
  else s
;;
...
let check_constraint_to_string t (c : constrainaction) =
  ...
  | LowerUpper _ ->
    (match c with
    | Check -> raise_s [%message "LowerUpper is really two other checks tied together"]
    | Constrain | Unconstrain -> "lub")
  | Offset _ | Multiplier _ | OffsetMultiplier _ ->
    (match c with
    | Check -> ""
    | Constrain | Unconstrain -> "offset_multiplier")
  | Identity -> ""
;;
...
let constraint_forl = function
  ...
  | Lower _
  | Upper _
  | LowerUpper _ -> Stmt.Helpers.for_scalar
  | Ordered
  | PositiveOrdered
  | Simplex
  | UnitVector
  ...
  | CholeskyCov
  | Correlation
  | Covariance -> Stmt.Helpers.for_eigen
;;

let same_shape decl_id decl_var id var meta =
  if UnsizedType.is_scalar_type (Expr.Typed.type_of var)
  then []
  else
    [ Stmt.
        { Fixed.pattern =
            NRFunApp
              ( StanLib
              , "check_matching_dims"
              , Expr.Helpers.[ str "constraint"; str decl_id; decl_var; str id; var ] )
        ; meta
        }
    ]
;;
...
let copy_indices indexed (var : Expr.Typed.t) =
  if UnsizedType.is_scalar_type var.meta.type_
  then var
  else (
...
let extra_constraint_args st = function
  ...
  | UnitVector
  | Identity -> []
  | Covariance | Correlation | CholeskyCorr -> [ List.hd_exn (SizedType.dims_of st) ]
...
let param_size transform sizedtype =
  let rec shrink_eigen f st =
    ...
    | SInt | SReal | SRowVector _ ->
      raise_s
        [%message "Expecting SVector or SMatrix, got " (st : Expr.Typed.t SizedType.t)]
  in
  let rec shrink_eigen_mat f st =
    ...
    | SInt | SReal | SRowVector _ | SVector _ ->
      raise_s [%message "Expecting SMatrix, got " (st : Expr.Typed.t SizedType.t)]
  in
  let k_choose_2 k =
  ...
  | Ordered | PositiveOrdered | UnitVector -> sizedtype
  | Simplex -> shrink_eigen (fun d -> Expr.Helpers.(binop d Minus (int 1))) sizedtype
  ...
  | CholeskyCov ->
    ...
    shrink_eigen_mat
      (fun m n ->
        Expr.Helpers.(
          binop (binop (k_choose_2 n) Plus n) Plus (binop (binop m Minus n) Times n)))
      sizedtype
  | Covariance ->
    shrink_eigen (fun k -> Expr.Helpers.(binop k Plus (k_choose_2 k))) sizedtype
;;
...
let remove_possibly_exn pst action loc =
  ...
  | Type.Sized st -> st
  | Unsized _ ->
    raise_s [%message "Error extracting sizedtype" ~action ~loc:(loc : Location_span.t)]
;;
...
let constrain_decl decl_type dconstrain t decl_id decl_var smeta =
  ...
  | Some constraint_str ->
    ...
    in
    let args var =
      (var :: mkstring constraint_str :: extract_transform_args var t) @ extra_args
    in
...
let trans_decl { dconstrain; dadlevel } smeta decl_type transform identifier initial_value
  =
  ...
  let rhs_assignment =
    Option.map
      ~f:(fun e ->
        Stmt.Fixed.{ pattern = Assignment ((decl_id, e.meta.type_, []), e); meta = smeta })
      rhs
    |> Option.to_list
  in
  if Utils.is_user_ident decl_id
  then (
...
let unwrap_block_or_skip = function
  | [ ({ Stmt.Fixed.pattern = Block _; _ } as b) ] | [ ({ pattern = Skip; _ } as b) ] -> b
  | x -> raise_s [%message "Expecting a block or skip, not" (x : Stmt.Located.t list)]
;;
...
let rec trans_stmt ud_dists (declc : decl_context) (ts : Ast.typed_statement) =
  let stmt_typed = ts.stmt
  and smeta = ts.smeta.loc in
  ...
  | Ast.Assignment { assign_lhs; assign_rhs; assign_op } ->
      ...
      { Ast.expr =
          ...
          | _ ->
            Ast.Indexed
              ...
              , assign_indices ))
      ; emeta =
          { Ast.loc = assign_lhs.lmeta.loc; ad_level = lhs_ad_level; type_ = lhs_type_ }
      }
    in
    let rhs =
    ...
    in
    Assignment
      ((assign_identifier.Ast.name, id_type_, List.map ~f:trans_idx assign_indices), rhs)
    |> swrap
  ...
  | Ast.Tilde { arg; distribution; args; truncation } ->
    ...
    in
    let name = distribution.name ^ Utils.proportional_to_distribution_infix ^ suffix in
    let add_dist =
  ...
  | Ast.Print ps ->
    NRFunApp (CompilerInternal, Internal_fun.to_string FnPrint, trans_printables smeta ps)
    |> swrap
  | Ast.Reject ps ->
    NRFunApp (CompilerInternal, Internal_fun.to_string FnReject, trans_printables smeta ps)
    |> swrap
  | Ast.IfThenElse (cond, ifb, elseb) ->
    IfElse (trans_expr cond, trans_single_stmt ifb, Option.map ~f:trans_single_stmt elseb)
    |> swrap
  | Ast.While (cond, body) -> While (trans_expr cond, trans_single_stmt body) |> swrap
  ...
  | Ast.ForEach (loopvar, iteratee, body) ->
    ...
    let bodyfn var =
      Stmt.Fixed.
        { pattern = Block (decl_loopvar :: assignment var :: body_stmts); meta = smeta }
    in
  ...
  | Ast.VarDecl { decl_type; transformation; identifier; initial_value; is_global = _ } ->
    trans_decl
      declc
      smeta
      decl_type
...
let trans_fun_def ud_dists (ts : Ast.typed_statement) =
  ...
  | Ast.FunDef { returntype; funname; arguments; body } ->
    [ Program.
        ...
        ; fdbody =
            trans_stmt ud_dists { dconstrain = None; dadlevel = AutoDiffable } body
            |> unwrap_block_or_skip
        ; fdloc = ts.smeta.loc
        }
    ]
  | _ -> raise_s [%message "Found non-function definition statement in function block"]
;;
...
let trans_prog filename (p : Ast.typed_program) : Program.Typed.t =
  ...
  and input_vars = map get_name_size datablock |> List.map ~f:(fun (n, st, _) -> n, st) in
  ...
  in
  let prepare_data =
    datab @ map (trans_stmt { declc with dconstrain = Some Check }) transformeddatablock
    |> migrate_checks_to_end_of_block
  in
  ...
  in
  let gen_from_block declc block = map (trans_stmt declc) (get_block block p) in
