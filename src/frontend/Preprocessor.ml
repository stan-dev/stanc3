...
let rec try_open_in paths fname pos =
  ...
  | [] ->
    raise
      (Errors.SyntaxError
         (Include
            ( "Could not find include file " ^ fname ^ " in specified include paths.\n"
    ...
    (try
       ...
       , sprintf
           ...
           (Middle.Location.to_string
              (Middle.Location.of_position_exn (Stack.top_exn include_stack).lex_start_p))
       )
     with
...
let maybe_remove_quotes str =
  let open String in
  if is_prefix str ~prefix:"\"" && is_suffix str ~suffix:"\""
...
let try_get_new_lexbuf fname pos =
  ...
  new_lexbuf.lex_start_p <- { pos_fname = path; pos_lnum = 1; pos_bol = 0; pos_cnum = 0 };
