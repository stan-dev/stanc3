...
module Make (StdLibrary : Std_library_utils.Library) : TYPECHECKER = struct
  ...
  let verify_name_fresh_udf loc tenv name =
    if
      ...
      StdLibrary.is_not_overloadable name
...
*)
  ...
  let library_function_return_type name arg_tys =
    match Hashtbl.find StdLibrary.variadic_signatures name with
    | Some {return_type; _} -> Some (UnsizedType.ReturnType return_type)
    | None when StdLibrary.is_special_function_name name ->
        StdLibrary.special_function_returntype name
  ...
  let check_id cf loc tenv id =
    ...
    | [] ->
        ...
        |> error
    | {kind= `StanMath; _} :: _ ->
        ( calculate_autodifftype cf.current_block MathLibrary
    ...
    | {kind= `Variable {origin; _}; type_} :: _ ->
        (calculate_autodifftype cf.current_block origin type_, type_)
    | { kind= `UserDefined | `UserDeclared _
      ...
      :: _ ->
        let type_ =
          UnsizedType.UFun
            (args, rt, Fun_kind.suffix_from_name id.name, mem_pattern) in
        (calculate_autodifftype cf.current_block Functions type_, type_)
...
*)
  ...
  let check_normal_fn ~is_cond_dist loc tenv id es =
    ...
    | [] ->
        ...
        | Some (prefix, suffix) -> (
            let known_families = StdLibrary.distribution_families in
  ...
  let rec check_fn ~is_cond_dist loc cf tenv id (tes : Ast.typed_expression list)
      =
    if StdLibrary.is_special_function_name id.name then
      StdLibrary.check_special_fn ~is_cond_dist loc cf.current_block tenv id tes
    else if StdLibrary.is_variadic_function_name id.name then
  ...
  and check_variadic ~is_cond_dist loc cf tenv id tes =
    let Std_library_utils.
      ...
      Hashtbl.find_exn StdLibrary.variadic_signatures id.name in
  ...
  and check_expression cf tenv ({emeta; expr} : Ast.untyped_expression) :
    ...
    | RealNumeral s ->
        mk_typed_expression ~expr:(RealNumeral s) ~ad_level:DataOnly
          ~type_:UReal ~loc
    | ImagNumeral s ->
    ...
    | GetLP ->
        ...
        then
          ...
          mk_typed_expression ~expr:GetLP
            ~ad_level:
              (calculate_autodifftype cf.current_block cf.current_block UReal)
            ~type_:UReal ~loc
    | GetTarget ->
        (* Target+= can only be used in model and functions with right suffix (same for tilde etc) *)
        if
  ...
  *)
  ...
  let check_incr_logprob loc cf tenv e =
    ...
    verify_target_pe_usage loc cf ;
    verify_target_pe_expr_type loc te ;
    mk_typed_statement ~stmt:(IncrementLogProb te) ~return_type:NoReturnType
      ~loc
  ...
  and list_until_escape xs =
    let rec aux accu = function
      | [next; next'] when stmt_is_escape next' ->
  ...
  *)
  let rec check_if_then_else loc cf tenv pred_e s_true s_false_opt =
    ...
    let ts_false_opt =
      s_false_opt
      |> Option.map ~f:(check_statement cf tenv)
      |> Option.map ~f:snd in
    let te =
    ...
    let return_type =
      try_compute_ifthenelse_statement_returntype loc srt1 srt2 in
