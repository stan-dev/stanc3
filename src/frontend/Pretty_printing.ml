...
let rec unwind_array_type = function
  | Middle.UnsizedType.UArray ut ->
    (match unwind_array_type ut with
    | ut2, d -> ut2, d + 1)
  | ut -> ut, 0
;;
...
and pp_truncation ppf = function
  ...
  | TruncateBetween (e1, e2) -> Fmt.pf ppf " T[%a, %a]" pp_expression e1 pp_expression e2

and pp_printable ppf = function
  | PString s -> Fmt.pf ppf "%s" s
  | PExpr e -> pp_expression ppf e

and pp_list_of_printables ppf l = Fmt.(list ~sep:comma_no_break pp_printable) ppf l
...
and pp_sizedtype ppf = function
  ...
  | SMatrix (e1, e2) -> Fmt.pf ppf "matrix[%a, %a]" pp_expression e1 pp_expression e2
...
and pp_transformed_type ppf (pst, trans) =
  ...
  let sizes_fmt =
    ...
    | Sized (SArray _) | Unsized _ | Sized Middle.SizedType.SInt | Sized SReal -> Fmt.nop
  in
  let cov_sizes_fmt =
    match pst with
    | Sized (SMatrix (e1, e2)) ->
      if e1 = e2
      then Fmt.const (fun ppf -> Fmt.pf ppf "[%a]" pp_expression) e1
      else Fmt.const (fun ppf -> Fmt.pf ppf "[%a, %a]" pp_expression e1 pp_expression) e2
    | _ -> Fmt.nop
  in
  match trans with
  | Middle.Program.Identity -> Fmt.pf ppf "%a%a" unsizedtype_fmt () sizes_fmt ()
  | Lower _ | Upper _ | LowerUpper _ | Offset _ | Multiplier _ | OffsetMultiplier _ ->
...
and pp_array_dims ppf = function
  ...
  | es ->
    Fmt.pf ppf "[";
    with_box ppf 0 (fun () -> Fmt.pf ppf "%a]" pp_list_of_expression (List.rev es))
...
and pp_statement ppf ({ stmt = s_content; _ } as ss) =
  ...
  | Assignment { assign_lhs = l; assign_op = assop; assign_rhs = e } ->
    with_hbox ppf (fun () ->
        Fmt.pf ppf "%a %a %a;" pp_lvalue l pp_assignmentoperator assop pp_expression e)
  | NRFunApp (_, id, es) ->
    Fmt.pf ppf "%a(" pp_identifier id;
    with_box ppf 0 (fun () -> Fmt.pf ppf "%a);" pp_list_of_expression es)
  | TargetPE e -> Fmt.pf ppf "target += %a;" pp_expression e
  | IncrementLogProb e ->
    with_hbox ppf (fun () -> Fmt.pf ppf "increment_log_prob(%a);" pp_expression e)
  ...
  | Return e -> with_hbox ppf (fun () -> Fmt.pf ppf "return %a;" pp_expression e)
  ...
  | Skip -> Fmt.pf ppf ";"
  | IfThenElse (_, _, _) -> with_vbox ppf 0 (fun () -> pp_recursive_ifthenelse ppf ss)
  ...
  | FunDef { returntype = rt; funname = id; arguments = args; body = b } ->
    ...
    with_box ppf 0 (fun () -> Fmt.pf ppf "%a" (Fmt.list ~sep:Fmt.comma pp_args) args);
    (match b with
...
let check_correctness prog pretty =
  let result_ast =
    Errors.without_warnings (Parse.parse_string Parser.Incremental.program) pretty
  in
