...
let typed_ast_to_string x = [%sexp (x : Ast.typed_program)] |> Sexp.to_string_hum
let typed_ast_logger t = if !typed_ast_printing then print_endline (typed_ast_to_string t)
