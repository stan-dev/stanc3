...
type identifier =
  { name : string
  ; id_loc : Location_span.t sexp_opaque [@compare.ignore]
  }
...
type typed_statement =
  (typed_expression, stmt_typed_located_meta, typed_lval, fun_kind) statement_with
[@@deriving sexp, compare, map, hash]

let mk_typed_statement ~stmt ~loc ~return_type = { stmt; smeta = { loc; return_type } }
...
let rec untyped_expression_of_typed_expression ({ expr; emeta } : typed_expression)
    : untyped_expression
  =
  { expr = map_expression untyped_expression_of_typed_expression (fun _ -> ()) expr
  ; emeta = { loc = emeta.loc }
  }
;;

let rec untyped_lvalue_of_typed_lvalue ({ lval; lmeta } : typed_lval) : untyped_lval =
  { lval =
      map_lvalue
        untyped_lvalue_of_typed_lvalue
...
let rec untyped_statement_of_typed_statement { stmt; smeta } =
  { stmt =
      map_statement
        untyped_expression_of_typed_expression
        untyped_statement_of_typed_statement
        untyped_lvalue_of_typed_lvalue
        (fun _ -> ())
        stmt
  ; smeta = { loc = smeta.loc }
