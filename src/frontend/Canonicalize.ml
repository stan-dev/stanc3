...
let deprecated_distributions =
  String.Map.of_alist_exn
    (List.concat_map
       Middle.Stan_math_signatures.distributions
...
let is_distribution name = Option.is_some (String.Map.find deprecated_distributions name)
  ...
  List.filter_map
    ~f:(function
      | { stmt = FunDef { funname = { name; _ }; _ }; _ } ->
        if String.is_suffix ~suffix:"_log_lpdf" name
           || String.is_suffix ~suffix:"_log_lpmf" name
        then Some (String.drop_suffix name 5)
        else if String.is_suffix ~suffix:"_log_log" name
        then Some (String.drop_suffix name 4)
        else None
      | _ -> None)
...
let without_suffix user_dists name =
  if String.is_suffix ~suffix:"_lpdf" name || String.is_suffix ~suffix:"_lpmf" name
  then String.drop_suffix name 5
  else if String.is_suffix ~suffix:"_log" name
          && not
               (is_distribution (name ^ "_log") || List.exists ~f:(( = ) name) user_dists)
...
let rec replace_deprecated_expr { expr; emeta } =
  let expr =
    ...
    | FunApp (UserDefined, { name; id_loc }, e) ->
      ...
      | Some newname ->
        CondDistApp
          (UserDefined, { name = newname; id_loc }, List.map ~f:replace_deprecated_expr e)
      | None ->
        FunApp (UserDefined, { name; id_loc }, List.map ~f:replace_deprecated_expr e))
  ...
  let stmt =
    ...
    | FunDef { returntype; funname = { name; id_loc }; arguments; body } ->
      FunDef
        ...
        ; funname =
            { name =
                Option.value ~default:name (String.Table.find deprecated_userdefined name)
            ; id_loc
            }
        ; arguments
...
let rec no_parens { expr; emeta } =
  match expr with
  | Paren e -> no_parens e
  | Variable _ | IntNumeral _ | RealNumeral _ | GetLP | GetTarget -> { expr; emeta }
...
and keep_parens { expr; emeta } =
  ...
  | Paren ({ expr = TernaryIf _; _ } as e) -> { expr = Paren (no_parens e); emeta }
...
let rec parens_stmt { stmt; smeta } =
  let stmt =
    match stmt with
    | VarDecl
        { decl_type = d; transformation = t; identifier; initial_value = init; is_global }
      ->
      VarDecl
...
let repair_syntax program : untyped_program =
  map_program (repair_syntax_stmt (userdef_distributions program.functionblock)) program
;;
       ...
       ~f:
         (List.iter ~f:(function
             | { stmt =
               ...
               let newname =
                 if String.is_suffix ~suffix:"_cdf_log" name
                 then String.drop_suffix name 8 ^ "_lcdf"
                 else if String.is_suffix ~suffix:"_ccdf_log" name
                 then String.drop_suffix name 9 ^ "_lccdf"
                 else if Middle.UnsizedType.is_real_type type_
                 then String.drop_suffix name 4 ^ "_lpdf"
