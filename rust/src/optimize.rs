...
fn walk_expr(e: &Expr, optimizer: &mut impl ExprRewriter) -> Expr {
    let inner = match e {
        FnApp(name, args) => {
            FnApp(name.clone(), args.iter().map(|e| walk_expr(e, optimizer)).collect())
        },
...
fn walk_statements(statements: &Vec<Statement>, optimizer: &mut impl ExprRewriter) -> Vec<Statement> {
    statements.iter().map(|s| match s {
        Statement::Assign(symbol, expr) => Statement::Assign(symbol.clone(), walk_expr(expr, optimizer)),
...
pub fn optimize(statements: Vec<Statement>) -> Vec<Statement> {
    ...
    const_fn_walker.fn_cache.into_iter().map(|(expr, var)| {
        Statement::Assign(var, expr)
    }).chain(new_ast.into_iter()).collect()
}
        ...
        match e {
            ...
            FnApp(ref name, ref args) => {
                ...
                if args_are_const {
                    let symbol = self
                        .fn_cache
                        .entry(FnApp(name.clone(), args.clone()))
                        .or_insert(String::from("new symbol lol XXX"));
                    Var(symbol.clone())
                } else {
                    FnApp(name.clone(), args.clone())
                }
            }
            anything_else => anything_else.clone(),
        }
    }
}
