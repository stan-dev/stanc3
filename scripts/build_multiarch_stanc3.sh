...
docker run -u 990:986 --volumes-from=$(docker ps -qf "ancestor=stanorg/stanc3:staticfi"):rw stanorg/stanc3:multiarchfi@$SHA /bin/bash -c "ls -lhart && whoami $&& echo ${UID} && getent group sudo"
...
# docker buildx build -t stanorg/stanc3:multiarchfi --platform linux/arm/v6,linux/arm/v7,linux/arm64,linux/ppc64le,linux/mips64le,linux/s390x --no-cache --build-arg PUID=990 --progress=plain --build-arg PGID=986 --push .
