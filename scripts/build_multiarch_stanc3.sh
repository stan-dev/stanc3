...
docker run -p -w $(pwd) -v $(pwd):$(pwd):rw,z stanorg/stanc3:multiarch@$SHA /bin/bash -c "cd $(pwd) && id && id -u && id -un && id -G && id -Gn && eval \$(opam env) && dune build @install --profile static"
