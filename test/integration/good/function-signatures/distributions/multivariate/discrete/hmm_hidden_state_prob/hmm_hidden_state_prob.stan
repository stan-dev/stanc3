...
transformed data {
  matrix[K, N + 1] = hmm_hidden_state_prob(log_omega, Gamma, rho);
}
...
transformed parameters {
 ...
 prob = hmm_hidden_state_prob(log_omega, Gamma, rho);
 prob = hmm_hidden_state_prob(log_omega_v, Gamma, rho);
 prob = hmm_hidden_state_prob(log_omega, Gamma_v, rho);
 prob = hmm_hidden_state_prob(log_omega, Gamma, rho_v);
 prob = hmm_hidden_state_prob(log_omega_v, Gamma_v, rho);
 prob = hmm_hidden_state_prob(log_omega_v, Gamma, rho_v);
 prob = hmm_hidden_state_prob(log_omega, Gamma_v, rho_v);
 prob = hmm_hidden_state_prob(log_omega_v, Gamma_v, rho_v);
}
