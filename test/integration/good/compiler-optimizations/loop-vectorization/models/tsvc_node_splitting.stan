...
model {
  // 1. TSVC s241 (NodeSplitting), UoB-HPC/TSVC_2 src/tsvc.c
  ...
  // without node splitting).
  {
    ...
    for (n in 1 : N) {
      a1[n] = b1[n] .* c[n] .* d1[n];
      b1[n] = a1[n] .* a1[n + 1] .* d1[n];
    }
    target += sum(a1);
    target += sum(b1);
  }

  // 2. TSVC s243 (NodeSplitting), UoB-HPC/TSVC_2 src/tsvc.c
  ...
  // -> seq.
  {
    ...
    for (n in 1 : N) {
      a2[n] = b2[n] + c[n] .* d2[n];
      b2[n] = a2[n] + d2[n] .* e[n];
      a2[n] = b2[n] + a2[n + 1] .* d2[n];
    }
    target += sum(a2);
    target += sum(b2);
  }

  // 3. TSVC s244 (NodeSplitting), UoB-HPC/TSVC_2 src/tsvc.c
  ...
  // Design: output S3->S1 {Lt, 1}, S1->S2 {Eq}, S2->S3 {Eq}: cycle -> seq.
  {
    ...
    for (n in 1 : N) {
      a3[n] = b3[n] + c[n] .* d3[n];
      b3[n] = c[n] + b3[n];
      a3[n + 1] = b3[n] + a3[n + 1] .* d3[n];
    }
    target += sum(a3);
    target += sum(b3);
  }

  // 4. TSVC s1244 (NodeSplitting), UoB-HPC/TSVC_2 src/tsvc.c
  ...
  // 5. TSVC s2244 (NodeSplitting), UoB-HPC/TSVC_2 src/tsvc.c
  ...
  // Emitted: vec `a5[2:(N+1)] = b5 + e`, vec `a5[:] = b5 + c` (= GCC f5).
  {
    vector[N + 1] a5 = a0;
    vector[N] b5 = b0;
    for (n in 1 : N) {
