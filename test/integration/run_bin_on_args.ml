...
let maybe_convert_cmd_to_windows cmd =
  ...
  if Stdlib.Sys.file_exists (to_windows path) then to_windows cmd else cmd

let run_capturing_output cmd =
  let noflags = Array.create ~len:0 "" in
  let stdout, stdin, stderr =
    Caml_unix.open_process_full (maybe_convert_cmd_to_windows cmd) noflags in
...
let () =
  ...
  let dirs =
    Array.(sub Stdlib.Sys.argv ~pos:2 ~len:(length Stdlib.Sys.argv - 2)) in
