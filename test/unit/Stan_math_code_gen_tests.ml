...
let%expect_test "udf-expressions" =
  ...
  { fdrt= Some UMatrix
  ; fdname= "sars"
  ; fdargs= [(DataOnly, "x", UMatrix); (AutoDiffable, "y", URowVector);(AutoDiffable, "z", URowVector); (AutoDiffable, "w", UArray UMatrix)]
  ; fdbody=
      Stmt.Fixed.Pattern.Return
        (Some
           (w @@ FunApp (StanLib, "add", [w @@ Var "x"; w @@ Lit (Int, "1")])))
  ...
  [%expect
    ...
    Eigen::Matrix<stan::promote_args_t<value_type_t<T0__>, value_type_t<T1__>,
    value_type_t<T2__>,
    T3__>, -1, -1>
